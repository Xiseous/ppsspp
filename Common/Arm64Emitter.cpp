...
bool IsImmLogical(uint64_t value, unsigned int width, unsigned int *n,
                  unsigned int *imm_s, unsigned int *imm_r) {
...
const u8 *ARM64XEmitter::GetCodePointer() const { return m_code; }

u8 *ARM64XEmitter::GetWritableCodePtr() { return m_writable; }
...
void ARM64XEmitter::FlushIcacheSection(const u8 *start, const u8 *end) {
  ...
  // start);
  sys_cache_control(kCacheFunctionPrepareForExecution, (void *)start,
                    end - start);
  ...
  if (cpu_info.sQuirks.bExynos8890DifferingCachelineSizes) {
    ...
    icache_line_size = isize =
        icache_line_size < isize ? icache_line_size : isize;
    dcache_line_size = dsize =
        dcache_line_size < dsize ? dcache_line_size : dsize;
  }
...
void ARM64XEmitter::EncodeCompareBranchInst(u32 op, ARM64Reg Rt,
                                            const void *ptr) {
  bool b64Bit = Is64Bit(Rt);
  s64 distance = (s64)ptr - (s64)m_code;

  _assert_msg_(!(distance & 0x3), "%s: distance must be a multiple of 4: %llx",
               __FUNCTION__, distance);

  distance >>= 2;

  _assert_msg_(distance >= -0x40000 && distance <= 0x3FFFF,
               "%s: Received too large distance: %llx", __FUNCTION__, distance);
...
void ARM64XEmitter::EncodeTestBranchInst(u32 op, ARM64Reg Rt, u8 bits,
                                         const void *ptr) {
  bool b64Bit = Is64Bit(Rt);
  s64 distance = (s64)ptr - (s64)m_code;
  ...
  _assert_msg_(distance >= -0x2000 && distance <= 0x1FFF,
               "%s: Received too large distance: %llx", __FUNCTION__, distance);
...
void ARM64XEmitter::EncodeUnconditionalBranchInst(u32 op, const void *ptr) {
  ...
  _assert_msg_(distance >= -0x2000000LL && distance <= 0x1FFFFFFLL,
               "%s: Received too large distance: %llx", __FUNCTION__, distance);

  Write32((op << 31) | (0x5 << 26) | (distance & 0x3FFFFFF));
}

void ARM64XEmitter::EncodeUnconditionalBranchInst(u32 opc, u32 op2, u32 op3,
                                                  u32 op4, ARM64Reg Rn) {
  Rn = DecodeReg(Rn);
  Write32((0x6B << 25) | (opc << 21) | (op2 << 16) | (op3 << 10) | (Rn << 5) |
          op4);
}

void ARM64XEmitter::EncodeExceptionInst(u32 instenc, u32 imm) {
  _assert_msg_(!(imm & ~0xFFFF),
               "%s: Exception instruction too large immediate: %d",
               __FUNCTION__, imm);

  Write32((0xD4 << 24) | (ExcEnc[instenc][0] << 21) | (imm << 5) |
          (ExcEnc[instenc][1] << 2) | ExcEnc[instenc][2]);
}

void ARM64XEmitter::EncodeSystemInst(u32 op0, u32 op1, u32 CRn, u32 CRm,
                                     u32 op2, ARM64Reg Rt) {
  Write32((0x354 << 22) | (op0 << 19) | (op1 << 16) | (CRn << 12) | (CRm << 8) |
          (op2 << 5) | Rt);
}

void ARM64XEmitter::EncodeArithmeticInst(u32 instenc, bool flags, ARM64Reg Rd,
                                         ARM64Reg Rn, ARM64Reg Rm,
                                         const ArithOption &Option) {
  ...
  Write32((b64Bit << 31) | (flags << 29) | (ArithEnc[instenc] << 21) |
          (Option.GetType() == ArithOption::TYPE_EXTENDEDREG ? (1 << 21) : 0) |
          (Rm << 16) | Option.GetData() | (Rn << 5) | Rd);
}

void ARM64XEmitter::EncodeArithmeticCarryInst(u32 op, bool flags, ARM64Reg Rd,
                                              ARM64Reg Rn, ARM64Reg Rm) {
  bool b64Bit = Is64Bit(Rd);
  ...
  Rm = DecodeReg(Rm);
  Rn = DecodeReg(Rn);
  Write32((b64Bit << 31) | (op << 30) | (flags << 29) | (0xD0 << 21) |
          (Rm << 16) | (Rn << 5) | Rd);
}

void ARM64XEmitter::EncodeCondCompareImmInst(u32 op, ARM64Reg Rn, u32 imm,
                                             u32 nzcv, CCFlags cond) {
  bool b64Bit = Is64Bit(Rn);

  _assert_msg_(!(imm & ~0x1F), "%s: too large immediate: %d", __FUNCTION__, imm)
      _assert_msg_(!(nzcv & ~0xF), "%s: Flags out of range: %d", __FUNCTION__,
                   nzcv)

          Rn = DecodeReg(Rn);
  Write32((b64Bit << 31) | (op << 30) | (1 << 29) | (0xD2 << 21) | (imm << 16) |
          (cond << 12) | (1 << 11) | (Rn << 5) | nzcv);
}

void ARM64XEmitter::EncodeCondCompareRegInst(u32 op, ARM64Reg Rn, ARM64Reg Rm,
                                             u32 nzcv, CCFlags cond) {
  ...
  _assert_msg_(!(nzcv & ~0xF), "%s: Flags out of range: %d", __FUNCTION__, nzcv)
      ...
      Rm = DecodeReg(Rm);
  Rn = DecodeReg(Rn);
  Write32((b64Bit << 31) | (op << 30) | (1 << 29) | (0xD2 << 21) | (Rm << 16) |
          (cond << 12) | (Rn << 5) | nzcv);
}

void ARM64XEmitter::EncodeCondSelectInst(u32 instenc, ARM64Reg Rd, ARM64Reg Rn,
                                         ARM64Reg Rm, CCFlags cond) {
  bool b64Bit = Is64Bit(Rd);
  ...
  Write32((b64Bit << 31) | (CondSelectEnc[instenc][0] << 30) | (0xD4 << 21) |
          (Rm << 16) | (cond << 12) | (CondSelectEnc[instenc][1] << 10) |
          (Rn << 5) | Rd);
}
...
void ARM64XEmitter::EncodeData2SrcInst(u32 instenc, ARM64Reg Rd, ARM64Reg Rn,
                                       ARM64Reg Rm) {
  bool b64Bit = Is64Bit(Rd);
  ...
  Rm = DecodeReg(Rm);
  Rn = DecodeReg(Rn);
  Write32((b64Bit << 31) | (0x0D6 << 21) | (Rm << 16) |
          (Data2SrcEnc[instenc] << 10) | (Rn << 5) | Rd);
}

void ARM64XEmitter::EncodeData3SrcInst(u32 instenc, ARM64Reg Rd, ARM64Reg Rn,
                                       ARM64Reg Rm, ARM64Reg Ra) {
  bool b64Bit = Is64Bit(Rd);
  ...
  Write32((b64Bit << 31) | (0xD8 << 21) | (Data3SrcEnc[instenc][0] << 21) |
          (Rm << 16) | (Data3SrcEnc[instenc][1] << 15) | (Ra << 10) |
          (Rn << 5) | Rd);
}

void ARM64XEmitter::EncodeLogicalInst(u32 instenc, ARM64Reg Rd, ARM64Reg Rn,
                                      ARM64Reg Rm, const ArithOption &Shift) {
  bool b64Bit = Is64Bit(Rd);
  ...
  Write32((b64Bit << 31) | (LogicalEnc[instenc][0] << 29) | (0x5 << 25) |
          (LogicalEnc[instenc][1] << 21) | Shift.GetData() | (Rm << 16) |
          (Rn << 5) | Rd);
}
...
void ARM64XEmitter::EncodeLoadRegisterInst(u32 bitop, ARM64Reg Rt, u32 imm) {
  ...
  Rt = DecodeReg(Rt);
  if (b64Bit &&
      bitop != 0x2) // LDRSW(0x2) uses 64bit reg, doesn't have 64bit bit set
    bitop |= 0x1;
  Write32((bitop << 30) | (bVec << 26) | (0x18 << 24) | (imm << 5) | Rt);
}

void ARM64XEmitter::EncodeLoadStoreExcInst(u32 instenc, ARM64Reg Rs,
                                           ARM64Reg Rt2, ARM64Reg Rn,
                                           ARM64Reg Rt) {
  Rs = DecodeReg(Rs);
  Rt2 = DecodeReg(Rt2);
  Rn = DecodeReg(Rn);
  ...
  Write32((LoadStoreExcEnc[instenc][0] << 30) | (0x8 << 24) |
          (LoadStoreExcEnc[instenc][1] << 23) |
          (LoadStoreExcEnc[instenc][2] << 22) |
          (LoadStoreExcEnc[instenc][3] << 21) | (Rs << 16) |
          (LoadStoreExcEnc[instenc][4] << 15) | (Rt2 << 10) | (Rn << 5) | Rt);
}

void ARM64XEmitter::EncodeLoadStorePairedInst(u32 op, ARM64Reg Rt, ARM64Reg Rt2,
                                              ARM64Reg Rn, u32 imm) {
  bool b64Bit = Is64Bit(Rt);
  ...
  Rt2 = DecodeReg(Rt2);
  Rn = DecodeReg(Rn);
  Write32((opc << 30) | (bVec << 26) | (op << 22) | (imm << 15) | (Rt2 << 10) |
          (Rn << 5) | Rt);
}

void ARM64XEmitter::EncodeLoadStoreIndexedInst(u32 op, u32 op2, ARM64Reg Rt,
                                               ARM64Reg Rn, s32 imm) {
  bool b64Bit = Is64Bit(Rt);
  bool bVec = IsVector(Rt);
  ...
  _assert_msg_(!(imm < -256 || imm > 255), "%s: offset too large %d",
               __FUNCTION__, imm);

  Rt = DecodeReg(Rt);
  Rn = DecodeReg(Rn);
  Write32((b64Bit << 30) | (op << 22) | (bVec << 26) | (offset << 12) |
          (op2 << 10) | (Rn << 5) | Rt);
}

void ARM64XEmitter::EncodeLoadStoreIndexedInst(u32 op, ARM64Reg Rt, ARM64Reg Rn,
                                               s32 imm, u8 size) {
  bool b64Bit = Is64Bit(Rt);
  bool bVec = IsVector(Rt);
  ...
  if (shift) {
    _assert_msg_(((imm >> shift) << shift) == imm,
                 "%s(INDEX_UNSIGNED): offset must be aligned %d", __FUNCTION__,
                 imm);
    imm >>= shift;
  }

  _assert_msg_(imm >= 0, "%s(INDEX_UNSIGNED): offset must be positive %d",
               __FUNCTION__, imm);
  _assert_msg_(!(imm & ~0xFFF), "%s(INDEX_UNSIGNED): offset too large %d",
               __FUNCTION__, imm);

  Rt = DecodeReg(Rt);
  Rn = DecodeReg(Rn);
  Write32((b64Bit << 30) | (op << 22) | (bVec << 26) | (imm << 10) | (Rn << 5) |
          Rt);
}

void ARM64XEmitter::EncodeMOVWideInst(u32 op, ARM64Reg Rd, u32 imm,
                                      ShiftAmount pos) {
  bool b64Bit = Is64Bit(Rd);

  _assert_msg_(!(imm & ~0xFFFF), "%s: immediate out of range: %d", __FUNCTION__,
               imm);

  Rd = DecodeReg(Rd);
  Write32((b64Bit << 31) | (op << 29) | (0x25 << 23) | (pos << 21) |
          (imm << 5) | Rd);
}

void ARM64XEmitter::EncodeBitfieldMOVInst(u32 op, ARM64Reg Rd, ARM64Reg Rn,
                                          u32 immr, u32 imms) {
  bool b64Bit = Is64Bit(Rd);
...
void ARM64XEmitter::EncodeLoadStoreRegisterOffset(u32 size, u32 opc,
                                                  ARM64Reg Rt, ARM64Reg Rn,
                                                  const ArithOption &Rm) {
  Rt = DecodeReg(Rt);
  Rn = DecodeReg(Rn);
...
void ARM64XEmitter::EncodeAddSubImmInst(u32 op, bool flags, u32 shift, u32 imm,
                                        ARM64Reg Rn, ARM64Reg Rd) {
  bool b64Bit = Is64Bit(Rd);

  _assert_msg_(!(imm & ~0xFFF), "%s: immediate too large: %x", __FUNCTION__,
               imm);

  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);
  Write32((b64Bit << 31) | (op << 30) | (flags << 29) | (0x11 << 24) |
          (shift << 22) | (imm << 10) | (Rn << 5) | Rd);
}

void ARM64XEmitter::EncodeLogicalImmInst(u32 op, ARM64Reg Rd, ARM64Reg Rn,
                                         u32 immr, u32 imms, int n) {
...
void ARM64XEmitter::EncodeLoadStorePair(u32 op, u32 load, IndexType type,
                                        ARM64Reg Rt, ARM64Reg Rt2, ARM64Reg Rn,
                                        s32 imm) {
  bool b64Bit = Is64Bit(Rt);
  u32 type_encode = 0;
  ...
  _assert_msg_(imm >= -64 && imm <= 63, "%s recieved too large imm: %d",
               __FUNCTION__, imm);
...
void ARM64XEmitter::EncodeLoadStoreUnscaled(u32 size, u32 op, ARM64Reg Rt,
                                            ARM64Reg Rn, s32 imm) {
  _assert_msg_(!(imm < -256 || imm > 255), "%s received too large offset: %d",
               __FUNCTION__, imm);
  Rt = DecodeReg(Rt);
  Rn = DecodeReg(Rn);

  Write32((size << 30) | (7 << 27) | (op << 22) | ((imm & 0x1FF) << 12) |
          (Rn << 5) | Rt);
}
...
static inline u32 MaskImm19(s64 distance) { return distance & 0x7FFFF; }

static inline u32 MaskImm14(s64 distance) { return distance & 0x3FFF; }

static inline u32 MaskImm26(s64 distance) { return distance & 0x3FFFFFF; }
...
void ARM64XEmitter::SetJumpTarget(FixupBranch const &branch) {
  ...
  case 0: // CBZ
  {
    _assert_msg_(IsInRangeImm19(distance),
                 "%s(%d): Received too large distance: %llx", __FUNCTION__,
                 branch.type, distance);
    bool b64Bit = Is64Bit(branch.reg);
    ARM64Reg reg = DecodeReg(branch.reg);
    inst = (b64Bit << 31) | (0x1A << 25) | (Not << 24) |
           (MaskImm19(distance) << 5) | reg;
  } break;
  ...
  case 3: // TBZ
  {
    _assert_msg_(IsInRangeImm14(distance),
                 "%s(%d): Received too large distance: %llx", __FUNCTION__,
                 branch.type, distance);
    ARM64Reg reg = DecodeReg(branch.reg);
  ...
  case 5: // B (unconditional)
    _assert_msg_(IsInRangeImm26(distance),
                 "%s(%d): Received too large distance: %llx", __FUNCTION__,
                 branch.type, distance);
...
void ARM64XEmitter::B(CCFlags cond, const void *ptr) {
  ...
  _assert_msg_(IsInRangeImm19(distance),
               "%s: Received too large distance: %p->%p %lld %llx",
               __FUNCTION__, m_code, ptr, distance, distance);
...
void ARM64XEmitter::QuickCallFunction(ARM64Reg scratchreg, const void *func) {
  ...
  if (!IsInRangeImm26(distance)) {
    // WARN_LOG(Log::JIT, "Distance too far in function call (%p to %p)! Using
    // scratch.", m_code, func);
...
void ARM64XEmitter::ERET() { EncodeUnconditionalBranchInst(4, 0x1F, 0, 0, SP); }
void ARM64XEmitter::DRPS() { EncodeUnconditionalBranchInst(5, 0x1F, 0, 0, SP); }

// Exception generation
void ARM64XEmitter::SVC(u32 imm) { EncodeExceptionInst(0, imm); }

void ARM64XEmitter::HVC(u32 imm) { EncodeExceptionInst(1, imm); }

void ARM64XEmitter::SMC(u32 imm) { EncodeExceptionInst(2, imm); }

void ARM64XEmitter::BRK(u32 imm) { EncodeExceptionInst(3, imm); }

void ARM64XEmitter::HLT(u32 imm) { EncodeExceptionInst(4, imm); }

void ARM64XEmitter::DCPS1(u32 imm) { EncodeExceptionInst(5, imm); }

void ARM64XEmitter::DCPS2(u32 imm) { EncodeExceptionInst(6, imm); }

void ARM64XEmitter::DCPS3(u32 imm) { EncodeExceptionInst(7, imm); }
...
static void GetSystemReg(PStateField field, int &o0, int &op1, int &CRn,
                         int &CRm, int &op2) {
  switch (field) {
  case FIELD_NZCV:
    o0 = 3;
...
void ARM64XEmitter::CLREX() { EncodeSystemInst(0, 3, 3, 0, 2, WSP); }
...
// Logical (shifted register)
void ARM64XEmitter::AND(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                        const ArithOption &Shift) {
  EncodeLogicalInst(0, Rd, Rn, Rm, Shift);
}
void ARM64XEmitter::BIC(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                        const ArithOption &Shift) {
  EncodeLogicalInst(1, Rd, Rn, Rm, Shift);
}
void ARM64XEmitter::ORR(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                        const ArithOption &Shift) {
  EncodeLogicalInst(2, Rd, Rn, Rm, Shift);
}
void ARM64XEmitter::ORN(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                        const ArithOption &Shift) {
  EncodeLogicalInst(3, Rd, Rn, Rm, Shift);
}
void ARM64XEmitter::EOR(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                        const ArithOption &Shift) {
  EncodeLogicalInst(4, Rd, Rn, Rm, Shift);
}
void ARM64XEmitter::EON(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                        const ArithOption &Shift) {
  EncodeLogicalInst(5, Rd, Rn, Rm, Shift);
}
void ARM64XEmitter::ANDS(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                         const ArithOption &Shift) {
  EncodeLogicalInst(6, Rd, Rn, Rm, Shift);
}
void ARM64XEmitter::BICS(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                         const ArithOption &Shift) {
...
void ARM64XEmitter::MOVfromSP(ARM64Reg Rd) { ADD(Rd, ARM64Reg::SP, 0, false); }

void ARM64XEmitter::MOVtoSP(ARM64Reg Rn) { ADD(ARM64Reg::SP, Rn, 0, false); }
...
void ARM64XEmitter::EXTR(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm, u32 shift) {
  ...
  Rn = DecodeReg(Rn);
  Rm = DecodeReg(Rm);
  Write32((sf << 31) | (0x27 << 23) | (N << 22) | (Rm << 16) | (shift << 10) |
          (Rm << 5) | Rd);
}
void ARM64XEmitter::SXTB(ARM64Reg Rd, ARM64Reg Rn) { SBFM(Rd, Rn, 0, 7); }
void ARM64XEmitter::SXTH(ARM64Reg Rd, ARM64Reg Rn) { SBFM(Rd, Rn, 0, 15); }
void ARM64XEmitter::SXTW(ARM64Reg Rd, ARM64Reg Rn) {
  _assert_msg_(Is64Bit(Rd), "%s requires 64bit register as destination",
               __FUNCTION__);
  SBFM(Rd, Rn, 0, 31);
}
void ARM64XEmitter::UXTB(ARM64Reg Rd, ARM64Reg Rn) { UBFM(Rd, Rn, 0, 7); }
void ARM64XEmitter::UXTH(ARM64Reg Rd, ARM64Reg Rn) { UBFM(Rd, Rn, 0, 15); }
...
// Load/Store pair
void ARM64XEmitter::LDP(IndexType type, ARM64Reg Rt, ARM64Reg Rt2, ARM64Reg Rn,
                        s32 imm) {
  EncodeLoadStorePair(0, 1, type, Rt, Rt2, Rn, imm);
}
void ARM64XEmitter::LDPSW(IndexType type, ARM64Reg Rt, ARM64Reg Rt2,
                          ARM64Reg Rn, s32 imm) {
  EncodeLoadStorePair(1, 1, type, Rt, Rt2, Rn, imm);
}
void ARM64XEmitter::STP(IndexType type, ARM64Reg Rt, ARM64Reg Rt2, ARM64Reg Rn,
                        s32 imm) {
...
void ARM64XEmitter::LDURSW(ARM64Reg Rt, ARM64Reg Rn, s32 imm) {
  _assert_msg_(!Is64Bit(Rt), "%s must have a 64bit destination register!",
               __FUNCTION__);
  EncodeLoadStoreUnscaled(2, 2, Rt, Rn, imm);
}

// Address of label/page PC-relative
void ARM64XEmitter::ADR(ARM64Reg Rd, s32 imm) { EncodeAddressInst(0, Rd, imm); }
...
void ARM64XEmitter::PUSH(ARM64Reg Rd) { STR(INDEX_PRE, Rd, SP, -16); }

void ARM64XEmitter::POP(ARM64Reg Rd) { LDR(INDEX_POST, Rd, SP, 16); }
...
// Float Emitter
void ARM64FloatEmitter::EmitLoadStoreImmediate(u8 size, u32 opc, IndexType type,
                                               ARM64Reg Rt, ARM64Reg Rn,
                                               s32 imm) {
  Rt = DecodeReg(Rt);
  Rn = DecodeReg(Rn);
  ...
  if (type == INDEX_UNSIGNED) {
    _assert_msg_(!(imm & ((size - 1) >> 3)),
                 "%s(INDEX_UNSIGNED) immediate offset must be aligned to size! "
                 "(%d) (%p)",
                 __FUNCTION__, imm, m_emit->GetCodePointer());
    _assert_msg_(imm >= 0,
                 "%s(INDEX_UNSIGNED) immediate offset must be positive!",
                 __FUNCTION__);
    if (size == 16)
      imm >>= 1;
    else if (size == 32)
    ...
    _assert_msg_(!(imm < -256 || imm > 255),
                 "%s immediate offset must be within range of -256 to 255!",
                 __FUNCTION__);
  ...
  Write32((encoded_size << 30) | (0xF << 26) |
          (type == INDEX_UNSIGNED ? (1 << 24) : 0) |
          (size == 128 ? (1 << 23) : 0) | (opc << 22) | (encoded_imm << 10) |
          (Rn << 5) | Rt);
}

void ARM64FloatEmitter::EmitScalar2Source(bool M, bool S, u32 type, u32 opcode,
                                          ARM64Reg Rd, ARM64Reg Rn,
                                          ARM64Reg Rm) {
  _assert_msg_(!IsQuad(Rd), "%s only supports double and single registers!",
               __FUNCTION__);
  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);
  Rm = DecodeReg(Rm);
...
void ARM64FloatEmitter::EmitThreeSame(bool U, u32 size, u32 opcode, ARM64Reg Rd,
                                      ARM64Reg Rn, ARM64Reg Rm) {
  ...
  Write32((quad << 30) | (U << 29) | (0x71 << 21) | (size << 22) | (Rm << 16) |
          (opcode << 11) | (1 << 10) | (Rn << 5) | Rd);
}

void ARM64FloatEmitter::EmitCopy(bool Q, u32 op, u32 imm5, u32 imm4,
                                 ARM64Reg Rd, ARM64Reg Rn) {
  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);
...
void ARM64FloatEmitter::EmitScalarPairwise(bool U, u32 size, u32 opcode,
                                           ARM64Reg Rd, ARM64Reg Rn) {
  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);

  Write32((1 << 30) | (U << 29) | (0b111100011 << 20) | (size << 22) |
          (opcode << 12) | (1 << 11) | (Rn << 5) | Rd);
}

void ARM64FloatEmitter::Emit2RegMisc(bool Q, bool U, u32 size, u32 opcode,
                                     ARM64Reg Rd, ARM64Reg Rn) {
  _assert_msg_(!IsSingle(Rd), "%s doesn't support singles!", __FUNCTION__);
  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);

  Write32((Q << 30) | (U << 29) | (0x71 << 21) | (size << 22) | (opcode << 12) |
          (1 << 11) | (Rn << 5) | Rd);
}

void ARM64FloatEmitter::EmitLoadStoreSingleStructure(bool L, bool R, u32 opcode,
                                                     bool S, u32 size,
                                                     ARM64Reg Rt, ARM64Reg Rn) {
...
void ARM64FloatEmitter::Emit1Source(bool M, bool S, u32 type, u32 opcode,
                                    ARM64Reg Rd, ARM64Reg Rn) {
...
void ARM64FloatEmitter::EmitConversion(bool sf, bool S, u32 type, u32 rmode,
                                       u32 opcode, ARM64Reg Rd, ARM64Reg Rn) {
...
void ARM64FloatEmitter::EmitConvertScalarToInt(ARM64Reg Rd, ARM64Reg Rn,
                                               RoundingMode round, bool sign) {
  ...
  if (IsGPR(Rd)) {
    ...
    Write32((0x5E << 24) | (sign << 29) | (sz << 22) | (1 << 21) |
            (opcode << 12) | (2 << 10) | (Rn << 5) | Rd);
  }
}
...
void ARM64FloatEmitter::FCVTZS(ARM64Reg Rd, ARM64Reg Rn, int scale) {
  if (IsScalar(Rd)) {
    ...
    Write32((1 << 30) | (0 << 29) | (0x1F << 24) | (imm << 16) | (0x1F << 11) |
            (1 << 10) | (Rn << 5) | Rd);
  } else {
    bool sf = Is64Bit(Rd);
    u32 type = 0;
    ...
    Write32((sf << 31) | (0 << 29) | (0x1E << 24) | (type << 22) |
...
void ARM64FloatEmitter::FCVTZU(ARM64Reg Rd, ARM64Reg Rn, int scale) {
  if (IsScalar(Rd)) {
    ...
    Write32((1 << 30) | (1 << 29) | (0x1F << 24) | (imm << 16) | (0x1F << 11) |
            (1 << 10) | (Rn << 5) | Rd);
  } else {
    bool sf = Is64Bit(Rd);
    u32 type = 0;
...
void ARM64FloatEmitter::EmitConversion2(bool sf, bool S, bool direction,
                                        u32 type, u32 rmode, u32 opcode,
                                        int scale, ARM64Reg Rd, ARM64Reg Rn) {
  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);

  Write32((sf << 31) | (S << 29) | (0xF0 << 21) | (direction << 21) |
          (type << 22) | (rmode << 19) | (opcode << 16) | (scale << 10) |
          (Rn << 5) | Rd);
}

void ARM64FloatEmitter::EmitCompare(bool M, bool S, u32 op, u32 opcode2,
                                    ARM64Reg Rn, ARM64Reg Rm) {
  ...
  Write32((M << 31) | (S << 29) | (0xF1 << 21) | (is_double << 22) |
          (Rm << 16) | (op << 14) | (1 << 13) | (Rn << 5) | opcode2);
}

void ARM64FloatEmitter::EmitCondSelect(bool M, bool S, CCFlags cond,
  ...
  Write32((M << 31) | (S << 29) | (0xF1 << 21) | (is_double << 22) |
          (Rm << 16) | (cond << 12) | (3 << 10) | (Rn << 5) | Rd);
}

void ARM64FloatEmitter::EmitCondCompare(bool M, bool S, CCFlags cond, int op,
                                        u8 nzcv, ARM64Reg Rn, ARM64Reg Rm) {
  _assert_msg_(!IsQuad(Rn), "%s doesn't support vector!", __FUNCTION__);
  bool is_double = IsDouble(Rn);
  ...
  Write32((M << 31) | (S << 29) | (0xF1 << 21) | (is_double << 22) |
          (Rm << 16) | (cond << 12) | (1 << 10) | (Rn << 5) | (op << 4) | nzcv);
}

void ARM64FloatEmitter::EmitPermute(u32 size, u32 op, ARM64Reg Rd, ARM64Reg Rn,
                                    ARM64Reg Rm) {
  _assert_msg_(!IsSingle(Rd), "%s doesn't support singles!", __FUNCTION__);
  ...
  Write32((quad << 30) | (7 << 25) | (encoded_size << 22) | (Rm << 16) |
          (op << 12) | (1 << 11) | (Rn << 5) | Rd);
}

void ARM64FloatEmitter::EmitScalarImm(bool M, bool S, u32 type, u32 imm5,
                                      ARM64Reg Rd, u32 imm8) {
  _assert_msg_(!IsQuad(Rd), "%s doesn't support vector!", __FUNCTION__);
  ...
  Write32((M << 31) | (S << 29) | (0xF1 << 21) | (is_double << 22) |
          (type << 22) | (imm8 << 13) | (1 << 12) | (imm5 << 5) | Rd);
}

void ARM64FloatEmitter::EmitShiftImm(bool Q, bool U, u32 immh, u32 immb,
                                     u32 opcode, ARM64Reg Rd, ARM64Reg Rn) {
...
void ARM64FloatEmitter::EmitScalarShiftImm(bool U, u32 immh, u32 immb,
                                           u32 opcode, ARM64Reg Rd,
                                           ARM64Reg Rn) {
  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);

  Write32((2 << 30) | (U << 29) | (0x3E << 23) | (immh << 19) | (immb << 16) |
          (opcode << 11) | (1 << 10) | (Rn << 5) | Rd);
}

void ARM64FloatEmitter::EmitLoadStoreMultipleStructure(u32 size, bool L,
                                                       u32 opcode, ARM64Reg Rt,
                                                       ARM64Reg Rn) {
  bool quad = IsQuad(Rt);
  u32 encoded_size = 0;
...
void ARM64FloatEmitter::EmitLoadStoreMultipleStructurePost(
    u32 size, bool L, u32 opcode, ARM64Reg Rt, ARM64Reg Rn, ARM64Reg Rm) {
  bool quad = IsQuad(Rt);
  u32 encoded_size = 0;
  ...
  Write32((quad << 30) | (0x19 << 23) | (L << 22) | (Rm << 16) |
          (opcode << 12) | (encoded_size << 10) | (Rn << 5) | Rt);
}

void ARM64FloatEmitter::EmitScalar1Source(bool M, bool S, u32 type, u32 opcode,
                                          ARM64Reg Rd, ARM64Reg Rn) {
  _assert_msg_(!IsQuad(Rd), "%s doesn't support vector!", __FUNCTION__);
...
void ARM64FloatEmitter::EmitVectorxElement(bool U, u32 size, bool L, u32 opcode,
                                           bool H, ARM64Reg Rd, ARM64Reg Rn,
                                           ARM64Reg Rm) {
  bool quad = IsQuad(Rd);
...
void ARM64FloatEmitter::EmitLoadStoreUnscaled(u32 size, u32 op, ARM64Reg Rt,
                                              ARM64Reg Rn, s32 imm) {
  _assert_msg_(!(imm < -256 || imm > 255), "%s received too large offset: %d",
               __FUNCTION__, imm);
  Rt = DecodeReg(Rt);
  ...
  Write32((size << 30) | (0xF << 26) | (op << 22) | ((imm & 0x1FF) << 12) |
          (Rn << 5) | Rt);
}

void ARM64FloatEmitter::EncodeLoadStorePair(u32 size, bool load, IndexType type,
                                            ARM64Reg Rt, ARM64Reg Rt2,
                                            ARM64Reg Rn, s32 imm) {
  u32 type_encode = 0;
  u32 opc = 0;
  ...
  if (size == 128) {
    _assert_msg_(!(imm & 0xF), "%s received invalid offset 0x%x!", __FUNCTION__,
                 imm);
    opc = 2;
    imm >>= 4;
  } else if (size == 64) {
    _assert_msg_(!(imm & 0x7), "%s received invalid offset 0x%x!", __FUNCTION__,
                 imm);
    opc = 1;
    imm >>= 3;
  } else if (size == 32) {
    _assert_msg_(!(imm & 0x3), "%s received invalid offset 0x%x!", __FUNCTION__,
                 imm);
    opc = 0;
    imm >>= 2;
  }
...
void ARM64FloatEmitter::EncodeLoadStoreRegisterOffset(u32 size, bool load,
                                                      ARM64Reg Rt, ARM64Reg Rn,
                                                      const ArithOption &Rm) {
  _assert_msg_(Rm.GetType() == ArithOption::TYPE_EXTENDEDREG,
  ...
  Write32((encoded_size << 30) | (encoded_op << 22) | (0x1E1 << 21) |
          (decoded_Rm << 16) | Rm.GetData() | (1 << 11) | (Rn << 5) | Rt);
}

void ARM64FloatEmitter::LDR(u8 size, IndexType type, ARM64Reg Rt, ARM64Reg Rn,
                            s32 imm) {
  EmitLoadStoreImmediate(size, 1, type, Rt, Rn, imm);
}
void ARM64FloatEmitter::STR(u8 size, IndexType type, ARM64Reg Rt, ARM64Reg Rn,
                            s32 imm) {
...
void ARM64FloatEmitter::LD1(u8 size, u8 count, ARM64Reg Rt, ARM64Reg Rn) {
  _assert_msg_(!(count == 0 || count > 4),
               "%s must have a count of 1 to 4 registers!", __FUNCTION__);
  u32 opcode = 0;
  if (count == 1)
    opcode = 7;
  else if (count == 2)
...
void ARM64FloatEmitter::LD1(u8 size, u8 count, IndexType type, ARM64Reg Rt,
                            ARM64Reg Rn, ARM64Reg Rm) {
  _assert_msg_(!(count == 0 || count > 4),
               "%s must have a count of 1 to 4 registers!", __FUNCTION__);
  _assert_msg_(type == INDEX_POST, "%s only supports post indexing!",
               __FUNCTION__);
...
void ARM64FloatEmitter::ST1(u8 size, u8 count, IndexType type, ARM64Reg Rt,
                            ARM64Reg Rn, ARM64Reg Rm) {
  _assert_msg_(!(count == 0 || count > 4),
               "%s must have a count of 1 to 4 registers!", __FUNCTION__);
  _assert_msg_(type == INDEX_POST, "%s only supports post indexing!",
...
void ARM64FloatEmitter::FMOV(ARM64Reg Rd, ARM64Reg Rn, bool top) {
  if (IsScalar(Rd) && IsScalar(Rn)) {
    ...
    Rd = DecodeReg(Rd);
    Rn = DecodeReg(Rn);
    Write32((sf << 31) | (0x1e2 << 20) | (rmode << 19) | (opcode << 16) |
            (Rn << 5) | Rd);
  }
}

// Loadstore paired
void ARM64FloatEmitter::LDP(u8 size, IndexType type, ARM64Reg Rt, ARM64Reg Rt2,
                            ARM64Reg Rn, s32 imm) {
  EncodeLoadStorePair(size, true, type, Rt, Rt2, Rn, imm);
}
void ARM64FloatEmitter::STP(u8 size, IndexType type, ARM64Reg Rt, ARM64Reg Rt2,
                            ARM64Reg Rn, s32 imm) {
  EncodeLoadStorePair(size, false, type, Rt, Rt2, Rn, imm);
}

// Loadstore register offset
void ARM64FloatEmitter::STR(u8 size, ARM64Reg Rt, ARM64Reg Rn,
                            const ArithOption &Rm) {
  EncodeLoadStoreRegisterOffset(size, false, Rt, Rn, Rm);
}
void ARM64FloatEmitter::LDR(u8 size, ARM64Reg Rt, ARM64Reg Rn,
                            const ArithOption &Rm) {
...
void ARM64FloatEmitter::FMADD(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                              ARM64Reg Ra) {
  EmitScalar3Source(IsDouble(Rd), Rd, Rn, Rm, Ra, 0);
}
void ARM64FloatEmitter::FMSUB(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                              ARM64Reg Ra) {
  EmitScalar3Source(IsDouble(Rd), Rd, Rn, Rm, Ra, 1);
}
void ARM64FloatEmitter::FNMADD(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                               ARM64Reg Ra) {
  EmitScalar3Source(IsDouble(Rd), Rd, Rn, Rm, Ra, 2);
}
void ARM64FloatEmitter::FNMSUB(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                               ARM64Reg Ra) {
  EmitScalar3Source(IsDouble(Rd), Rd, Rn, Rm, Ra, 3);
}

void ARM64FloatEmitter::EmitScalar3Source(bool isDouble, ARM64Reg Rd,
                                          ARM64Reg Rn, ARM64Reg Rm, ARM64Reg Ra,
                                          int opcode) {
  ...
  int o1 = opcode >> 1;
  int o0 = opcode & 1;
  m_emit->Write32((0x1F << 24) | (type << 22) | (o1 << 21) | (Rm << 16) |
                  (o0 << 15) | (Ra << 10) | (Rn << 5) | Rd);
}
...
void ARM64FloatEmitter::CMEQ(u8 size, ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm) {
  _assert_msg_(!IsQuad(Rd) || size != 64, "%s cannot be used for scalar double",
               __FUNCTION__);
...
void ARM64FloatEmitter::INS(u8 size, ARM64Reg Rd, u8 index1, ARM64Reg Rn,
                            u8 index2) {
  u32 imm5 = 0, imm4 = 0;
...
void ARM64FloatEmitter::UMOV(u8 size, ARM64Reg Rd, ARM64Reg Rn, u8 index) {
  ...
  _assert_msg_(!(b64Bit && size != 64),
               "%s must have a size of 64 when destination is 64bit!",
               __FUNCTION__);
  u32 imm5 = 0;
...
void ARM64FloatEmitter::SMOV(u8 size, ARM64Reg Rd, ARM64Reg Rn, u8 index) {
  bool b64Bit = Is64Bit(Rd);
  _assert_msg_(Rd < SP, "%s destination must be a GPR!", __FUNCTION__);
  _assert_msg_(size != 64, "%s doesn't support 64bit destination. Use UMOV!",
               __FUNCTION__);
  u32 imm5 = 0;
...
void ARM64FloatEmitter::EncodeModImm(bool Q, u8 op, u8 cmode, u8 o2,
                                     ARM64Reg Rd, u8 abcdefgh) {
  Rd = DecodeReg(Rd);
  u8 abc = abcdefgh >> 5;
  u8 defgh = abcdefgh & 0x1F;
  Write32((Q << 30) | (op << 29) | (0xF << 24) | (abc << 16) | (cmode << 12) |
          (o2 << 11) | (1 << 10) | (defgh << 5) | Rd);
}

void ARM64FloatEmitter::FMOV(u8 size, ARM64Reg Rd, u8 imm8) {
  _assert_msg_(!IsSingle(Rd), "%s doesn't support singles", __FUNCTION__);
  _assert_msg_(size == 32 || size == 64, "%s: unsupported size", __FUNCTION__);
  _assert_msg_(IsQuad(Rd) || size == 32,
               "Use non-SIMD FMOV to load one double imm8");
  EncodeModImm(IsQuad(Rd), size >> 6, 0b1111, 0, Rd, imm8);
}

void ARM64FloatEmitter::MOVI(u8 size, ARM64Reg Rd, u8 imm8, u8 shift,
                             bool MSL) {
  _assert_msg_(!IsSingle(Rd), "%s doesn't support singles", __FUNCTION__);
  _assert_msg_(size == 8 || size == 16 || size == 32 || size == 64,
               "%s: unsupported size %d", __FUNCTION__, size);
  _assert_msg_((shift & 7) == 0 && shift < size, "%s: unsupported shift %d",
               __FUNCTION__, shift);
  _assert_msg_(!MSL || (size == 32 && shift > 0 && shift <= 16),
               "MOVI MSL shift requires size 32, shift must be 8 or 16");
...
void ARM64FloatEmitter::MVNI(u8 size, ARM64Reg Rd, u8 imm8, u8 shift,
                             bool MSL) {
  _assert_msg_(!IsSingle(Rd), "%s doesn't support singles", __FUNCTION__);
  _assert_msg_(size == 16 || size == 32, "%s: unsupported size %d",
               __FUNCTION__, size);
  _assert_msg_((shift & 7) == 0 && shift < size, "%s: unsupported shift %d",
               __FUNCTION__, shift);
  _assert_msg_(!MSL || (size == 32 && shift > 0 && shift <= 16),
               "MVNI MSL shift requires size 32, shift must be 8 or 16");
...
// One source
void ARM64FloatEmitter::FCVT(u8 size_to, u8 size_from, ARM64Reg Rd,
                             ARM64Reg Rn) {
  u32 dst_encoding = 0;
  u32 src_encoding = 0;
...
void ARM64FloatEmitter::SCVTF(ARM64Reg Rd, ARM64Reg Rn) {
  if (IsScalar(Rn)) {
    ...
    Rn = DecodeReg(Rn);
    int sz = IsDouble(Rn);
    Write32((0x5e << 24) | (sign << 29) | (sz << 22) | (0x876 << 10) |
            (Rn << 5) | Rd);
  } else {
    bool sf = Is64Bit(Rn);
    u32 type = 0;
...
void ARM64FloatEmitter::SCVTF(ARM64Reg Rd, ARM64Reg Rn, int scale) {
  if (IsScalar(Rn)) {
    ...
    Write32((1 << 30) | (0 << 29) | (0x1F << 24) | (imm << 16) | (0x1C << 11) |
            (1 << 10) | (Rn << 5) | Rd);
  } else {
    bool sf = Is64Bit(Rn);
    u32 type = 0;
...
void ARM64FloatEmitter::UCVTF(ARM64Reg Rd, ARM64Reg Rn, int scale) {
  if (IsScalar(Rn)) {
    ...
    Write32((1 << 30) | (1 << 29) | (0x1F << 24) | (imm << 16) | (0x1C << 11) |
            (1 << 10) | (Rn << 5) | Rd);
  } else {
    bool sf = Is64Bit(Rn);
    u32 type = 0;
...
void ARM64FloatEmitter::FCSEL(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                              CCFlags cond) {
...
void ARM64FloatEmitter::FCCMPE(ARM64Reg Rn, ARM64Reg Rm, u8 nzcv,
                               CCFlags cond) {
...
void ARM64FloatEmitter::EXT(ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm, int index) {
  ...
  bool quad = IsQuad(Rd);
  _assert_msg_(index >= 0 && index < 16 && (quad || index < 8),
               "%s start index out of bounds", __FUNCTION__);
  _assert_msg_(IsQuad(Rd) == IsQuad(Rn) && IsQuad(Rd) == IsQuad(Rm),
               "%s operands not same size", __FUNCTION__);

  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);
  Rm = DecodeReg(Rm);

  Write32((quad << 30) | (0x17 << 25) | (Rm << 16) | (index << 11) | (Rn << 5) |
          Rd);
}

// Shift by immediate
void ARM64FloatEmitter::SSHLL(u8 src_size, ARM64Reg Rd, ARM64Reg Rn,
                              u32 shift) {
  SSHLL(src_size, Rd, Rn, shift, false);
}
void ARM64FloatEmitter::SSHLL2(u8 src_size, ARM64Reg Rd, ARM64Reg Rn,
                               u32 shift) {
  SSHLL(src_size, Rd, Rn, shift, true);
}
void ARM64FloatEmitter::SHRN(u8 dest_size, ARM64Reg Rd, ARM64Reg Rn,
                             u32 shift) {
  SHRN(dest_size, Rd, Rn, shift, false);
}
void ARM64FloatEmitter::SHRN2(u8 dest_size, ARM64Reg Rd, ARM64Reg Rn,
                              u32 shift) {
  SHRN(dest_size, Rd, Rn, shift, true);
}
void ARM64FloatEmitter::USHLL(u8 src_size, ARM64Reg Rd, ARM64Reg Rn,
                              u32 shift) {
  USHLL(src_size, Rd, Rn, shift, false);
}
void ARM64FloatEmitter::USHLL2(u8 src_size, ARM64Reg Rd, ARM64Reg Rn,
                               u32 shift) {
...
void ARM64FloatEmitter::SSHLL(u8 src_size, ARM64Reg Rd, ARM64Reg Rn, u32 shift,
                              bool upper) {
  _assert_msg_(shift < src_size,
               "%s shift amount must less than the element size!",
               __FUNCTION__);
...
void ARM64FloatEmitter::SHRN(u8 dest_size, ARM64Reg Rd, ARM64Reg Rn, u32 shift,
                             bool upper) {
  _assert_msg_(shift > 0, "%s shift amount must be greater than zero!",
               __FUNCTION__);
  _assert_msg_(shift <= dest_size,
               "%s shift amount must less than or equal to the element size!",
               __FUNCTION__);
...
void ARM64FloatEmitter::USHR(u8 dest_size, ARM64Reg Rd, ARM64Reg Rn,
                             u32 shift) {
  _assert_msg_(shift < dest_size,
               "%s shift amount must less than the element size!",
               __FUNCTION__);
...
void ARM64FloatEmitter::SSHR(u8 dest_size, ARM64Reg Rd, ARM64Reg Rn,
                             u32 shift) {
  _assert_msg_(shift < dest_size,
               "%s shift amount must less than the element size!",
               __FUNCTION__);
...
void ARM64FloatEmitter::FMUL(u8 size, ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                             u8 index) {
  _assert_msg_(size == 32 || size == 64,
               "%s only supports 32bit or 64bit size!", __FUNCTION__);
...
void ARM64FloatEmitter::ABI_PushRegisters(uint32_t registers,
                                          uint32_t fp_registers) {
  _assert_msg_((registers & 0x60000000) == 0,
               "ABI_PushRegisters: Do not include FP and LR, those are handled "
               "non-conditionally");
...
void ARM64XEmitter::ANDI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
                           ARM64Reg scratch) {
  // It's probably okay to AND by extra bits.
  if (!Is64Bit(Rn))
    imm &= 0xFFFFFFFF;
  if (!TryANDI2R(Rd, Rn, imm)) {
    _assert_msg_(scratch != INVALID_REG,
...
void ARM64XEmitter::ORRI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
                           ARM64Reg scratch) {
  _assert_msg_(Is64Bit(Rn) || (imm & 0xFFFFFFFF00000000UL) == 0,
               "ORRI2R - more bits in imm than Rn");
...
void ARM64XEmitter::EORI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
                           ARM64Reg scratch) {
  _assert_msg_(Is64Bit(Rn) || (imm & 0xFFFFFFFF00000000UL) == 0,
               "EORI2R - more bits in imm than Rn");
...
void ARM64XEmitter::ANDSI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
                            ARM64Reg scratch) {
  if (!Is64Bit(Rn))
    imm &= 0xFFFFFFFF;
...
void ARM64XEmitter::ADDI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
                           ARM64Reg scratch) {
...
void ARM64XEmitter::SUBI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
                           ARM64Reg scratch) {
  if (!TrySUBI2R(Rd, Rn, imm)) {
    _assert_msg_(scratch != INVALID_REG,
                 "SUBI2R - failed to construct arithmetic immediate value from "
                 "%08x, need scratch",
                 (u32)imm);
    MOVI2R(scratch, imm);
    SUB(Rd, Rn, scratch);
...
void ARM64XEmitter::CMPI2R(ARM64Reg Rn, u64 imm, ARM64Reg scratch) {
  if (!TryCMPI2R(Rn, imm)) {
    _assert_msg_(scratch != INVALID_REG,
                 "CMPI2R - failed to construct arithmetic immediate value from "
                 "%08x, need scratch",
                 (u32)imm);
    MOVI2R(scratch, imm);
    CMP(Rn, scratch);
...
bool ARM64XEmitter::TryORRI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm) {
  _assert_msg_(Is64Bit(Rn) || (imm & 0xFFFFFFFF00000000UL) == 0,
               "TryORRI2R - more bits in imm than Rn");
  u32 n, imm_r, imm_s;
  if (IsImmLogical(imm, Is64Bit(Rn) ? 64 : 32, &n, &imm_s, &imm_r)) {
...
bool ARM64XEmitter::TryEORI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm) {
  _assert_msg_(Is64Bit(Rn) || (imm & 0xFFFFFFFF00000000UL) == 0,
               "TryEORI2R - more bits in imm than Rn");
  u32 n, imm_r, imm_s;
  if (IsImmLogical(imm, Is64Bit(Rn) ? 64 : 32, &n, &imm_s, &imm_r)) {
...
void ARM64FloatEmitter::MOVI2F(ARM64Reg Rd, float value, ARM64Reg scratch,
                               bool negate) {
  ...
  if (value == 0.0) {
    ...
    _assert_msg_(
        scratch != INVALID_REG,
        "Failed to find a way to generate FP immediate %f without scratch",
        value);
    u32 ival;
    if (negate) {
      value = -value;
...
bool ARM64FloatEmitter::TryMOVI(u8 size, ARM64Reg Rd, uint64_t elementValue) {
  if (size == 8) {
    // Can always do 8.
    MOVI(size, Rd, elementValue & 0xFF, 0, false);
    return true;
  } else if (size == 16) {
    ...
    MOVI(size, Rd, imm8, 0, false);
    return true;
  }
  return false;
}

bool ARM64FloatEmitter::TryAnyMOVI(u8 size, ARM64Reg Rd,
...
void ARM64XEmitter::SUBSI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
                            ARM64Reg scratch) {
  u32 val;
  bool shift;
  if (IsImmArithmetic(imm, &val, &shift)) {
    ...
    _assert_msg_(
        scratch != INVALID_REG,
        "SUBSI2R - failed to construct immediate value from %08x, need scratch",
        (u32)imm);
    MOVI2R(scratch, imm);
    SUBS(Rd, Rn, scratch);
  }
...
void ARM64CodeBlock::PoisonMemory(int offset) {
  ...
  // remaining bytes with anything Less than optimal, but there would be nothing
  // we could do but throw a runtime warning anyway. AArch64: 0xD4200000 = BRK 0
  while (ptr < maxptr)
    *ptr++ = 0xD4200000;
}

...
// Use stb_image on Switch - libpng crashes in png_read_process_IDAT
// RetroArch uses its own rpng library for the same reason
#include "ext/stb/stb_image.h"

// *image_data_ptr should be deleted with free()
// return value of 1 == success.
int pngLoad(const char *file, int *pwidth, int *pheight,
            unsigned char **image_data_ptr) {
  // Read file into memory first
  FILE *fp = fopen(file, "rb");
  if (!fp) {
    WARN_LOG(Log::IO, "pngLoad: Failed to open file: %s", file);
  ...
  int result = pngLoadPtr(fileData, fileSize, pwidth, pheight, image_data_ptr);
  free(fileData);
  return result;
}
...
int pngLoadPtr(const unsigned char *input_ptr, size_t input_len, int *pwidth,
  ...
  *image_data_ptr = stbi_load_from_memory(input_ptr, (int)input_len, pwidth,
                                          pheight, &channels, 4);

  if (!*image_data_ptr) {
    WARN_LOG(Log::IO, "stb_image: %s", stbi_failure_reason());
    return 0;
  }
...
// PNG saving is not critical for Switch - stub it out to avoid libpng
// dependency
bool pngSave(const Path &filename, const void *buffer, int w, int h,
             int bytesPerPixel) {
  // TODO: Implement using stb_image_write if PNG saving is needed on Switch
  WARN_LOG(Log::IO, "pngSave not implemented on Switch: %s", filename.c_str());
  return false;
}

#else
// Use libpng on all other platforms
#include <png.h>
...
bool pngSave(const Path &filename, const void *buffer, int w, int h,
  ...
  if (!fp) {
    ERROR_LOG(Log::IO, "Unable to open png file for writing: %s",
              filename.c_str());
    return false;
  }

  int result =
      png_image_write_to_stdio(&png, fp, 0, buffer, row_stride, nullptr);

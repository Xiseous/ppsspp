...
// Switch: Disable shader translation - glslang crashes on Switch
// (HlslParseContext::assignClipCullDistance) and shader translation
// is not needed since we force GLES2 mode without post-processing shaders
...
void ShaderTranslationInit() {
  INFO_LOG(Log::G3D,
           "Switch: Shader translation disabled (glslang not available)\n");
}

void ShaderTranslationShutdown() {}

bool TranslateShader(std::string *dest, ShaderLanguage destLang,
                     const ShaderLanguageDesc &desc,
                     TranslatedShaderMetadata *destMetadata, std::string src,
                     ShaderLanguage srcLang, ShaderStage stage,
                     std::string *errorMessage) {
  *errorMessage = "Shader translation not available on Switch";
  return false;
}
...
void ShaderTranslationInit() { glslang::InitializeProcess(); }
void ShaderTranslationShutdown() { glslang::FinalizeProcess(); }
...
// Should probably do it in the source shader instead and then back translate to
// old style GLSL, but SPIRV-Cross currently won't compile with the Android NDK
...
static_assert(Draw::SEM_TEXCOORD0 == 3,
              "Semantic shader hardcoded in glsl below.");

bool ConvertToVulkanGLSL(std::string *dest,
                         TranslatedShaderMetadata *destMetadata,
                         std::string src, ShaderStage stage,
                         std::string *errorMessage) {
  std::stringstream out;
  ...
  static const struct {
    ...
    const char *replacement;
      ...
      {ShaderStage::Vertex, "attribute vec4 a_position;",
       "layout(location = 0) in vec4 a_position;"},
      {ShaderStage::Vertex, "attribute vec2 a_texcoord0;",
       "layout(location = 3) in vec2 a_texcoord0;"},
      {ShaderStage::Vertex, "varying vec2 v_position;",
       "layout(location = 0) out vec2 v_position;"},
      {ShaderStage::Fragment, "varying vec2 v_position;",
  ...
  while (std::getline(instream, line)) {
    ...
    if (line.find("uniform bool") != std::string::npos) {
      ...
      if (sscanf(line.c_str(), "uniform sampler2D sampler%d", &num) == 1)
        line =
            StringFromFormat("layout(set = 0, binding = %d) ", num + 1) + line;
      ...
      continue;
    } else if (2 == sscanf(line.c_str(), "varying vec%d v_texcoord%d;",
                           &vecSize, &num)) {
      if (stage == ShaderStage::Fragment) {
        line = StringFromFormat("layout(location = %d) in vec%d v_texcoord%d;",
                                num, vecSize, num);
      } else {
        line = StringFromFormat("layout(location = %d) out vec%d v_texcoord%d;",
                                num, vecSize, num);
      }
    }
    for (int i = 0; i < ARRAY_SIZE(replacements); i++) {
      if (replacements[i].stage == stage)
        line = ReplaceAll(line, replacements[i].needle,
...
bool TranslateShader(std::string *dest, ShaderLanguage destLang,
  ...
  if (srcLang != GLSL_3xx && srcLang != GLSL_1xx) {
    *errorMessage = StringFromFormat("Bad src shader language: %s",
                                     ShaderLanguageAsString(srcLang));
    return false;
  }
  ...
  if ((srcLang == GLSL_1xx || srcLang == GLSL_3xx) && destLang == GLSL_VULKAN) {
    ...
    bool result =
        ConvertToVulkanGLSL(dest, destMetadata, src, stage, errorMessage);
    return result;
  }
  ...
  // Don't enable SPIR-V and Vulkan rules when parsing GLSL. Our postshaders are
  // written in oldschool GLES 2.0.
  ...
  if (!shader.parse(&Resources, 100, EProfile::ECompatibilityProfile, false,
                    false, messages)) {
    *errorMessage = StringFromFormat(
        "%s parser failure: %s\n%s", ShaderStageAsString(stage),
  ...
  // For whatever reason, with our config, the above outputs an invalid SPIR-V
  // version, 0. Patch it up so spirv-cross accepts it.
  spirv[1] = glslang::EShTargetSpv_1_0;

  // Alright, step 1 done. Now let's take this SPIR-V shader and output in our
  // desired format.
  ...
  case GLSL_3xx: {
    ...
    // macOS OpenGL 4.1 implementation does not support
    // GL_ARB_shading_language_420pack. Prevent explicit binding location
    // emission enabled in SPIRV-Cross by default.
    options.enable_420pack_extension =
  ...
  default:
    *errorMessage = StringFromFormat("Unsupported destination language: %s",
                                     ShaderLanguageAsString(destLang));
    return false;
  }
}

...
bool MemArena::GrabMemSpace(size_t size) {
  ...
  if (ftruncate(fd, size) != 0) {
    ERROR_LOG(MEMMAP, "Failed to ftruncate %d (%s) to size %08x", (int)fd,
...
void MemArena::ReleaseSpace() { close(fd); }
...
// Do not sync memory to underlying file. Linux has this by default.
  ...
  if (retval == MAP_FAILED) {
    NOTICE_LOG(MEMMAP, "mmap on %s (fd: %d) failed: %s", ram_temp_file.c_str(),
...
void MemArena::ReleaseView(void *view, size_t size) { munmap(view, size); }

...
#define ppsspp_round_page(x)                                                   \
  ((((uintptr_t)(x)) + MEM_PAGE_MASK) & ~(MEM_PAGE_MASK))
...
static uint32_t ConvertProtFlagsWin32(uint32_t flags) {
  ...
  case MEM_PROT_READ | MEM_PROT_EXEC:
    ...
    break;
  case MEM_PROT_WRITE | MEM_PROT_EXEC:
...
static uint32_t ConvertProtFlagsUnix(uint32_t flags) {
  uint32_t protect = 0;
#ifndef HAVE_LIBNX // We don't need this on Switch
...
static void *SearchForFreeMem(size_t size) {
  ...
  while (VirtualQuery((void *)last_executable_addr, &info, sizeof(info)) ==
         sizeof(info)) {
...
void *AllocateExecutableMemory(size_t size) {
  ...
  if ((uintptr_t)&hint_location > 0xFFFFFFFFULL) {
    ...
    if (!ptr) {
      ...
      // When we deallocate, this doesn't change, so we eventually run out of
      // space.
    ...
    if (ptr) {
      ...
      WARN_LOG(COMMON, "Unable to find nearby executable memory for jit. "
                       "Proceeding with far memory.");
...
#if PPSSPP_ARCH(AMD64)
  // Try to request one that is close to our memory location if we're in high
  // memory. We use a dummy global variable to give us a good location to start
  // from.
  if (!map_hint) {
    if ((uintptr_t)&hint_location > 0xFFFFFFFFULL)
      map_hint = (char *)ppsspp_round_page(&hint_location) -
                 0x20000000; // 0.5gb lower than our approximate location
    else
      map_hint = (char *)0x20000000; // 0.5GB mark in memory
  } else if ((uintptr_t)map_hint > 0xFFFFFFFFULL) {
    map_hint -= ppsspp_round_page(
        size); /* round down to the next page if we're in high memory */
  }
#endif

#ifdef HAVE_LIBNX
  void *ptr = malloc(size); // Switch: just use malloc
#else
  ...
  if (ptr == failed_result) {
    ptr = nullptr;
    ERROR_LOG(MEMMAP, "Failed to allocate executable memory (%d) errno=%d",
              (int)size, errno);
  }
...
bool PlatformIsWXExclusive() {
  // Needed on platforms that disable W^X pages for security. Even without block
  // linking, still should be much faster than IR JIT. This might also come in
  // useful for UWP (Universal Windows Platform) if I'm understanding things
  // correctly.
  ...
  return true;
#else
  // Returning true here lets you test the W^X path on Windows and other non-W^X
  // platforms.
  return false;
#endif
}
...
bool ProtectMemoryPages(const void *ptr, size_t size, uint32_t memProtFlags) {
  VERBOSE_LOG(JIT, "ProtectMemoryPages: %p (%d) : r%d w%d x%d", ptr, (int)size,
              (memProtFlags & MEM_PROT_READ) != 0,
              (memProtFlags & MEM_PROT_WRITE) != 0,
              (memProtFlags & MEM_PROT_EXEC) != 0);

  if (PlatformIsWXExclusive()) {
    if ((memProtFlags & (MEM_PROT_WRITE | MEM_PROT_EXEC)) ==
        (MEM_PROT_WRITE | MEM_PROT_EXEC)) {
      _assert_msg_(false,
                   "Bad memory protect flags %d: W^X is in effect, can't both "
                   "write and exec",
                   memProtFlags);
    }
  }
  // Note - VirtualProtect will affect the full pages containing the requested
  // range. mprotect does not seem to, at least not on Android unless I made a
  // mistake somewhere, so we manually round.
#ifdef _WIN32
  uint32_t protect = ConvertProtFlagsWin32(memProtFlags);
  ...
  return true;
#elif defined(HAVE_LIBNX)
  return true; // Nothing to do on Switch
#else
  uint32_t protect = ConvertProtFlagsUnix(memProtFlags);
  uintptr_t page_size = GetMemoryProtectPageSize();
  ...
  if (retval != 0) {
    ERROR_LOG(MEMMAP, "mprotect failed (%p)! errno=%d (%s)", (void *)start,

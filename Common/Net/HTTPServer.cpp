...
ServerRequest::ServerRequest(int fd) : fd_(fd) {
  in_ = new net::InputSink(fd);
  out_ = new net::OutputSink(fd);
  header_.ParseHeaders(in_);

  if (header_.ok) {
    VERBOSE_LOG(Log::HTTP, "The request carried with it %i bytes",
                (int)header_.content_length);
  } else {
    Close();
  }
}
...
ServerRequest::~ServerRequest() {
  ...
  delete in_;
  if (!out_->Empty()) {
    WARN_LOG(Log::HTTP, "Output not empty - connection abort? (%s) (%d bytes)",
             this->header_.resource, (int)out_->BytesRemaining());
  }
  delete out_;
}

void ServerRequest::WriteHttpResponseHeader(const char *ver, int status,
                                            int64_t size, const char *mimeType,
                                            const char *otherHeaders) const {
  ...
  case 410:
    ...
    break;
  case 416:
    statusStr = "Range Not Satisfiable";
    break;
  case 418:
...
Server::Server(NewThreadExecutor *executor) : port_(0), executor_(executor) {
  RegisterHandler(
      "/", std::bind(&Server::HandleListing, this, std::placeholders::_1));
  SetFallbackHandler(
      std::bind(&Server::Handle404, this, std::placeholders::_1));
}

Server::~Server() { delete executor_; }

void Server::RegisterHandler(const char *url_path, UrlHandlerFunc handler) {
  handlers_[std::string(url_path)] = handler;
}

void Server::SetFallbackHandler(UrlHandlerFunc handler) { fallback_ = handler; }
...
bool Server::Listen4(int port, const char *reason) {
  ...
  setsockopt(listenerSock_, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt,
             sizeof(opt));

  if (bind(listenerSock_, (struct sockaddr *)&server_addr,
...
#endif
    ERROR_LOG(Log::HTTP,
              "%s: Failed to bind to port %d, error=%d - Bailing (ipv4)",
              reason, port, err);
  ...
  INFO_LOG(Log::HTTP, "HTTP IPv4 server started on port %d: %s (ip: %s)", port,
           reason, localAddress_.c_str());
...
bool Server::Listen6(int port, bool ipv6_only, const char *reason) {
  ...
  opt = ipv6_only ? 1 : 0;
  setsockopt(listenerSock_, IPPROTO_IPV6, IPV6_V6ONLY, (const char *)&opt,
             sizeof(opt));
...
#endif
    ERROR_LOG(Log::HTTP,
              "%s: Failed to bind to port %d, error=%d - Bailing (ipv6)",
              reason, port, err);
    closesocket(listenerSock_);
    return false;
  }
  ...
  localAddress_ = fd_util::GetLocalIP(listenerSock_);
  INFO_LOG(Log::HTTP, "HTTP IPv6 server started on port %d: %s (ip: %s)", port,
           reason, localAddress_.c_str());
  port_ = port;
  return true;
#else
...
void Server::Stop() { closesocket(listenerSock_); }
...
void Server::Handle404(const ServerRequest &request) {
  INFO_LOG(Log::HTTP, "No handler for '%.*s', falling back to 404.",
           (int)request.resource().size(), request.resource().data());

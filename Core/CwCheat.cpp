...
// Cache invalidation
//
// It should be obvious why we need to invalidate the instruction cache
// (effectively, in PPSSPP's case, the JIT translation cache) for writes. But
// currently we do also need to do it for reads, in case cheats check what MIPS
// opcode is at an address - the JIT sometimes overwrites them. Invalidating the
// cache will restore that.
//
// Long term, we should get rid of the instruction overwriting hack, or we could
// use Memory::Read_Instruction with workarounds for 8 and 16 bit reads.
...
CheatFileParser::CheatFileParser(const Path &filename,
...
bool CheatFileParser::Parse() {
  ...
  for (int lineNumber = 1; fp_ && !feof(fp_); ++lineNumber) {
    ...
    if (line.length() >= 5 && line[0] == '_') {
      ...
      errors_.push_back(StringFromFormat(
          "Unrecognized content on line %d: expecting _", lineNumber));
    }
  }
...
void CheatFileParser::AddError(const std::string &err, int lineNumber) {
  errors_.push_back(
      StringFromFormat("Error on line %d: %s", lineNumber, err.c_str()));
}
...
void CheatFileParser::ParseLine(const std::string &line, int lineNumber) {
  ...
  case 'S':
    ...
    if (ValidateGameID(line.substr(2))) {
      ...
      // For BC, let's allow if the game id doesn't match, but there's only one
      // line.
  ...
  case 'C':
    ...
    if (line.length() >= 3 && line[2] >= '1' && line[2] <= '9') {
      lastCheatInfo_ = {lineNumber, line.length() >= 5 ? line.substr(4) : "",
                        true};
      cheatEnabled_ = true;
...
void CheatFileParser::ParseDataLine(const std::string &line, int lineNumber) {
  ...
  CheatLine cheatLine;
  int len = 0;
  if (sscanf(line.c_str(), "%x %x %n", &cheatLine.part1, &cheatLine.part2,
             &len) == 2) {
...
bool CheatFileParser::ValidateGameID(std::string_view gameID) {
  return validGameID_.empty() ||
         ReplaceAll(TrimString(gameID), "-", "") == validGameID_;
}
...
static void __CheatStart() {
  ...
  // This only generates ini files on boot, let's leave homebrew ini file for
  // UI.
...
static int GetRefreshMs() {
  ...
  // Horrible hack for Tony Hawk - Underground 2. See #3854. Avoids crashing
  // somehow but still causes regular JIT invalidations which causes stutters.
...
void __CheatInit() {
  // Always register the event, want savestates to be compatible whether cheats
  // on or off.
...
void hleCheat(u64 userdata, int cyclesLate) {
  bool shouldBeEnabled =
      !Achievements::HardcoreModeActive() && g_Config.bEnableCheats;
  ...
  if (PSP_CoreParameter().compat.flags().JitInvalidationHack) {
    ...
    if (gameTitle == "ULUS10014") {
    ...
    } else if (gameTitle == "ULES00033" || gameTitle == "ULES00034" ||
               gameTitle == "ULES00035") { // euro, also 34 and 35
      ...
      currentMIPS->InvalidateICache(0x0886DD40, 4);
               ...
               "ULES00581") { // MTX MotoTrax EU (ported from US cwcheat codes)
  ...
  if (g_Config.bReloadCheats) { // Checks if the "reload cheats" button has been
                                // pressed.
    cheatEngine->ParseCheats();
    g_Config.bReloadCheats = false;
  }
  ...
  if (!File::Exists(filename_)) {
    ...
    if (!File::Exists(filename_)) {
      ...
      g_OSD.Show(OSDType::MESSAGE_ERROR,
                 err->T("Unable to create cheat file, disk may be full"));
    }
  }
}
...
CheatOperation CWCheatEngine::InterpretNextCwCheat(const CheatCode &cheat,
                                                   size_t &i) {
  ...
  case 0x8: // 8-bit or 16-bit multi-write patch data.
    ...
    if (i < cheat.lines.size()) {
      ...
      const uint32_t val =
          is8Bit ? (line2.part1 & 0xFF) : (line2.part1 & 0xFFFF);
  ...
  case 0xD: // Line skip tests & joker codes.
    ...
    case 0x3: // Inverse joker code - button not pressed.
    {
      ...
      CheatOperation op = {pressed ? CheatOp::IfPressed : CheatOp::IfNotPressed,
                           0, 0, arg & 0x0FFFFFFF};
  ...
  case 0xE: // Multiple line skip tests.
    addr = GetAddress(arg & 0x0FFFFFFF);
    {
      ...
      const uint32_t val =
          is8Bit ? (line1.part1 & 0xFF) : (line1.part1 & 0xFFFF);
...
void CWCheatEngine::ApplyMemoryOperator(const CheatOperation &op,
                                        uint32_t (*oper)(uint32_t, uint32_t)) {
  if (Memory::IsValidRange(op.addr, op.sz)) {
    InvalidateICache(op.addr, op.sz);
    if (op.sz == 1)
      Memory::WriteUnchecked_U8(
          (u8)oper(Memory::ReadUnchecked_U8(op.addr), op.val), op.addr);
    else if (op.sz == 2)
      Memory::WriteUnchecked_U16(
          (u16)oper(Memory::ReadUnchecked_U16(op.addr), op.val), op.addr);
    else if (op.sz == 4)
      Memory::WriteUnchecked_U32(
          (u32)oper(Memory::ReadUnchecked_U32(op.addr), op.val), op.addr);
  }
}

bool CWCheatEngine::TestIf(const CheatOperation &op,
                           bool (*oper)(int, int)) const {
  if (Memory::IsValidRange(op.addr, op.sz)) {
...
bool CWCheatEngine::TestIfAddr(const CheatOperation &op,
                               bool (*oper)(int, int)) const {
  if (Memory::IsValidRange(op.addr, op.sz) &&
      Memory::IsValidRange(op.ifAddrTypes.compareAddr, op.sz)) {
    InvalidateICache(op.addr, op.sz); // See note at top of file
...
void CWCheatEngine::ExecuteOp(const CheatOperation &op, const CheatCode &cheat,
                              size_t &i) {
  switch (op.op) {
  ...
  case CheatOp::Add:
    ApplyMemoryOperator(op, [](uint32_t a, uint32_t b) { return a + b; });
    break;
  ...
  case CheatOp::MultiWrite:
    if (Memory::IsValidAddress(op.addr)) {
      InvalidateICache(op.addr,
                       op.multiWrite.count * op.multiWrite.step + op.sz);
  ...
  case CheatOp::CopyBytesFrom:
    if (Memory::IsValidRange(op.addr, op.val) &&
        Memory::IsValidRange(op.copyBytesFrom.destAddr, op.val)) {
  ...
  case CheatOp::PostShader: {
    ...
    if (op.PostShaderUniform.shader < shaderChain.size()) {
      std::string shaderName =
          shaderChain[op.PostShaderUniform.shader]->section;
      g_Config.mPostShaderSetting[StringFromFormat(
          "%sSettingCurrentValue%d", shaderName.c_str(),
          op.PostShaderUniform.uniform + 1)] = op.PostShaderUniform.value.f;
    }
  } break;
  ...
  case CheatOp::PostShaderFromMemory: {
    ...
    if (Memory::IsValidRange(op.addr, 4) &&
        op.PostShaderUniform.shader < shaderChain.size()) {
      union {
        float f;
        uint32_t u;
      } value;
      ...
      case 0:
        g_Config.mPostShaderSetting[StringFromFormat(
            "%sSettingCurrentValue%d", shaderName.c_str(),
            op.PostShaderUniform.uniform + 1)] = value.u & 0x000000FF;
        break;
      case 1:
        g_Config.mPostShaderSetting[StringFromFormat(
            "%sSettingCurrentValue%d", shaderName.c_str(),
            op.PostShaderUniform.uniform + 1)] = value.u & 0x0000FFFF;
        break;
      case 2:
        g_Config.mPostShaderSetting[StringFromFormat(
            "%sSettingCurrentValue%d", shaderName.c_str(),
            op.PostShaderUniform.uniform + 1)] = value.u;
        break;
      case 3:
        g_Config.mPostShaderSetting[StringFromFormat(
            "%sSettingCurrentValue%d", shaderName.c_str(),
            op.PostShaderUniform.uniform + 1)] = value.f;
        break;
      }
    }
  } break;
  ...
  case CheatOp::CwCheatPointerCommands: {
    InvalidateICache(op.addr + op.pointerCommands.baseOffset,
                     4); // See note at top of file
    ...
    for (int a = 0; a < op.pointerCommands.count; ++a) {
      ...
      case 0x1: // type copy byte
      {
        ...
        u32 dstAddr =
            Memory::Read_U32(op.addr + op.pointerCommands.baseOffset) +
            (line.part1 & 0x0FFFFFFF);
        if (Memory::IsValidRange(dstAddr, val) &&
            Memory::IsValidRange(srcAddr, val)) {
...
bool CWCheatEngine::HasCheats() { return !cheats_.empty(); }

...
bool SymbolMap::LoadSymbolMap(const Path &filename) {
  ...
  while (!gzeof(f)) {
    ...
    if (sscanf(line, ".module %x %08x %08x %127c", (unsigned int *)&moduleIndex,
               &address, &size, name) >= 3) {
    ...
    int matched = sscanf(line, "%08x %08x %x %i %127c", &address, &size,
                         &vaddress, &typeInt, name);
    if (matched < 1)
      continue;
    type = (SymbolType)typeInt;
    if (!hasModules) {
      if (!Memory::IsValidAddress(vaddress)) {
        ERROR_LOG(Log::Loader, "Invalid address in symbol file: %08x (%s)",
                  vaddress, name);
        continue;
      }
    } else {
    ...
    // Note: it's still useful to save these for grepping and importing into
    // other tools.
...
bool SymbolMap::SaveNocashSym(const Path &filename) const {
  ...
  for (auto it = functions.begin(), end = functions.end(); it != end; ++it) {
    const FunctionEntry &e = it->second;
    fprintf(f, "%08X %s,%04X\n", GetModuleAbsoluteAddr(e.start, e.module),
            GetLabelNameRel(e.start, e.module), e.size);
  }
...
u32 SymbolMap::GetNextSymbolAddress(u32 address, SymbolType symmask) {
  ...
  std::lock_guard<std::recursive_mutex> guard(lock_);
  const auto functionEntry = symmask & ST_FUNCTION
                                 ? activeFunctions.upper_bound(address)
                                 : activeFunctions.end();
  const auto dataEntry =
      symmask & ST_DATA ? activeData.upper_bound(address) : activeData.end();

  if (functionEntry == activeFunctions.end() && dataEntry == activeData.end())
    return INVALID_ADDRESS;

  u32 funcAddress = (functionEntry != activeFunctions.end())
                        ? functionEntry->first
                        : 0xFFFFFFFF;
  u32 dataAddress =
      (dataEntry != activeData.end()) ? dataEntry->first : 0xFFFFFFFF;
...
bool SymbolMap::IsModuleActive(int moduleIndex) {
  ...
  std::lock_guard<std::recursive_mutex> guard(lock_);
  for (auto it = activeModuleEnds.begin(), end = activeModuleEnds.end();
       it != end; ++it) {
...
void SymbolMap::AddFunction(const char *name, u32 address, u32 size,
  ...
  if (existing != functions.end()) {
    ...
    if (active != activeFunctions.end() &&
        active->second.module == moduleIndex) {
...
u32 SymbolMap::GetFunctionSize(u32 startAddress) {
  if (activeNeedUpdate_) {
    ...
    // This is common, from the jit.  Direct lookup is faster than updating
    // active symbols.
...
void SymbolMap::AssignFunctionIndices() {
  std::lock_guard<std::recursive_mutex> guard(lock_);
  int index = 0;
  for (auto mod = activeModuleEnds.begin(), modend = activeModuleEnds.end();
       mod != modend; ++mod) {
...
// Copies functions, labels and data to the active set depending on which
// modules are "active".
void SymbolMap::UpdateActiveSymbols() {
  ...
  if (activeModuleEnds.empty() ||
      (functions.empty() && labels.empty() && data.empty())) {
    return;
  }
...
bool SymbolMap::SetFunctionSize(u32 startAddress, u32 newSize) {
  ...
  if (funcInfo != activeFunctions.end()) {
    auto symbolKey =
        std::make_pair(funcInfo->second.module, funcInfo->second.start);
...
void SymbolMap::SetLabelName(const char *name, u32 address) {
  ...
  if (labelInfo == activeLabels.end()) {
    ...
    auto symbolKey =
        std::make_pair(labelInfo->second.module, labelInfo->second.addr);
    ...
    if (label != labels.end()) {
      ...
      auto active = activeLabels.find(address);
      if (active != activeLabels.end() &&
          active->second.module == label->second.module) {
        activeLabels.erase(active);
...
static const DefaultSymbol defaultSymbols[] = {
    {0x08800000, "User memory"}, {0x08804000, "Default load address"},
    {0x04000000, "VRAM"},        {0x88000000, "Kernel memory"},
...
void SymbolMap::FillSymbolListBox(HWND listbox, SymbolType symType) {
  ...
  switch (symType) {
  case ST_FUNCTION: {
    SendMessage(listbox, LB_INITSTORAGE, (WPARAM)activeFunctions.size(),
                (LPARAM)activeFunctions.size() * 30);

    for (auto it = activeFunctions.begin(), end = activeFunctions.end();
         it != end; ++it) {
  ...
  case ST_DATA: {
    ...
    for (int i = 0; i < ARRAYSIZE(defaultSymbols); i++) {
      wsprintf(temp, L"0x%08X (%S)", defaultSymbols[i].address,
               defaultSymbols[i].name);
      int index = ListBox_AddString(listbox, temp);
      ListBox_SetItemData(listbox, index, defaultSymbols[i].address);
    }

    for (auto it = activeData.begin(), end = activeData.end(); it != end;
         ++it) {
      const char *name = GetLabelName(it->first);

...
public:
  ...
  bool GetSymbolInfo(SymbolInfo *info, u32 address,
                     SymbolType symmask = ST_FUNCTION);
  ...
  // Only valid for currently loaded modules.  Not guaranteed there will be a
  // function.
...
private:
  ...
  // These are flattened, read-only copies of the actual data in active modules
  // only.

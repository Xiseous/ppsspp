...
BlockDevice *ConstructBlockDevice(FileLoader *fileLoader,
  ...
  // No check above passed, should be just a regular ISO file. Let's open it as
  // a plain block device and let the other systems take over.
  if (!device) {
...
void BlockDevice::NotifyReadError() {
  if (!reportedError_) {
    ...
    g_OSD.Show(OSDType::MESSAGE_WARNING,
               err->T("Game disc read error - ISO corrupt"),
               fileLoader_->GetPath().ToVisualString(), 6.0f);
    reportedError_ = true;
  }
}
...
bool FileBlockDevice::ReadBlock(int blockNumber, u8 *outPtr, bool uncached) {
  FileLoader::Flags flags =
      uncached ? FileLoader::Flags::HINT_UNCACHED : FileLoader::Flags::NONE;
  size_t retval = fileLoader_->ReadAt((u64)blockNumber * (u64)GetBlockSize(), 1,
                                      2048, outPtr, flags);
  if (retval != 2048) {
    DEBUG_LOG(
        Log::FileSystem,
        "Could not read 2048 byte block, at block offset %d. Only got %d bytes",
        blockNumber, (int)retval);
    return false;
  }
  return true;
}
...
bool FileBlockDevice::ReadBlocks(u32 minBlock, int count, u8 *outPtr) {
  size_t retval = fileLoader_->ReadAt((u64)minBlock * (u64)GetBlockSize(), 2048,
                                      count, outPtr);
  if (retval != (size_t)count) {
    ERROR_LOG(
        Log::FileSystem,
        "Could not read %d blocks, at block offset %d. Only got %d blocks",
        count, minBlock, (int)retval);
    return false;
  }
  return true;
}
...
CISOFileBlockDevice::CISOFileBlockDevice(FileLoader *fileLoader)
  ...
  if ((frameSize & (frameSize - 1)) != 0) {
    errorString_ = StringFromFormat(
        "CSO block size %i unsupported, must be a power of two", frameSize);
    return;
  } else if (frameSize < 0x800) {
    errorString_ = StringFromFormat(
        "CSO block size %i unsupported, must be at least one sector",
        frameSize);
    return;
  }
  ...
  VERBOSE_LOG(Log::Loader, "CSO numBlocks=%i numFrames=%i align=%i", numBlocks,
              numFrames, indexShift);
  ...
  if (fileLoader->ReadAt(headerEnd, sizeof(u32), indexSize, index) !=
      indexSize) {
    NotifyReadError();
  ...
  if (fileLoader->ReadAt(headerEnd, sizeof(u32), indexSize, indexTemp) !=
      indexSize) {
    NotifyReadError();
  ...
  // Double check that the CSO is not truncated.  In most cases, this will be
  // the exact size.
  ...
  if (expectedFileSize > fileSize) {
    errorString_ = StringFromFormat(
        "Expected CSO to at least be %lld bytes, but file is %lld bytes",
...
bool CISOFileBlockDevice::ReadBlock(int blockNumber, u8 *outPtr,
  ...
  const size_t compressedReadSize =
      (size_t)(compressedReadEnd - compressedReadPos);
  const u32 compressedOffset =
      (blockNumber & ((1 << blockShift) - 1)) * GetBlockSize();

  bool plain = (idx & 0x80000000) != 0;
  if (ver_ >= 2) {
    // CSO v2+ requires blocks be uncompressed if large enough to be.  High bit
    // means other things.
    plain = compressedReadSize >= frameSize;
  }
  if (plain) {
    int readSize = (u32)fileLoader_->ReadAt(
        compressedReadPos + compressedOffset, 1, GetBlockSize(), outPtr, flags);
    ...
    const u32 readSize = (u32)fileLoader_->ReadAt(
        compressedReadPos, 1, compressedReadSize, readBuffer, flags);
...
bool CISOFileBlockDevice::ReadBlocks(u32 minBlock, int count, u8 *outPtr) {
  ...
  if (lastBlock < minBlock + count) {
    memset(outPtr + GetBlockSize() * (count - missingBlocks), 0,
           GetBlockSize() * missingBlocks);
  }
  ...
  for (u32 frame = minFrameNumber; frame <= lastFrameNumber; ++frame) {
    ...
    const u32 frameBlocks =
        std::min(lastBlock - block + 1, blocksPerFrame - frameBlockOffset);

    if (frameReadEnd > readBufferEnd) {
      const s64 maxNeeded = totalReadEnd - frameReadPos;
      const size_t chunkSize = (size_t)std::min(
          maxNeeded, (s64)std::max(frameReadSize, CSO_READ_BUFFER_SIZE));

      const u32 readSize =
          (u32)fileLoader_->ReadAt(frameReadPos, 1, chunkSize, readBuffer);
    ...
    if (plain) {
      memcpy(outPtr, rawBuffer + frameBlockOffset * GetBlockSize(),
             frameBlocks * GetBlockSize());
    } else {
      ...
      if (status != Z_STREAM_END) {
        ...
        memcpy(outPtr, zlibBuffer + frameBlockOffset * GetBlockSize(),
               frameBlocks * GetBlockSize());
...
NPDRMDemoBlockDevice::NPDRMDemoBlockDevice(FileLoader *fileLoader)
  ...
  if (psar_id == 'SISP') {
    ...
    ERROR_LOG(Log::Loader,
              "PSX not supported! Should have been caught earlier.");
  ...
  // Local kirk instance to not clash with other block devices and other
  // decryption things.
  kirk_init(&kirk_);
  ...
  // Protect against a badly decrypted header, and send information through the
  // assert about what's being played (implicitly).
  _dbg_assert_msg_(blockLBAs_ <= 4096,
                   "Bad blockLBAs in header: %08x (%s) psar: %s", blockLBAs_,
                   fileLoader->GetPath().ToVisualString().c_str(), psarStr);
  ...
  readSize =
      fileLoader_->ReadAt(psarOffset + tableOffset_, 1, tableSize_, table_);
...
bool NPDRMDemoBlockDevice::ReadBlock(int blockNumber, u8 *outPtr,
  ...
  size_t readSize = fileLoader_->ReadAt(psarOffset + table_[block].offset, 1,
                                        table_[block].size, readBuf, flags);
  ...
  if ((table_[block].flag & 4) == 0) {
    CIPHER_KEY ckey;
    sceDrmBBCipherInit(&kirk_, &ckey, 1, 2, hkey, vkey,
                       table_[block].offset >> 4);
  ...
  if (table_[block].size < blockSize_) {
    int lzsize =
        lzrc_decompress(blockBuf_, 0x00100000, readBuf, table_[block].size);
...
struct ExtendedCoreFile {
  core_file
      core; // Must be the first struct member, for some tricky pointer casts.
  uint64_t seekPos;
};
...
CHDFileBlockDevice::CHDFileBlockDevice(FileLoader *fileLoader)
  ...
  core_file_->core.fseek = [](core_file *file, int64_t offset,
                              int seekType) -> int {
  ...
  core_file_->core.fread = [](void *out_data, size_t size, size_t count,
                              core_file *file) {
    ExtendedCoreFile *coreFile = (ExtendedCoreFile *)file;
    FileLoader *loader = (FileLoader *)file->argp;
  ...
  if (err != CHDERR_NONE) {
          ERROR_LOG(Log::Loader, "Error loading CHD header for '%s': %s",
  paths[0].c_str(), chd_error_string(err)); NotifyReadError(); return;
  }
  ...
  file.fullName.GetFileExtension(); if (extension != ".chd") { continue;
                          }

                          if (chd_read_header(filepath.c_str(), &parentHeader)
  == CHDERR_NONE && memcmp(parentHeader.sha1, childHeader.parentsha1,
  sizeof(parentHeader.sha1)) == 0) {
                                  // ERROR_LOG(Log::Loader, "Checking '%s'",
  filepath.c_str()); paths[++depth] = filepath; break;
                          }
                  }

                  // Check if parentHeader was opened
                  if (parentHeader.length == 0) {
                          ERROR_LOG(Log::Loader, "Error loading CHD '%s':
  parents not found", fileLoader->GetPath().c_str()); NotifyReadError(); return;
                  }
                  memcpy(childHeader.parentsha1, parentHeader.parentsha1,
  sizeof(childHeader.parentsha1)); } while (memcmp(nullsha1,
  childHeader.parentsha1, sizeof(childHeader.sha1)) != 0);
  }
  */

  chd_file *file = nullptr;
  chd_error err =
      chd_open_core_file(&core_file_->core, CHD_OPEN_READ, NULL, &file);
  if (err != CHDERR_NONE) {
    errorString_ = StringFromFormat("CHD error: %s", paths[depth].c_str(),
...
bool CHDFileBlockDevice::ReadBlock(int blockNumber, u8 *outPtr, bool uncached) {
  if (!impl_->chd) {
    ERROR_LOG(Log::Loader, "ReadBlock: CHD not open. %s",
              fileLoader_->GetPath().c_str());
    return false;
  }
  if ((u32)blockNumber >= numBlocks) {
    memset(outPtr, 0, GetBlockSize());
  ...
  if (currentHunk != hunk) {
    ...
    if (err != CHDERR_NONE) {
      ERROR_LOG(Log::Loader, "CHD read failed: %d %d %s", blockNumber, hunk,
                chd_error_string(err));
      NotifyReadError();
    }
    currentHunk = hunk;
  }
  memcpy(outPtr, readBuffer + blockInHunk * impl_->header->unitbytes,
         GetBlockSize());
  return true;
}

...
public:
  ...
  virtual ~BlockDevice() {}
  virtual bool ReadBlock(int blockNumber, u8 *outPtr,
                         bool uncached = false) = 0;
  ...
  int GetBlockSize() const {
    return 2048;
  } // forced, it cannot be changed by subclasses
...
public:
  ...
  bool ReadBlock(int blockNumber, u8 *outPtr, bool uncached = false) override;
  bool ReadBlocks(u32 minBlock, int count, u8 *outPtr) override;
  u32 GetNumBlocks() const override {
    return (u32)(filesize_ / GetBlockSize());
  }
  bool IsDisc() const override { return true; }
  u64 GetUncompressedSize() const override { return filesize_; }
...
private:
  // This is in case two threads hit this same block device, which shouldn't
  // really happen.
  std::mutex mutex_;

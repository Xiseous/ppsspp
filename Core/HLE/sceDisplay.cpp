...
// 1.001f to compensate for the classic 59.94 NTSC framerate that the PSP seems
// to have.
...
// Value is the goal vcount number (in case the callback takes >= 1 vcount to
// return.)
...
static void ScheduleLagSync(int over = 0) {
  ...
  if (lagSyncScheduled) {
    // Reset over if it became too high, such as after pausing or initial
    // loading. There's no real sense in it being more than 1/60th of a second.
...
void __DisplayInit() {
  ...
  CoreTiming::ScheduleEvent(msToCycles(frameMs - vblankMs), enterVblankEvent,
                            0);
  curFrameTime = 0.0;
  nextFrameTime = 0.0;
  lastFrameTime = 0.0;

  __KernelRegisterWaitTypeFuncs(WAITTYPE_VBLANK, __DisplayVblankBeginCallback,
                                __DisplayVblankEndCallback);
}
...
void __DisplayDoState(PointerWrap &p) {
  ...
  CoreTiming::RestoreRegisterEvent(enterVblankEvent, "EnterVBlank",
                                   &hleEnterVblank);
  Do(p, leaveVblankEvent);
  CoreTiming::RestoreRegisterEvent(leaveVblankEvent, "LeaveVBlank",
                                   &hleLeaveVblank);
  Do(p, afterFlipEvent);
  ...
  // Display just displays the buffers the GPU has drawn, they are really
  // completely distinct. Maybe a bit tricky to move at this point, though...
...
void __DisplayShutdown() { vblankWaitingThreads.clear(); }

void __DisplayVblankBeginCallback(SceUID threadID, SceUID prevCallbackId) {
  SceUID pauseKey = prevCallbackId == 0 ? threadID : prevCallbackId;

  // This means two callbacks in a row.  PSP crashes if the same callback waits
  // inside itself (may need more testing.)
  ...
  DEBUG_LOG(Log::sceDisplay,
            "sceDisplayWaitVblankCB: Suspending vblank wait for callback");
}
...
void __DisplayVblankEndCallback(SceUID threadID, SceUID prevCallbackId) {
  ...
  vblankWaitingThreads.push_back(WaitVBlankInfo(
      __KernelGetCurThread(), vcountUnblock - __DisplayGetVCount()));
  DEBUG_LOG(Log::sceDisplay,
            "sceDisplayWaitVblankCB: Resuming vblank wait from callback");
}

void __DisplaySetWasPaused() { wasPaused = true; }
...
static void DoFrameDropLogging(float scaledTimestep) {
  if (lastFrameTime != 0.0 && !wasPaused &&
      lastFrameTime + scaledTimestep < curFrameTime) {
    const double actualTimestep = curFrameTime - lastFrameTime;

    char stats[4096];
    __DisplayGetDebugStats(stats, sizeof(stats));
    NOTICE_LOG(Log::sceDisplay,
               "Dropping frames - budget = %.2fms / %.1ffps, actual = %.2fms "
               "(+%.2fms) / %.1ffps\n%s",
               scaledTimestep * 1000.0, 1.0 / scaledTimestep,
               actualTimestep * 1000.0,
               (actualTimestep - scaledTimestep) * 1000.0, 1.0 / actualTimestep,
               stats);
  }
}

// All the throttling and frameskipping logic is here.
// This is called just before we drop out of the main loop, in order to allow
// the submit and present to happen.
static void DoFrameTiming(bool throttle, bool *skipFrame, float scaledTimestep,
                          bool endOfFrame) {
  PROFILE_THIS_SCOPE("timing");
  *skipFrame = false;

  // Check if the frameskipping code should be enabled. If neither throttling or
  // frameskipping is on, we have nothing to do here.
  ...
  if (lastFrameTime == 0.0 || wasPaused) {
    ...
    nextFrameTime =
        std::max(lastFrameTime + scaledTimestep,
                 time_now_d() - maxFallBehindFrames * scaledTimestep);
  }
  ...
  // Auto-frameskip automatically if speed limit is set differently than the
  // default.
  ...
  if (curFrameTime < nextFrameTime && throttle) {
    ...
    if (nextFrameTime - curFrameTime > 2 * scaledTimestep) {
      ...
      // If we're ending the frame here, we'll defer the sleep until after the
      // command buffers have been handed off to the render thread, for some
      // more overlap.
      if (endOfFrame) {
        g_frameTiming.DeferWaitUntil(nextFrameTime, &curFrameTime);
      } else {
        WaitUntil(curFrameTime, nextFrameTime, "display-wait");
        curFrameTime = time_now_d(); // I guess we could also just set it to
                                     // nextFrameTime...
      }
    }
  }
...
static void DoFrameIdleTiming() {
  ...
  // If we have over at least a vblank of spare time, maintain at least 30fps in
  // delay. This prevents fast forward during loading screens. Give a little
  // extra wiggle room in case the next vblank does more work.
  const double goal =
      lastFrameTime + (numVBlanksSinceFlip - 1) * scaledVblank - 0.001;
  if (numVBlanksSinceFlip >= 2 && before < goal) {
    ...
    if ((DebugOverlay)g_Config.iDebugOverlay == DebugOverlay::FRAME_GRAPH ||
...
void hleEnterVblank(u64 userdata, int cyclesLate) {
  ...
  CoreTiming::ScheduleEvent(msToCycles(vblankMs) - cyclesLate, leaveVblankEvent,
                            vbCount + 1);

  // Trigger VBlank interrupt handlers.
  __TriggerInterrupt(PSP_INTR_IMMEDIATE | PSP_INTR_ONLY_IF_ENABLED |
                         PSP_INTR_ALWAYS_RESCHED,
                     PSP_VBLANK_INTR, PSP_INTR_SUB_ALL);
  ...
  for (size_t i = 0; i < vblankWaitingThreads.size(); i++) {
    if (--vblankWaitingThreads[i].vcountUnblock == 0) {
      ...
      SceUID waitID = __KernelGetWaitID(vblankWaitingThreads[i].threadID,
                                        WAITTYPE_VBLANK, error);
      if (waitID == 1) {
  ...
  if (framebufIsLatched) {
    DEBUG_LOG(Log::sceDisplay, "Setting latched framebuffer %08x (prev: %08x)",
              latchedFramebuf.topaddr, framebuf.topaddr);
...
static void NotifyUserIfSlow() {
  ...
  if (!g_Config.bHideSlowWarnings && !hasNotifiedSlow &&
      PSP_CoreParameter().fpsLimit == FPSLimit::NORMAL &&
      DisplayIsRunningSlow() && g_Config.bSoftwareRendering) {
#ifndef _DEBUG
    auto err = GetI18NCategory(I18NCat::ERRORS);
    g_OSD.Show(OSDType::MESSAGE_INFO,
               err->T("Running slow: Try turning off Software Rendering"),
               5.0f);
#endif
...
void __DisplayFlip(int cyclesLate) {
  ...
  flippedThisFrame = true;
  // We flip only if the framebuffer was dirty. This eliminates flicker when
  // using non-buffered rendering. The interaction with frame skipping seems to
  // need some work. But, let's flip at least once every 10 vblanks, to update
  // fps, etc.
  const bool noRecentFlip =
      !g_Config.bSkipBufferEffects && numVBlanksSinceFlip >= 10;
  // Also let's always flip for animated shaders.
  bool postEffectRequiresFlip = false;

  bool duplicateFrames =
      g_Config.bRenderDuplicateFrames && g_Config.iFrameSkip == 0;

  if (!g_Config.bSkipBufferEffects) {
    postEffectRequiresFlip =
        duplicateFrames || g_Config.bShaderChainRequires60FPS;
  }

  if (!FrameTimingThrottled()) {
    // NOTICE_LOG(Log::System, "Throttle: %d %d", (int)fastForwardSkipFlip,
    // (int)postEffectRequiresFlip);
  }
  ...
  if (!needFlip) {
    // Okay, there's no new frame to draw, game might be sitting in a static
    // loading screen or similar, and not long enough to trigger noRecentFlip.
    // But audio may be playing, so we need to time still.
    DoFrameIdleTiming();
  ...
  // Avoid skipping on devices that have 58 or 59 FPS, except when alternate
  // speed is set.
  const double fpsLimit = FrameTimingLimit();
  bool throttle = fpsLimit != 0.0;

  bool refreshRateNeedsSkip =
      (fpsLimit != framerate && fpsLimit > refreshRate) || !throttle;
  ...
  if (g_frameTiming.FastForwardNeedsSkipFlip() &&
      (!FrameTimingThrottled() || refreshRateNeedsSkip)) {
  ...
  if (numSkippedFrames >= maxFrameskip ||
      gpuDebug->GetRecorder()->IsActivePending()) {
    skipFrame = false;
  }
  ...
  if (skipFrame) {
    ...
    // NOTE!! It can happen that if we just toggled frameskip (especially auto),
    // we are still in a state where we don't have a framebuffer bound, from the
    // last frame. But framebuffermanager still might think that we're in
    // non-buffered mode.
    if (gpu->GetFramebufferManagerCommon() &&
        !gpu->GetFramebufferManagerCommon()->UseBufferedRendering() &&
        !g_Config.bSkipBufferEffects) {
      gpu->GetFramebufferManagerCommon()->ForceUseBufferedRendering(
          !g_Config.bSkipBufferEffects);
      gstate_c.skipDrawReason &= ~SKIPDRAW_NON_DISPLAYED_FB;
    }
  }

  // Returning here with coreState == CORE_NEXTFRAME causes a buffer flip to
  // happen (next frame). Right after, we regain control for a little bit in
  // hleAfterFlip. I think that's a great place to do housekeeping.
...
void hleLeaveVblank(u64 userdata, int cyclesLate) {
  ...
  CoreTiming::ScheduleEvent(msToCycles(frameMs - vblankMs) - cyclesLate,
                            enterVblankEvent, userdata);
...
void hleLagSync(u64 userdata, int cyclesLate) {
  // The goal here is to prevent network, audio, and input lag from the real
  // world. Our normal timing is very "stop and go".  This is efficient, but
  // causes real world lag. This event (optionally) runs every 1ms to sync with
  // the real world.
  PROFILE_THIS_SCOPE("timing");
...
void __DisplayWaitForVblanks(const char *reason, int vblanks, bool callbacks) {
  ...
  // These syscalls take about 115 us, so if the next vblank is before then,
  // we're waiting extra. At least, on real firmware a wait >= 16500 into the
  // frame will wait two.
  if (cyclesToNextVblank <= usToCycles(115)) {
    ++vblanks;
  }

  vblankWaitingThreads.push_back(
      WaitVBlankInfo(__KernelGetCurThread(), vblanks));
  __KernelWaitCurThread(WAITTYPE_VBLANK, 1, 0, 0, callbacks, reason);
}

static u32 sceDisplaySetMode(int displayMode, int displayWidth,
                             int displayHeight) {
  if (displayMode != PSP_DISPLAY_MODE_LCD || displayWidth != 480 ||
      displayHeight != 272) {
    WARN_LOG_REPORT(Log::sceDisplay,
                    "Video out requested, not supported: mode=%d size=%d,%d",
                    displayMode, displayWidth, displayHeight);
  }
  if (displayMode != PSP_DISPLAY_MODE_LCD) {
    return hleLogWarning(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_MODE,
                         "invalid mode");
  }
  if (displayWidth != 480 || displayHeight != 272) {
    return hleLogWarning(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_SIZE,
                         "invalid size");
  }
...
void __DisplaySetFramebuf(u32 topaddr, int linesize, int pixelFormat,
                          int sync) {
  ...
  if (sync == PSP_DISPLAY_SETBUF_IMMEDIATE) {
    ...
    // Doing it in non-buffered though creates problems (black screen) on
    // occasion though so let's not.
    ...
    // If we update the format or stride, this affects the current framebuf
    // immediately.
...
// Some games (GTA) never call this during gameplay, so bad place to put a
// framerate counter.
int sceDisplaySetFramebuf(u32 topaddr, int linesize, int pixelformat,
                          int sync) {
  if (sync != PSP_DISPLAY_SETBUF_IMMEDIATE &&
      sync != PSP_DISPLAY_SETBUF_NEXTFRAME) {
    return hleLogError(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_MODE,
                       "invalid sync mode");
  }
  if (topaddr != 0 && !Memory::IsRAMAddress(topaddr) &&
      !Memory::IsVRAMAddress(topaddr)) {
    return hleLogError(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_POINTER,
                       "invalid address");
  }
  if ((topaddr & 0xF) != 0) {
    return hleLogError(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_POINTER,
  ...
  if (pixelformat < 0 || pixelformat > GE_FORMAT_8888) {
    return hleLogError(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_FORMAT,
                       "invalid format");
  }

  if (sync == PSP_DISPLAY_SETBUF_IMMEDIATE) {
    if ((GEBufferFormat)pixelformat != latchedFramebuf.fmt ||
        linesize != latchedFramebuf.stride) {
      return hleReportWarning(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_MODE,
                              "must change latched framebuf first");
    }
  }
  ...
  if (topaddr != 0 &&
      (topaddr != framebuf.topaddr ||
       PSP_CoreParameter().compat.flags().SplitFramebufferMargin) &&
      framebuf.topaddr != 0 &&
      PSP_CoreParameter().compat.flags().ForceMax60FPS) {
    // sceDisplaySetFramebuf() isn't supposed to delay threads at all.  This is
    // a hack. So let's only delay when it's more than 1ms.
    const s64 FLIP_DELAY_CYCLES_MIN = usToCycles(1000);
    // Some games (like Final Fantasy 4) only call this too much in spurts.
    // The goal is to fix games where this would result in a consistent
    // overhead.
    const int FLIP_DELAY_MIN_FLIPS = 30;
    // Since we move nextFlipCycles forward a whole frame each time, we allow it
    // to be a little ahead. Otherwise it'll always be ahead if the game messes
    // up even once.
  ...
  if (delayCycles > 0 && !__IsInInterrupt()) {
    // Okay, the game is going at too high a frame rate.  God of War and Fat
    // Princess both do this. Simply eating the cycles works and is fast, but
    // breaks other games (like Jeanne d'Arc.) So, instead, we delay this HLE
    // thread only (a small deviation from correct behavior.)
    return hleDelayResult(
        hleLogDebug(Log::sceDisplay, 0, "delaying frame thread"),
        "set framebuf", cyclesToUs(delayCycles));
  } else {
    if (topaddr == 0) {
...
bool __DisplayGetFramebuf(PSPPointer<u8> *topaddr, u32 *linesize,
                          u32 *pixelFormat, int latchedMode) {
  const FrameBufferState &fbState =
      latchedMode == PSP_DISPLAY_SETBUF_NEXTFRAME ? latchedFramebuf : framebuf;
  if (topaddr != nullptr)
...
static u32 sceDisplayGetFramebuf(u32 topaddrPtr, u32 linesizePtr,
                                 u32 pixelFormatPtr, int latchedMode) {
  const FrameBufferState &fbState =
      latchedMode == PSP_DISPLAY_SETBUF_NEXTFRAME ? latchedFramebuf : framebuf;
...
static int sceDisplayWaitVblankStartMulti(int vblanks) {
  if (vblanks <= 0) {
    return hleLogWarning(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_VALUE,
                         "invalid number of vblanks");
  }
  if (!__KernelIsDispatchEnabled())
    return hleLogWarning(Log::sceDisplay, SCE_KERNEL_ERROR_CAN_NOT_WAIT,
                         "dispatch disabled");
  if (__IsInInterrupt())
    return hleLogWarning(Log::sceDisplay, SCE_KERNEL_ERROR_ILLEGAL_CONTEXT,
                         "in interrupt");
...
static int sceDisplayAdjustAccumulatedHcount(int value) {
  if (value < 0) {
    return hleLogError(Log::sceDisplay, SCE_KERNEL_ERROR_INVALID_VALUE,
                       "invalid value");
  }

  // Since it includes the current hCount, find the difference to apply to the
  // base.
...
static u32 sceDisplaySetResumeMode(u32 rMode) {
  // Not sure what this does, seems to do nothing in tests and accept all
  // values.
  resumeMode = rMode;
...
const HLEFunction sceDisplay[] = {
    {0X0E20F177, &WrapU_III<sceDisplaySetMode>, "sceDisplaySetMode", 'x',
     "iii"},
    {0X289D82FE, &WrapI_UIII<sceDisplaySetFramebuf>, "sceDisplaySetFrameBuf",
     'i', "xiii"},
    {0XEEDA2E54, &WrapU_UUUI<sceDisplayGetFramebuf>, "sceDisplayGetFrameBuf",
     'x', "pppi"},
    {0X36CDFADE, &WrapI_V<sceDisplayWaitVblank>, "sceDisplayWaitVblank", 'i',
     "", HLE_NOT_DISPATCH_SUSPENDED},
    {0X984C27E7, &WrapI_V<sceDisplayWaitVblankStart>,
     "sceDisplayWaitVblankStart", 'i', "",
     HLE_NOT_IN_INTERRUPT | HLE_NOT_DISPATCH_SUSPENDED},
    {0X40F1469C, &WrapI_I<sceDisplayWaitVblankStartMulti>,
     "sceDisplayWaitVblankStartMulti", 'i', "i"},
    {0X8EB9EC49, &WrapI_V<sceDisplayWaitVblankCB>, "sceDisplayWaitVblankCB",
     'i', "", HLE_NOT_DISPATCH_SUSPENDED},
    {0X46F186C3, &WrapI_V<sceDisplayWaitVblankStartCB>,
     "sceDisplayWaitVblankStartCB", 'i', "",
     HLE_NOT_IN_INTERRUPT | HLE_NOT_DISPATCH_SUSPENDED},
    {0X77ED8B3A, &WrapI_I<sceDisplayWaitVblankStartMultiCB>,
     "sceDisplayWaitVblankStartMultiCB", 'i', "i"},
    {0XDBA6C4C4, &WrapF_V<sceDisplayGetFramePerSec>, "sceDisplayGetFramePerSec",
     'f', ""},
    {0X773DD3A3, &WrapI_V<sceDisplayGetCurrentHcount>,
     "sceDisplayGetCurrentHcount", 'i', ""},
    {0X210EAB3A, &WrapI_V<sceDisplayGetAccumulatedHcount>,
     "sceDisplayGetAccumulatedHcount", 'i', ""},
    {0XA83EF139, &WrapI_I<sceDisplayAdjustAccumulatedHcount>,
     "sceDisplayAdjustAccumulatedHcount", 'i', "i"},
    {0X9C6EAAD7, &WrapI_V<sceDisplayGetVcount>, "sceDisplayGetVcount", 'i', ""},
    {0XDEA197D4, &WrapU_UUU<sceDisplayGetMode>, "sceDisplayGetMode", 'x',
     "ppp"},
    {0X7ED59BC4, &WrapU_U<sceDisplaySetHoldMode>, "sceDisplaySetHoldMode", 'x',
     "x"},
    ...
    {0XB4F378FA, &WrapU_V<sceDisplayIsForeground>, "sceDisplayIsForeground",
     'x', ""},
    {0X31C4BAA8, &WrapU_UU<sceDisplayGetBrightness>, "sceDisplayGetBrightness",
     'x', "pp"},
    {0X9E3C6DC6, &WrapU_II<sceDisplaySetBrightness>, "sceDisplaySetBrightness",
     'x', "ii"},

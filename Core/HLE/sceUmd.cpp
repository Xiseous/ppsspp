...
static constexpr uint32_t UMD_STAT_ALLOW_WAIT =
    PSP_UMD_NOT_PRESENT | PSP_UMD_PRESENT | PSP_UMD_NOT_READY | PSP_UMD_READY |
    PSP_UMD_READABLE;
...
void __UmdInit() {
  umdStatTimeoutEvent =
      CoreTiming::RegisterEvent("UmdTimeout", __UmdStatTimeout);
  umdStatChangeEvent = CoreTiming::RegisterEvent("UmdChange", __UmdStatChange);
  umdInsertChangeEvent =
      CoreTiming::RegisterEvent("UmdInsertChange", __UmdInsertChange);
  umdActivated = true;
  umdStatus = 0;
  umdErrorStat = 0;
  driveCBId = 0;
  ...
  __KernelRegisterWaitTypeFuncs(WAITTYPE_UMD, __UmdBeginCallback,
                                __UmdEndCallback);
}
...
void __UmdDoState(PointerWrap &p) {
  ...
  CoreTiming::RestoreRegisterEvent(umdStatTimeoutEvent, "UmdTimeout",
                                   __UmdStatTimeout);
  Do(p, umdStatChangeEvent);
  CoreTiming::RestoreRegisterEvent(umdStatChangeEvent, "UmdChange",
                                   __UmdStatChange);
  ...
  CoreTiming::RestoreRegisterEvent(umdInsertChangeEvent, "UmdInsertChange",
                                   __UmdInsertChange);
}
...
static void __KernelUmdActivate() {
  ...
  CoreTiming::ScheduleEvent(usToCycles(MICRO_DELAY_ACTIVATE),
                            umdStatChangeEvent, 1);
}
...
static void __UmdBeginCallback(SceUID threadID, SceUID prevCallbackId) {
  ...
  if (HLEKernel::VerifyWait(threadID, WAITTYPE_UMD, 1)) {
    // This means two callbacks in a row.  PSP crashes if the same callback runs
    // inside itself.
    ...
    DEBUG_LOG(Log::sceIo,
              "sceUmdWaitDriveStatCB: Suspending lock wait for callback");
  } else
    WARN_LOG_REPORT(
        Log::sceIo,
        "sceUmdWaitDriveStatCB: beginning callback with bad wait id?");
}
...
static void __UmdEndCallback(SceUID threadID, SceUID prevCallbackId) {
  ...
  if (cyclesLeft < 0 && waitDeadline != 0)
    ...
    _dbg_assert_msg_(umdStatTimeoutEvent != -1, "Must have a umd timer");
    CoreTiming::ScheduleEvent(cyclesLeft, umdStatTimeoutEvent,
                              __KernelGetCurThread());

    umdWaitingThreads.push_back(threadID);

    DEBUG_LOG(Log::sceIo,
              "sceUmdWaitDriveStatCB: Resuming lock wait for callback");
  }
}
...
static u32 sceUmdRegisterUMDCallBack(u32 cbId) {
  ...
  // TODO: If the callback is invalid, return
...
static u32 sceUmdGetDriveStat() {
  if (!UMDInserted) {
    return hleLogWarning(
        Log::sceIo, PSP_UMD_NOT_PRESENT,
  ...
  return hleLogVerbose(Log::sceIo, retVal, "0x%02x=sceUmdGetDriveStat()",
                       retVal);
}
...
static void __UmdWaitStat(u32 timeout) {
  ...
  CoreTiming::ScheduleEvent(usToCycles((int)timeout), umdStatTimeoutEvent,
                            __KernelGetCurThread());
}
...
static int sceUmdWaitDriveStat(u32 stat) {
  if ((stat & UMD_STAT_ALLOW_WAIT) == 0) {
    return hleLogDebug(Log::sceIo, SCE_KERNEL_ERROR_ERRNO_INVALID_ARGUMENT,
                       "bad status");
  }
  if (!__KernelIsDispatchEnabled()) {
    return hleLogDebug(Log::sceIo, SCE_KERNEL_ERROR_CAN_NOT_WAIT,
                       "dispatch disabled");
  }
  if (__IsInInterrupt()) {
    return hleLogDebug(Log::sceIo, SCE_KERNEL_ERROR_ILLEGAL_CONTEXT,
                       "inside interrupt");
  }
...
static int sceUmdWaitDriveStatWithTimer(u32 stat, u32 timeout) {
  ...
  if ((stat & __KernelUmdGetState()) == 0) {
    ...
    umdWaitingThreads.push_back(__KernelGetCurThread());
    __KernelWaitCurThread(WAITTYPE_UMD, 1, stat, 0, false,
                          "umd stat waited with timer");
...
static u32 sceUmdCancelWaitDriveStat() {
  for (size_t i = 0; i < umdWaitingThreads.size(); ++i) {
    ...
    HLEKernel::ResumeFromWait(threadID, WAITTYPE_UMD, 1,
                              (int)SCE_KERNEL_ERROR_WAIT_CANCEL);
  }
  umdWaitingThreads.clear();
...
void __UmdReplace(const Path &filepath) {
  ...
  CoreTiming::ScheduleEvent(usToCycles(200 * 1000), umdInsertChangeEvent,
                            0); // Wait sceUmdCheckMedium call
...
bool getUMDReplacePermit() { return g_UMDReplacePermit; }
...
const HLEFunction sceUmdUser[] = {
    ...
    {0X8EF08FCE, &WrapI_U<sceUmdWaitDriveStat>, "sceUmdWaitDriveStat", 'i',
     "x"},
    {0X56202973, &WrapI_UU<sceUmdWaitDriveStatWithTimer>,
     "sceUmdWaitDriveStatWithTimer", 'i', "xx"},
    {0X4A9E5E29, &WrapI_UU<sceUmdWaitDriveStatCB>, "sceUmdWaitDriveStatCB", 'i',
     "xx"},
    {0X6AF9B50A, &WrapU_V<sceUmdCancelWaitDriveStat>,
     "sceUmdCancelWaitDriveStat", 'x', ""},
    {0X20628E6F, &WrapU_V<sceUmdGetErrorStat>, "sceUmdGetErrorStat", 'x', ""},
    {0X340B7686, &WrapU_U<sceUmdGetDiscInfo>, "sceUmdGetDiscInfo", 'x', "x"},
    {0XAEE7404D, &WrapU_U<sceUmdRegisterUMDCallBack>,
     "sceUmdRegisterUMDCallBack", 'x', "x"},
    {0XBD2BDE07, &WrapI_I<sceUmdUnRegisterUMDCallBack>,
     "sceUmdUnRegisterUMDCallBack", 'i', "i"},

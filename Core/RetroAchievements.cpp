...
// be seen above, relicensed to GPL 2.0.
...
static bool HashISOFile(ISOFileSystem *fs, const std::string filename,
                        md5_context *md5) {
...
static std::string FormatRCheevosMD5(uint8_t digest[16]) {
  char hashStr[33];
  /* NOTE: sizeof(hash) is 4 because it's still treated like a pointer, despite
   * specifying a size */
  snprintf(hashStr, 33,
...
// If failed, returns an empty string, otherwise a 32-character string with the
// hash in hex format.
...
static std::string ComputePSPHomebrewHash(FileLoader *fileLoader) {
  ...
  // Cap the data we read to 64MB (MAX_BUFFER_SIZE in rcheevos' hash.c), and
  // hash that.
...
static inline const char *DeNull(const char *ptr) { return ptr ? ptr : ""; }
...
rc_client_t *GetClient() { return g_rcClient; }
...
bool HardcoreModeActive() {
  ...
  return IsLoggedIn() && rc_client_get_hardcore_enabled(g_rcClient) &&
         rc_client_is_processing_required(g_rcClient);
}
...
bool WarnUserIfHardcoreModeActive(bool isSaveStateAction,
  ...
  if (!HardcoreModeActive() ||
      (isSaveStateAction && g_Config.bAchievementsSaveStateInHardcoreMode)) {
    return false;
  }
...
bool IsBlockingExecution() {
  if (g_isLoggingIn || g_isIdentifying) {
    ...
    // INFO_LOG(Log::Achievements, "isLoggingIn: %d   isIdentifying: %d",
    // (int)g_isLoggingIn, (int)g_isIdentifying);
  }
  return g_isLoggingIn || g_isIdentifying;
}

bool IsActive() { return GetGameID() != 0; }

#ifdef RC_CLIENT_SUPPORTS_RAINTEGRATION

static void raintegration_write_memory_handler(uint32_t address,
                                               uint8_t *buffer,
                                               uint32_t num_bytes,
                                               rc_client_t *client) {
...
static uint32_t read_memory_callback(uint32_t address, uint8_t *buffer,
                                     uint32_t num_bytes, rc_client_t *client) {
  // Achievements are traditionally defined relative to the base of main memory
  // of the emulated console. This is some kind of RetroArch-related legacy. In
  // the PSP's case, this is simply a straight offset of 0x08000000.
  ...
  if (!Memory::IsValidRange(address, num_bytes)) {
    ...
    if (g_Config.bAchievementsLogBadMemReads) {
      WARN_LOG(Log::G3D,
               "RetroAchievements PeekMemory: Bad address %08x (%d bytes) "
               "(%08x was passed in)",
               address, num_bytes, orig_address);
    }

    // This tells rcheevos that the access was bad, which should now be handled
    // properly.
    return 0;
  }
...
// This is the HTTP request dispatcher that is provided to the rc_client.
// Whenever the client needs to talk to the server, it will call this function.
static void server_call_callback(const rc_api_request_t *request,
                                 rc_client_server_callback_t callback,
                                 void *callback_data, rc_client_t *client) {
  // If post data is provided, we need to make a POST request, otherwise, a GET
  // request will suffice.
  auto ac = GetI18NCategory(I18NCat::ACHIEVEMENTS);
  if (request->post_data) {
    std::shared_ptr<http::Request> download =
        g_DownloadManager.AsyncPostWithCallback(
            std::string(request->url), std::string(request->post_data),
            "application/x-www-form-urlencoded",
            http::RequestFlags::ProgressBar |
                http::RequestFlags::ProgressBarDelayed,
...
static void log_message_callback(const char *message,
                                 const rc_client_t *client) {
  INFO_LOG(Log::Achievements, "RetroAchievements: %s", message);
}

// For detailed documentation, see
// https://github.com/RetroAchievements/rcheevos/wiki/rc_client_set_event_handler.
static void event_handler_callback(const rc_client_event_t *event,
                                   rc_client_t *client) {
  auto ac = GetI18NCategory(I18NCat::ACHIEVEMENTS);

  switch (event->type) {
  case RC_CLIENT_EVENT_ACHIEVEMENT_TRIGGERED:
    // An achievement was earned by the player. The handler should notify the
    // player that the achievement was earned.
    g_OSD.ShowAchievementUnlocked(event->achievement->id);
    System_PostUIMessage(UIMessage::REQUEST_PLAY_SOUND, "achievement_unlocked");
    INFO_LOG(Log::Achievements, "Achievement unlocked: '%s' (%d)",
  ...
  case RC_CLIENT_EVENT_GAME_COMPLETED: {
    ...
    // All achievements for the game have been earned. The handler should notify
    // the player that the game was completed or mastered, depending on mode,
    // hardcore or not.
    auto ac = GetI18NCategory(I18NCat::ACHIEVEMENTS);
    ...
    // TODO: Translation?
    std::string title =
        ApplySafeSubstitutions(ac->T("Mastered %1"), gameInfo->title);

    rc_client_user_game_summary_t summary;
    rc_client_get_user_game_summary(g_rcClient, &summary);

    std::string message = ApplySafeSubstitutions(
        ac->T("%1 achievements, %2 points"), summary.num_unlocked_achievements,
        summary.points_unlocked);

    g_OSD.Show(OSDType::MESSAGE_INFO, title, message,
               DeNull(gameInfo->badge_name), 10.0f);
  ...
  case RC_CLIENT_EVENT_LEADERBOARD_FAILED: {
    ...
    // A leaderboard attempt has started. The handler may show a message with
    // the leaderboard title and /or description indicating the attempt started.
    const char *title = "";
    const char *description = "";
    // Hack around some problematic events in Burnout Legends. Hopefully this
    // can be fixed in the backend.
    ...
    INFO_LOG(Log::Achievements, "Attempt %s: %s",
             started ? "started" : "failed", title);
    g_OSD.ShowLeaderboardStartEnd(
        ApplySafeSubstitutions(
            ac->T(started ? "%1: Attempt started" : "%1: Attempt failed"),
            title),
        description, started);
    break;
  }
  case RC_CLIENT_EVENT_LEADERBOARD_SUBMITTED: {
    INFO_LOG(Log::Achievements, "Leaderboard result submitted: %s",
             event->leaderboard->title);
    // Actually showing the result when we get the scoreboard message and have
    // the new rank.
    break;
  }
  case RC_CLIENT_EVENT_LEADERBOARD_SCOREBOARD: {
    ...
    const rc_client_leaderboard_scoreboard_t *scoreboard =
        event->leaderboard_scoreboard;
    if (event->leaderboard_scoreboard) {
      msg = ApplySafeSubstitutions(ac->T("Submitted %1 for %2"),
                                   DeNull(scoreboard->submitted_score), title);
      if (!strcmp(scoreboard->best_score, scoreboard->submitted_score)) {
        ...
        msg += ": ";
        msg +=
            ApplySafeSubstitutions(ac->T("Best: %1"), scoreboard->best_score);
      }
    } else {
      msg = ApplySafeSubstitutions(ac->T("Submitted %1 for %2"),
                                   DeNull(event->leaderboard->tracker_value),
                                   title);
    }
    g_OSD.ShowLeaderboardSubmitted(msg, "");
    System_PostUIMessage(UIMessage::REQUEST_PLAY_SOUND,
                         "leaderboard_submitted");
    break;
  }
  case RC_CLIENT_EVENT_ACHIEVEMENT_CHALLENGE_INDICATOR_SHOW:
    INFO_LOG(Log::Achievements, "Challenge indicator show: %s",
    ...
    // A challenge achievement has become active. The handler should show a
    // small version of the achievement icon to indicate the challenge is
    // active.
    break;
  case RC_CLIENT_EVENT_ACHIEVEMENT_CHALLENGE_INDICATOR_HIDE:
    INFO_LOG(Log::Achievements, "Challenge indicator hide: %s",
             event->achievement->title);
    g_OSD.ShowChallengeIndicator(event->achievement->id, false);
    g_activeChallenges.erase(event->achievement->id);
    // The handler should hide the small version of the achievement icon that
    // was shown by the corresponding
    // RC_CLIENT_EVENT_ACHIEVEMENT_CHALLENGE_INDICATOR_SHOW event.
    break;
  case RC_CLIENT_EVENT_ACHIEVEMENT_PROGRESS_INDICATOR_SHOW:
    INFO_LOG(Log::Achievements,
             "Progress indicator show: %s, progress: '%s' (%f)",
             event->achievement->title, event->achievement->measured_progress,
             event->achievement->measured_percent);
    // An achievement that tracks progress has changed the amount of progress
    // that has been made. The handler should show a small version of the
    // achievement icon along with the achievement->measured_progress text (for
    // two seconds). Only one progress indicator should be shown at a time. If a
    // progress indicator is already visible, it should be updated with the new
    // icon and text, and the two second timer should be restarted.
    g_OSD.ShowAchievementProgress(event->achievement->id, true);
    break;
  case RC_CLIENT_EVENT_ACHIEVEMENT_PROGRESS_INDICATOR_UPDATE:
    INFO_LOG(Log::Achievements,
             "Progress indicator update: %s, progress: '%s' (%f)",
             event->achievement->title, event->achievement->measured_progress,
             event->achievement->measured_percent);
    g_OSD.ShowAchievementProgress(event->achievement->id, true);
    break;
  ...
  case RC_CLIENT_EVENT_LEADERBOARD_TRACKER_SHOW:
    INFO_LOG(Log::Achievements, "Leaderboard tracker show: '%s' (id %d)",
             event->leaderboard_tracker->display,
             event->leaderboard_tracker->id);
    // A leaderboard_tracker has become active. The handler should show the
    // tracker text on screen. Multiple active leaderboards may share a single
    // tracker if they have the same definition and value. As such, the
    // leaderboard tracker IDs are unique amongst the leaderboard trackers, and
    // have no correlation to the active leaderboard(s). Use
    // event->leaderboard_tracker->id for uniqueness checks, and display
    // event->leaderboard_tracker->display (string)
    g_OSD.ShowLeaderboardTracker(event->leaderboard_tracker->id,
                                 event->leaderboard_tracker->display, true);
    break;
  case RC_CLIENT_EVENT_LEADERBOARD_TRACKER_HIDE:
    // A leaderboard_tracker has become inactive. The handler should hide the
    // tracker text from the screen.
    INFO_LOG(Log::Achievements, "Leaderboard tracker hide: '%s' (id %d)",
             event->leaderboard_tracker->display,
             event->leaderboard_tracker->id);
    g_OSD.ShowLeaderboardTracker(event->leaderboard_tracker->id, "", false);
    break;
  case RC_CLIENT_EVENT_LEADERBOARD_TRACKER_UPDATE:
    // A leaderboard_tracker value has been updated. The handler should update
    // the tracker text on the screen.
    INFO_LOG(Log::Achievements, "Leaderboard tracker update: '%s' (id %d)",
             event->leaderboard_tracker->display,
             event->leaderboard_tracker->id);
    g_OSD.ShowLeaderboardTracker(event->leaderboard_tracker->id,
                                 event->leaderboard_tracker->display, true);
  ...
  case RC_CLIENT_EVENT_RESET:
    WARN_LOG(Log::Achievements,
             "Resetting game due to achievement setting change!");
    // Hardcore mode was enabled, or something else that forces a game reset.
    System_PostUIMessage(UIMessage::REQUEST_GAME_RESET);
    break;
  case RC_CLIENT_EVENT_SERVER_ERROR:
    ERROR_LOG(Log::Achievements, "Server error: %s: %s",
              event->server_error->api, event->server_error->error_message);
    g_OSD.Show(OSDType::MESSAGE_ERROR, "Server error", "", g_RAImageID);
    break;
  default:
    WARN_LOG(Log::Achievements, "Unhandled rc_client event %d, ignoring",
             event->type);
    break;
  }
}

static void login_token_callback(int result, const char *error_message,
  ...
  case RC_OK: {
    ...
    if (!isInitialAttempt) {
      ...
      g_OSD.Show(OSDType::MESSAGE_SUCCESS,
                 ac->T("Reconnected to RetroAchievements."), "", g_RAImageID);
    }
    break;
  }
  case RC_NO_RESPONSE: {
    if (isInitialAttempt) {
      ...
      g_OSD.Show(
          ...
          di->T("Failed to connect to server, check your internet connection."),
          "", g_RAImageID);
    }
    break;
  }
  ...
  default: {
    ERROR_LOG(Log::Achievements,
              "Callback: Failure logging in via token: %d, %s", result,
              error_message);
    if (isInitialAttempt) {
      auto ac = GetI18NCategory(I18NCat::ACHIEVEMENTS);
      char message[512];
      ...
      g_OSD.Show(OSDType::MESSAGE_WARNING,
                 ac->T("Failed logging in to RetroAchievements"), message,
                 g_RAImageID);
    }
...
static void raintegration_get_game_name_handler(char *buffer,
...
static void
raintegration_event_handler(const rc_client_raintegration_event_t *event,
                            rc_client_t *client) {
  switch (event->type) {
  case RC_CLIENT_RAINTEGRATION_EVENT_MENUITEM_CHECKED_CHANGED:
    // The checked state of one of the menu items has changed and should be
    // reflected in the UI. Call the handy helper function if the menu was
    // created by rc_client_raintegration_rebuild_submenu.
    System_RunCallbackInWndProc(
        [](void *vhWnd, void *userdata) {
          auto menuItem =
              reinterpret_cast<const rc_client_raintegration_menu_item_t *>(
                  userdata);
  ...
  case RC_CLIENT_RAINTEGRATION_EVENT_HARDCORE_CHANGED:
    ...
    // through the use of the tools). The frontend doesn't necessarily need to
    // know that this value changed, they can still query it whenever it's
    // appropriate, but the event lets the frontend do things like
    // enable/disable rewind or cheats.
...
static void load_integration_callback(int result, const char *error_message,
                                      rc_client_t *client, void *userdata) {
  auto ac = GetI18NCategory(I18NCat::ACHIEVEMENTS);
  ...
  case RC_OK: {
    ...
    g_OSD.Show(
        OSDType::MESSAGE_SUCCESS,
        ApplySafeSubstitutions(ac->T("%1 loaded."), RAINTEGRATION_FILENAME));

    rc_client_raintegration_set_console_id(g_rcClient, RC_CONSOLE_PSP);
    rc_client_raintegration_set_event_handler(g_rcClient,
                                              &raintegration_event_handler);
    rc_client_raintegration_set_write_memory_function(
        g_rcClient, &raintegration_write_memory_handler);
    rc_client_raintegration_set_get_game_name_function(
        g_rcClient, &raintegration_get_game_name_handler);
  ...
  case RC_MISSING_VALUE:
    ...
    g_OSD.Show(OSDType::MESSAGE_WARNING,
               ac->T("RAIntegration is enabled, but %1 was not found."));
    break;
  case RC_ABORTED:
    // This is fine(-ish), proceeding to login.
    g_OSD.Show(
        OSDType::MESSAGE_WARNING,
        ApplySafeSubstitutions("Wrong version of %1?", RAINTEGRATION_FILENAME));
    break;
  default:
    g_OSD.Show(
        OSDType::MESSAGE_ERROR,
        StringFromFormat("RAIntegration init failed: %s", error_message));
    // Bailing.
    return;
  }

  // Things are ready to load a game. If the DLL was initialized, calling
  // rc_client_begin_load_game will be redirected through the DLL so the toolkit
  // has access to the game data. Similarly, things like
  // rc_create_leaderboard_list will be redirected through the DLL to reflect
  // any local changes made by the user.
...
void Initialize() {
  ...
  rc_client_enable_logging(g_rcClient, RC_CLIENT_LOG_LEVEL_VERBOSE,
  ...
  rc_client_set_hardcore_enabled(g_rcClient,
                                 g_Config.bAchievementsHardcoreMode ? 1 : 0);
  rc_client_set_encore_mode_enabled(g_rcClient,
                                    g_Config.bAchievementsEncoreMode ? 1 : 0);
  rc_client_set_unofficial_enabled(g_rcClient,
                                   g_Config.bAchievementsUnofficial ? 1 : 0);
  ...
  if (g_Config.bAchievementsEnableRAIntegration) {
    ...
    rc_client_begin_load_raintegration(g_rcClient, szFilePath, hWnd, "PPSSPP",
                                       PPSSPP_GIT_VERSION,
                                       &load_integration_callback, hWnd);
    return;
  }
#endif
  TryLoginByToken(true);
...
bool HasToken() { return !NativeLoadSecret(RA_TOKEN_SECRET_NAME).empty(); }
...
static void TryLoginByToken(bool isInitialAttempt) {
  ...
  if (!api_token.empty()) {
    ...
    rc_client_begin_login_with_token(
        g_rcClient, g_Config.sAchievementsUserName.c_str(), api_token.c_str(),
        &login_token_callback, (void *)isInitialAttempt);
  }
}

static void login_password_callback(int result, const char *error_message,
                                    rc_client_t *client, void *userdata) {
  auto di = GetI18NCategory(I18NCat::DIALOG);
  switch (result) {
  case RC_OK: {
  ...
  case RC_EXPIRED_TOKEN:
  default: {
    ERROR_LOG(Log::Achievements, "Failure logging in via password: %d, %s",
              result, error_message);
    g_OSD.Show(OSDType::MESSAGE_WARNING,
               di->T("Failed to log in, check your username and password."), "",
               g_RAImageID);
    OnAchievementsLoginStateChange();
    break;
  }
  ...
  g_loginResult =
      RC_OK; // For these, we don't want the "permanence" of the login-by-token
             // failure, this prevents LoginProblems from returning true.
  g_isLoggingIn = false;
}
...
bool LoginAsync(const char *username, const char *password) {
  ...
  rc_client_begin_login_with_password(g_rcClient, username, password,
                                      &login_password_callback, nullptr);
  return true;
}
...
void Idle() {
  ...
  if (g_Config.bAchievementsEnable && GetUIState() == UISTATE_MENU &&
      now > g_lastLoginAttemptTime + LOGIN_ATTEMPT_INTERVAL_S) {
    g_lastLoginAttemptTime = now;
    if (g_rcClient && IsLoggedIn()) {
      return; // All good.
    }
    if (g_Config.sAchievementsUserName.empty() || g_isLoggingIn ||
        !HasToken()) {
      // Didn't try to login yet or is in the process of logging in. Also OK.
      return;
    }

    // In this situation, there's a token, but we're not logged in. Probably
    // disrupted internet connection during startup. Let's make an attempt.
...
void DoState(PointerWrap &p) {
  ...
  if (!sw) {
    ...
    if (HasAchievementsOrLeaderboards()) {
      auto ac = GetI18NCategory(I18NCat::ACHIEVEMENTS);
      g_OSD.Show(OSDType::MESSAGE_WARNING,
                 ac->T("Save state loaded without achievement data"), "",
                 g_RAImageID, 5.0f, "");
    }
    rc_client_reset(g_rcClient);
    return;
  ...
  if (!IsActive()) {
    ...
    if (p.mode == PointerWrap::MODE_READ) {
      WARN_LOG(Log::Achievements, "Save state contained achievement data, but "
                                  "achievements are not active. Ignore.");
    }
    p.SkipBytes(data_size);
    return;
  }

  if (p.mode == PointerWrap::MODE_MEASURE ||
      p.mode == PointerWrap::MODE_WRITE || p.mode == PointerWrap::MODE_VERIFY ||
      p.mode == PointerWrap::MODE_NOOP) {
    data_size =
        (uint32_t)(g_rcClient ? rc_client_progress_size(g_rcClient) : 0);
  }
  Do(p, data_size);
  ...
  if (data_size > 0) {
    ...
    case PointerWrap::MODE_VERIFY: {
      ...
      if (retval != RC_OK) {
        ERROR_LOG(Log::Achievements,
                  "Error %d serializing achievement data. Ignoring.", retval);
      }
      break;
    }
    default:
    ...
    case PointerWrap::MODE_READ: {
      ...
      if (retval != RC_OK) {
        ...
        ERROR_LOG(Log::Achievements,
                  "Error %d deserializing achievement data. Ignoring.", retval);
      }
      break;
    }
    default:
...
void DownloadImageIfMissing(const std::string &cache_key,
                            std::string_view url) {
  if (g_iconCache.MarkPending(cache_key)) {
    INFO_LOG(Log::Achievements, "Downloading image: %.*s (%s)", (int)url.size(),
             url.data(), cache_key.c_str());
    g_DownloadManager.StartDownloadWithCallback(
        url, Path(), http::RequestFlags::Default,
        [cache_key](http::Request &download) {
...
Statistics GetStatistics() { return g_stats; }
...
std::string GetGameAchievementSummary() {
  ...
  if (summary.num_core_achievements + summary.num_unofficial_achievements ==
      0) {
    summaryString = ac->T("This game has no achievements");
  } else {
    summaryString = ApplySafeSubstitutions(
        ac->T(
            "Earned",
            "You have unlocked %1 of %2 achievements, earning %3 of %4 points"),
...
void ShowNotLoggedInMessage() {
  ...
  g_OSD.Show(OSDType::MESSAGE_ERROR,
             ac->T("Failed to connect to RetroAchievements. Achievements will "
                   "not unlock."),
             "", g_RAImageID, 6.0f);
}

void identify_and_load_callback(int result, const char *error_message,
                                rc_client_t *client, void *userdata) {
  auto ac = GetI18NCategory(I18NCat::ACHIEVEMENTS);

  NOTICE_LOG(Log::Achievements, "Load callback: %d (%s)", result,
             error_message);
  ...
  case RC_OK: {
    ...
    g_OSD.Show(OSDType::MESSAGE_INFO, title, GetGameAchievementSummary(),
               cacheId, 5.0f);
    break;
  }
  case RC_NO_GAME_LOADED: {
    ...
    g_OSD.Show(OSDType::MESSAGE_INFO, title,
               ac->T("RetroAchievements are not available for this game"),
               g_RAImageID, 3.0f);
    break;
  }
  case RC_NO_RESPONSE:
    // We lost the internet connection at some point and can't report
    // achievements.
    ShowNotLoggedInMessage();
    break;
  default:
    // Other various errors.
    ERROR_LOG(Log::Achievements, "Failed to identify/load game: %d (%s)",
              result, error_message);
    g_OSD.Show(OSDType::MESSAGE_ERROR,
               ac->T("Failed to identify game. Achievements will not unlock."),
               "", g_RAImageID, 6.0f);
    break;
  }
...
void SetGame(const Path &path, IdentifiedFileType fileType,
  ...
  default:
    ...
    WARN_LOG(Log::Achievements,
             "File type of '%s' is not yet compatible with RetroAchievements",
             path.c_str());
    return;
  }

  if (g_isLoggingIn) {
    // IsReadyToStart should have been checked the same frame, so we shouldn't
    // be here. Maybe there's a race condition possible, but don't think so.
    ERROR_LOG(Log::Achievements,
              "Still logging in during SetGame - shouldn't happen");
  }
  ...
  // The caller should hold off on executing game code until this turns false,
  // checking with IsBlockingExecution()
  g_gamePath = path;
  ...
  if (homebrew) {
    ...
    // ISO hashing method.
    //
    // TODO: Fish the block device out of the loading process somewhere else.
    // Though, probably easier to just do it here, we need a temporary
    // blockdevice anyway since it gets consumed by ComputePSPISOHash.
    std::string errorString;
    BlockDevice *blockDevice(ConstructBlockDevice(fileLoader, &errorString));
    if (!blockDevice) {
      ERROR_LOG(
          Log::Achievements,
          "Failed to construct block device for '%s' - can't identify: %s",
          path.c_str(), errorString.c_str());
      g_isIdentifying = false;
      return;
    }
  ...
  rc_client_begin_load_game(g_rcClient, s_game_hash.c_str(),
                            &identify_and_load_callback, nullptr);
}
...
void change_media_callback(int result, const char *error_message,
                           rc_client_t *client, void *userdata) {
  auto ac = GetI18NCategory(I18NCat::ACHIEVEMENTS);
  NOTICE_LOG(Log::Achievements, "Change media callback: %d (%s)", result,
             error_message);
  g_isIdentifying = false;
...
void ChangeUMD(const Path &path, FileLoader *fileLoader) {
  ...
  rc_client_begin_change_media_from_hash(g_rcClient, s_game_hash.c_str(),
                                         &change_media_callback, nullptr);
}

std::set<uint32_t> GetActiveChallengeIDs() { return g_activeChallenges; }

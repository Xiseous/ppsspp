...
// Updated by Rescan. This is to avoid calling Exists a lot of times, which
// could be slow on some Android devices when we allow many save states.
static double g_lastRescan = 0.0;
static std::map<std::string, uint64_t, std::less<>>
    g_files; // the value is the mod-time.
...
struct Operation {
  // The slot number is for visual purposes only. Set to -1 for operations where
  // we don't display a message for example.
...
// Should only fail if the game hasn't created a framebuffer yet. Some games
// play video without a framebuffer, we should probably just read back memory
// then instead (GTA for example). But this is a really unimportant edge case,
...
void SaveStart::DoState(PointerWrap &p) {
  ...
  if (s >= 2) {
    ...
    // This saves the first git version to create this save state (or generation
    // of save states.)
  ...
  // Gotta do CoreTiming before HLE, but from v3 we've moved it after the memory
  // stuff.
  if (s <= 2) {
  ...
  // Don't bother restoring if reading, we'll deal with that in
  // KernelModuleDoState. In theory, different functions might have been runtime
  // loaded in the state.
...
#endif
  // Kernel object destructors might close open files, so do the filesystem
  // last.
...
void Enqueue(const SaveState::Operation &op) {
  ...
  if (false) {
    ...
    if (g_Config.bAchievementsSaveStateInHardcoreMode &&
        ((op.type == SaveState::OperationType::Save) ||
         (op.type == OperationType::SaveScreenshot))) {
...
void Load(const Path &filename, int slot, Callback callback) {
  ...
  if (coreState == CoreState::CORE_RUNTIME_ERROR)
    Core_Break(CoreBreakReason::SavestateLoad, 0);
...
void Save(const Path &filename, int slot, Callback callback) {
  ...
  rewindStates.NotifyState();
  if (coreState == CoreState::CORE_RUNTIME_ERROR)
    Core_Break(CoreBreakReason::SavestateSave, 0);
...
void Rewind(Callback callback) {
  ...
  if (coreState == CoreState::CORE_RUNTIME_ERROR)
    Core_Break(CoreBreakReason::SavestateRewind, 0);
...
bool CanRewind() { return !rewindStates.Empty(); }

// Slot utilities

std::string AppendSlotTitle(const std::string &filename,
  ...
  auto detectSlot = [&](const std::string &ext) {
    ...
    // If we take out the extension, '_', etc. we should be left with only a
    // single digit.
...
std::string GetTitle(const Path &filename) {
  std::string title;
  if (CChunkFileReader::GetFileTitle(filename, &title) ==
...
// This means that this must always be called in-game, otherwise it will not
// work!
//
...
// The prefix is always based on GenerateFullDiscId. So we can find these by
// scanning, too.
std::string GenerateSaveSlotFilename(std::string_view gamePrefix, int slot,
                                     const char *extension) {
  return StringFromFormat("%.*s_%d.%s", STR_VIEW(gamePrefix), slot, extension);
}

Path GenerateSaveSlotPath(std::string_view gamePrefix, int slot,
                          const char *extension) {
...
int GetCurrentSlot() { return g_Config.iCurrentStateSlot; }

void PrevSlot() {
  g_Config.iCurrentStateSlot =
      (g_Config.iCurrentStateSlot - 1 + g_Config.iSaveStateSlotCount) %
  ...
  if (!fn.empty()) {
    ...
    if (g_Config.bEnableStateUndo) {
      ...
      auto saveCallback = [=](Status status, std::string_view message) {
        if (status != Status::FAILURE) {
          ...
          ERROR_LOG(Log::SaveState, "Saving load undo state failed: %.*s",
      ...
      if (!backup.empty()) {
        ...
        ERROR_LOG(Log::SaveState,
                  "Saving load undo state failed. Error in the file system.");
        Load(fn, slot, callback);
      }
    } else {
      Load(fn, slot, callback);
    ...
    if (callback) {
      ...
      callback(Status::FAILURE,
               sy->T("Failed to load state. Error in the file system."));
    }
  }
}
...
bool UndoLoad(std::string_view gamePrefix, Callback callback) {
  ...
  if (g_Config.sStateLoadUndoGame != gamePrefix) {
    if (callback) {
      auto sy = GetI18NCategory(I18NCat::SYSTEM);
      callback(Status::FAILURE,
               sy->T("Error: load undo state is from a different game"));
    }
    return false;
  }
...
void SaveSlot(std::string_view gamePrefix, int slot, Callback callback) {
  ...
  if (!fn.empty()) {
    ...
    if (g_Config.bEnableStateUndo) {
      Path shotUndo =
          GenerateSaveSlotPath(gamePrefix, slot, UNDO_SCREENSHOT_EXTENSION);
    ...
    if (callback) {
      auto sy = GetI18NCategory(I18NCat::SYSTEM);
      callback(Status::FAILURE,
               sy->T("Failed to save state. Error in the file system."));
    }
  }
  Rescan(gamePrefix);
}
...
void Rescan(std::string_view gamePrefix) {
  ...
  if (!File::GetFilesInDir(GetSysDirectory(DIRECTORY_SAVESTATE), &file, nullptr,
                           0, prefix)) {
...
static bool SaveStateFileExists(std::string_view gamePrefix, int slot,
                                const char *extension) {
  return g_files.find(GenerateSaveSlotFilename(gamePrefix, slot, extension)) !=
         g_files.end();
}
...
bool operator!(const tm &t1) {
  if (t1.tm_year || t1.tm_mon || t1.tm_mday || t1.tm_hour || t1.tm_min ||
      t1.tm_sec)
    return false;
  return true;
}
...
int GetNewestSlot(std::string_view gamePrefix) {
  ...
  for (int i = 0; i < g_Config.iSaveStateSlotCount; i++) {
    auto iter =
        g_files.find(GenerateSaveSlotFilename(gamePrefix, i, STATE_EXTENSION));
    if (iter != g_files.end()) {
...
bool HandleLoadFailure(bool wasRewinding) {
  if (wasRewinding) {
    ...
    // Okay, first, let's give the next rewind state a shot - maybe we can at
    // least not reset entirely. Actually, this seems like a bad thing - some
    // systems can end up in bad states, like sceFont :(
...
bool HasLoadedState() { return hasLoadedState; }
...
static Status TriggerLoadWarnings(std::string &callbackMessage) {
  ...
  if (IsStale()) {
    ...
    callbackMessage =
        sc->T("Loaded. Save in game, restart, and load for less bugs.");
    return Status::SUCCESS;
  }
  if (IsOldVersion()) {
  ...
  // If the loaded state (saveDataGeneration) is older, the game may prevent
  // saving again. This can happen with newer too, but ignore to/from 0 as a
  // common likely safe case.
  if (saveDataGeneration != lastSaveDataGeneration && saveDataGeneration != 0 &&
      lastSaveDataGeneration != 0) {
    if (saveDataGeneration < lastSaveDataGeneration)
      callbackMessage =
          sc->T("Loaded. Game may refuse to save over newer savedata.");
    else
      callbackMessage =
          sc->T("Loaded. Game may refuse to save over different savedata.");
    return Status::WARNING;
  }
  return Status::SUCCESS;
}

// NOTE: This can cause ending of the current renderpass, due to the readback
// needed for the screenshot.
// TODO: This should run the actual operations on a thread. While this returns
// true (for example), emulation *must* not run further, in order not to disturb
...
bool Process() {
  ...
  if (!__KernelIsRunning()) {
    ERROR_LOG(Log::SaveState, "Savestate failure: Unable to load without "
  ...
  for (const auto &op : operations) {
    ...
    std::string slot_prefix =
        op.slot >= 0 ? StringFromFormat("(%d) ", op.slot + 1) : "";
    std::string errorString;

    switch (op.type) {
    case OperationType::Load:
      INFO_LOG(Log::SaveState, "Loading state from '%s'", op.filename.c_str());
      // Use the state's latest version as a guess for
      // saveStateInitialGitVersion.
      result = CChunkFileReader::Load(op.filename, &saveStateInitialGitVersion,
                                      state, &errorString);
      if (result == CChunkFileReader::ERROR_NONE) {
        callbackMessage = op.slot != LOAD_UNDO_SLOT
                              ? sc->T("Loaded State")
                              : sc->T("State load undone");
        ...
        if (g_Config.bSaveLoadResetsAVdumping) {
          if (g_Config.bDumpFrames) {
            AVIDump::Stop();
            AVIDump::Start(PSP_CoreParameter().renderWidth,
                           PSP_CoreParameter().renderHeight);
          }
        ...
        ERROR_LOG(Log::SaveState, "Load state failure: %s",
                  errorString.c_str());
    ...
    case OperationType::Save:
      ...
      result =
          CChunkFileReader::Save(op.filename, title, PPSSPP_GIT_VERSION, state);
      if (result == CChunkFileReader::ERROR_NONE) {
        ...
        // TODO: What else might we want to do here? This should be very
        // unusual.
    ...
    case OperationType::Verify: {
      int tempResult =
          CChunkFileReader::Verify(state) == CChunkFileReader::ERROR_NONE;
    ...
    case OperationType::Rewind:
      ...
      if (result == CChunkFileReader::ERROR_NONE) {
        ...
        // Cripes.  Good news is, we might have more.  Let's try those too,
        // better than a reset.
    ...
    case OperationType::SaveScreenshot: {
      ...
      ScreenshotResult tempResult = TakeGameScreenshot(
          nullptr, op.filename, ScreenshotFormat::JPG, SCREENSHOT_DISPLAY,
          maxResMultiplier, [](bool success) {
            if (success) {
              screenshotFailures = 0;
            }
          });
      ...
      case ScreenshotResult::ScreenshotNotPossible:
        ...
        callbackResult = Status::FAILURE;
        WARN_LOG(Log::SaveState,
                 "Failed to take a screenshot for the savestate! (%s) The "
                 "savestate will lack an icon.",
                 op.filename.c_str());
        if (coreState != CORE_STEPPING_CPU &&
            screenshotFailures++ < SCREENSHOT_FAILURE_RETRIES) {
          // Requeue for next frame (if we were stepping, no point, will just
          // spam errors quickly).
    ...
    default:
      ERROR_LOG(Log::SaveState, "Savestate failure: unknown operation type %d",
                op.type);
      callbackResult = Status::FAILURE;
      break;
    }

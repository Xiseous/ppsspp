...
BootState PSP_GetBootState() { return g_bootState; }

FileLoader *PSP_LoadedFile() { return g_loadedFile; }

void ResetUIState() { globalUIState = UISTATE_MENU; }
...
GlobalUIState GetUIState() { return globalUIState; }
...
GPUBackend GetGPUBackend() { return gpuBackend; }

std::string GetGPUBackendDevice() { return gpuBackendDevice; }
...
static bool LoadSymbolsIfSupported() {
  if (System_GetPropertyBool(SYSPROP_HAS_DEBUGGER)) {
    ...
    bool result1 = g_symbolMap->LoadSymbolMap(
        SymbolMapFilename(PSP_CoreParameter().fileToStart, ".ppmap"));
    // Load the old-style map file.
    if (!result1)
      result1 = g_symbolMap->LoadSymbolMap(
          SymbolMapFilename(PSP_CoreParameter().fileToStart, ".map"));
    bool result2 = g_symbolMap->LoadNocashSym(
        SymbolMapFilename(PSP_CoreParameter().fileToStart, ".sym"));
...
static bool SaveSymbolMapIfSupported() {
  if (g_symbolMap) {
    return g_symbolMap->SaveSymbolMap(
        SymbolMapFilename(PSP_CoreParameter().fileToStart, ".ppmap"));
  }
  return false;
}

bool DiscIDFromGEDumpPath(const Path &path, FileLoader *fileLoader,
                          std::string *id) {
  ...
  if (fileLoader->ReadAt(0, sizeof(header), &header) == sizeof(header)) {
    const bool magicMatch =
        memcmp(header.magic, HEADER_MAGIC, sizeof(header.magic)) == 0;
  ...
  if (filename.size() > 10 && (filename[0] == 'U' || filename[0] == 'N') &&
      filename[9] == '_') {
...
static void GetBootError(IdentifiedFileType type, std::string *errorString) {
  ...
  default:
    *errorString =
        StringFromFormat("Unhandled identified file type %d", (int)type);
    break;
  }
}
...
static void ShowCompatWarnings(const Compatibility &compat) {
  ...
  if (compat.flags().RequireBufferedRendering && g_Config.bSkipBufferEffects &&
      !g_Config.bSoftwareRendering) {
    auto gr = GetI18NCategory(I18NCat::GRAPHICS);
    g_OSD.Show(OSDType::MESSAGE_WARNING,
               gr->T("BufferedRenderingRequired",
                     "Warning: This game requires Rendering Mode to be set to "
                     "Buffered."),
               10.0f);
  }

  if (compat.flags().RequireBlockTransfer &&
    ...
    g_OSD.Show(
        ...
        gr->T("BlockTransferRequired",
              "Warning: This game requires Skip GPU Readbacks be set to No."),
        10.0f);
  }
  ...
  if (compat.flags().RequireDefaultCPUClock && g_Config.iLockedCPUSpeed != 0) {
    ...
    g_OSD.Show(
        ...
        gr->T(
            ...
            "Warning: This game requires the CPU clock to be set to default."),
        10.0f);
  }
}
...
// NOTE: The loader has already been fully resolved (ResolveFileLoaderTarget)
// and identified here.
static bool CPU_Init(FileLoader *fileLoader, IdentifiedFileType type,
  ...
  case IdentifiedFileType::PSP_DISC_DIRECTORY:
    ...
    if (type == IdentifiedFileType::PSP_DISC_DIRECTORY) {
      // Check for existence of ppsspp-index.lst - if it exists, the user likely
      ...
      if (!File::Exists(g_CoreParameter.fileToStart / INDEX_FILENAME)) {
        ...
        g_OSD.Show(
            OSDType::MESSAGE_WARNING,
            sc->T("ExtractedIsoWarning", "Extracted ISOs often don't "
                                         "work.\nPlay the ISO file directly."),
            g_CoreParameter.fileToStart.ToVisualString(), 7.0f);
      } else {
        INFO_LOG(Log::Loader,
                 "Extracted ISO loaded without warning - %s is present.",
  ...
  case IdentifiedFileType::PPSSPP_GE_DUMP:
    ...
    if (DiscIDFromGEDumpPath(g_CoreParameter.fileToStart, fileLoader,
                             &geDumpDiscID)) {
    ...
    break;
  default: {
    // Trying to boot other things lands us here. We need to return a sensible
    // error string.
    ERROR_LOG(Log::Loader, "CPU_Init didn't recognize file. %s",
              errorString->c_str());
  ...
  // Here we have read the PARAM.SFO, let's see if we need any compatibility
  // overrides. Homebrew get fake disc IDs assigned to the global paramSFO, so
  // they shouldn't clash with real games.
  g_CoreParameter.compat.Load(g_paramSFO.GetDiscID());
  ShowCompatWarnings(g_CoreParameter.compat);

  // This must be before Memory::Init because plugins can override the memory
  // size.
  if (type != IdentifiedFileType::PPSSPP_GE_DUMP) {
    HLEPlugins::Init();
  }

  // Initialize the memory map as early as possible (now that we've read the
  // PARAM.SFO).
  if (!Memory::Init()) {
    // We're screwed.
  ...
  g_logManager.EnableOutput(LogOutput::File, g_Config.bEnableFileLogging ||
                                                 g_fileLoggingWasEnabled);

  if ((g_logManager.GetOutputsEnabled() & LogOutput::File) &&
      !g_logManager.GetLogFilePath().empty()) {
    ...
    g_OSD.Show(
        ...
        ApplySafeSubstitutions("%1: %2", dev->T("Log to file"),
                               g_logManager.GetLogFilePath().ToVisualString()),
        0.0f, "log_to_file");
    if (System_GetPropertyBool(SYSPROP_CAN_SHOW_FILE)) {
      g_OSD.SetClickCallback(
          "log_to_file",
          [](bool clicked, void *userdata) {
  ...
  if (!g_CoreParameter.mountIso.empty()) {
    g_CoreParameter.mountIsoLoader =
        ConstructFileLoader(g_CoreParameter.mountIso);
  }

  // Game-specific settings are load from for example Load_PSP_ISO (which calls
  // g_Config.LoadGameConfig). We can't do things that depend on these before
  // the below switch. So for example, the adjustment of the GPU core to
  // software has now been moved below it.

  // If they shut down early, we'll catch it when load completes.
  // Note: this may return before init is complete, which is checked if
  // CPU_IsReady().
  ...
  case IdentifiedFileType::PSP_PBP:
  case IdentifiedFileType::PSP_ELF: {
    INFO_LOG(Log::Loader, "File is an ELF or loose PBP %s",
             fileLoader->GetPath().c_str());
    if (!Load_PSP_ELF_PBP(fileLoader, errorString)) {
      ERROR_LOG(Log::Loader, "Failed to load ELF or loose PBP: %s",
                errorString->c_str());
      return false;
    }
    break;
  ...
  case IdentifiedFileType::PSP_ISO:
  case IdentifiedFileType::PSP_ISO_NP:
  case IdentifiedFileType::PSP_DISC_DIRECTORY: // behaves the same as the
                                               // mounting is already done by
                                               // now
  ...
  if (g_Config.bSoftwareRendering ||
      g_CoreParameter.compat.flags().ForceSoftwareRenderer) {
...
static void InitGPU(std::string *error_string) {
  if (!gpu) { // should be!
    ...
    Draw::DrawContext *draw =
        g_CoreParameter.graphicsContext
            ? g_CoreParameter.graphicsContext->GetDrawContext()
            : nullptr;
...
bool PSP_InitStart(const CoreParameter &coreParam) {
  ...
  _assert_msg_(!g_loadingThread.joinable(), "%s",
  ...
  g_loadingThread = std::thread([error_string]() {
    ...
    FileLoader *loadedFile =
        ResolveFileLoaderTarget(ConstructFileLoader(filename));

    IdentifiedFileType type =
        Identify_File(loadedFile, &g_CoreParameter.errorString);
    g_CoreParameter.fileType = type;
    ...
    if (System_GetPropertyBool(SYSPROP_ENOUGH_RAM_FOR_FULL_ISO)) {
      if (g_Config.bCacheFullIsoInRam) {
        ...
        default:
          INFO_LOG(Log::Loader,
                   "RAM caching is on, but file is not an ISO, so ignoring");
          break;
        }
      }
    }

    // TODO: The reason we pass in g_CoreParameter.errorString here is that it's
    // persistent - it gets written to from the loader thread that gets spawned.
    ...
    // Initialize the GPU as far as we can here (do things like load cache
    // files).
    _dbg_assert_(!gpu);
#ifndef __LIBRETRO__
...
BootState PSP_InitUpdate(std::string *error_string) {
  ...
  if (bootState == BootState::Booting || bootState == BootState::Off) {
    ...
    _dbg_assert_(bootState == BootState::Booting ||
                 !g_loadingThread.joinable());
    return bootState;
  }
  ...
  // Ok, async part of the boot completed, let's finish up things on the main
  // thread.
  if (gpu) {
...
// Most platforms should not use this one, they should call PSP_InitStart and
// then do their thing while repeatedly calling PSP_InitUpdate. This is
// basically just for libretro convenience.
...
void PSP_RunLoopWhileState() {
  // We just run the CPU until we get to vblank. This will quickly sync up
  // pretty nicely. The actual number of cycles doesn't matter so much here as
  // we will break due to CORE_NEXTFRAME, most of the time hopefully...
...
void DumpFileIfEnabled(const u8 *dataPtr, const u32 length,
                       std::string_view name, DumpFileType type) {
  if (!(g_Config.iDumpFileTypes & (int)type)) {
    return;
  }
  if (!dataPtr) {
    ERROR_LOG(Log::Loader, "Error dumping %s: invalid pointer",
              DumpFileTypeToString(DumpFileType::EBOOT));
    return;
  }
  if (length == 0) {
    ERROR_LOG(Log::Loader, "Error dumping %s: invalid length",
              DumpFileTypeToString(DumpFileType::EBOOT));
    return;
  }

  const char *extension = DumpFileTypeToFileExtension(type);
  std::string filenameToDumpTo =
      g_paramSFO.GetDiscID() + "_" + std::string(name);
  ...
  if (File::Exists(fullPath)) {
    INFO_LOG(Log::sceModule, "%s already exists for this game, skipping dump.",
             filenameToDumpTo.c_str());

    char *path = new char[strlen(fullPath.c_str()) + 1];
    strcpy(path, fullPath.c_str());

    g_OSD.Show(OSDType::MESSAGE_INFO, titleStr, fullPath.ToVisualString(),
               5.0f);
    if (System_GetPropertyBool(SYSPROP_CAN_SHOW_FILE)) {
      g_OSD.SetClickCallback(
          "file_dumped",
  ...
  if (!File::Exists(dumpDirectory)) {
    if (!File::CreateDir(dumpDirectory)) {
      ERROR_LOG(Log::sceModule,
                "Unable to create directory for EBOOT dumping, aborting.");
      return;
    }
  }
  ...
  INFO_LOG(Log::sceModule, "Successfully wrote %s to %s",
           DumpFileTypeToString(type), fullPath.ToVisualString().c_str());

  char *path = new char[strlen(fullPath.c_str()) + 1];
  strcpy(path, fullPath.c_str());

  // Re-suing the translation string here.
  g_OSD.Show(OSDType::MESSAGE_SUCCESS, titleStr, fullPath.ToVisualString(),
             5.0f);
  if (System_GetPropertyBool(SYSPROP_CAN_SHOW_FILE)) {
    g_OSD.SetClickCallback(
        "decr",

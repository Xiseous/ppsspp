...
#include <switch.h> // For nwindowGetDefault()
#endif
...
EGLConfig EGL_FindConfig(int *contextVersion) {
  ...
  for (const EGLConfig &config : configs) {
    ...
    int colorScore = readConfigMax(EGL_RED_SIZE, 8) +
                     readConfigMax(EGL_BLUE_SIZE, 8) +
                     readConfigMax(EGL_GREEN_SIZE, 8);
    ...
    int transparentScore =
        readConfig(EGL_TRANSPARENT_TYPE) == EGL_NONE ? 50 : 0;

    EGLint caveat = readConfig(EGL_CONFIG_CAVEAT);
    // Let's assume that non-conformant configs aren't so awful.
    int caveatScore = caveat == EGL_NONE
                          ? 100
                          : (caveat == EGL_NON_CONFORMANT_CONFIG ? 95 : 0);

#ifndef USING_FBDEV
    EGLint surfaceType = readConfig(EGL_SURFACE_TYPE);
    // Only try a non-Window config in the worst case when there are only
    // non-Window configs.
...
#ifdef USING_GLES2
    int renderableScoreGLES =
        renderableGLES3 ? 100 : (renderableGLES2 ? 80 : 0);
...
int8_t EGL_Init(SDL_Window *window) {
  ...
  g_eglContext =
      eglCreateContext(g_eglDisplay, eglConfig, nullptr, contextAttributes);
...
bool SDLGLGraphicsContext::InitFromRenderThread(std::string *errorMessage) {
  ...
  // HACK: Ensure that the swap interval is set after context creation (needed
  // for kmsdrm)
...
int SDLGLGraphicsContext::Init(SDL_Window *&window, int x, int y, int w, int h,
  ...
  for (size_t i = 0; i < ARRAY_SIZE(attemptVersions); ++i) {
    ...
    // Let's keep trying.  To be safe, destroy the window - docs say needed to
    // change profile. in practice, it doesn't seem to matter, but maybe it
    // differs by platform.

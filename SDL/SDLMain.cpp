...
static void InitSDLAudioDevice(const std::string &name = "") {
  ...
  if (deviceCount > 0) {
    ...
    INFO_LOG(Log::Audio, "Failed to list audio devices: retval=%d",
             deviceCount);
  }
  ...
  if (!startDevice.empty()) {
    ...
    audioDev = SDL_OpenAudioDevice(startDevice.c_str(), 0, &fmt, &g_retFmt,
                                   SDL_AUDIO_ALLOW_FREQUENCY_CHANGE);
    if (audioDev <= 0) {
      WARN_LOG(Log::Audio, "Failed to open audio device '%s'",
               startDevice.c_str());
    }
  }
  if (audioDev <= 0) {
    if (audioDev < 0) {
      WARN_LOG(Log::Audio, "SDL: Error: '%s'. Trying the default audio device",
               SDL_GetError());
    } else {
      INFO_LOG(Log::Audio, "Opening default audio device");
    }
    audioDev = SDL_OpenAudioDevice(nullptr, 0, &fmt, &g_retFmt,
                                   SDL_AUDIO_ALLOW_FREQUENCY_CHANGE);
  }
  if (audioDev <= 0) {
    ERROR_LOG(Log::Audio,
              "Failed to open audio device '%s', second try. Giving up.",
              SDL_GetError());
  } else {
    if (g_retFmt.samples != fmt.samples) // Notify, but still use it
      ERROR_LOG(Log::Audio, "Output audio samples: %d (requested: %d)",
                g_retFmt.samples, fmt.samples);
    if (g_retFmt.format != fmt.format || g_retFmt.channels != fmt.channels) {
      ERROR_LOG(Log::Audio,
                "Sound buffer format does not match requested format.");
      ERROR_LOG(Log::Audio, "Output audio freq: %d (requested: %d)",
                g_retFmt.freq, fmt.freq);
      ERROR_LOG(Log::Audio, "Output audio format: %d (requested: %d)",
                g_retFmt.format, fmt.format);
      ERROR_LOG(Log::Audio, "Output audio channels: %d (requested: %d)",
                g_retFmt.channels, fmt.channels);
      ERROR_LOG(Log::Audio, "Provided output format does not match "
                            "requirement, turning audio off");
...
static void InitializeFilters(std::vector<std::string> &filters,
                              BrowseFileType type) {
  switch (type) {
  case BrowseFileType::BOOTABLE:
    filters.push_back("All supported file types (*.iso *.cso *.chd *.pbp *.elf "
                      "*.prx *.zip *.ppdmp)");
...
bool System_MakeRequest(SystemRequestType type, int requestId,
  ...
  case SystemRequestType::BROWSE_FOR_FILE: {
    DarwinDirectoryPanelCallback callback = [requestId](bool success,
                                                        Path path) {
      if (success) {
    ...
    DarwinFileSystemServices::presentDirectoryPanel(
        callback, /* allowFiles = */ true, /* allowDirectories = */ false,
        fileType);
    return true;
  }
  ...
  case SystemRequestType::BROWSE_FOR_FOLDER: {
    ...
    DarwinFileSystemServices::presentDirectoryPanel(
        callback, /* allowFiles = */ false, /* allowDirectories = */ true);
    return true;
  }
  ...
  case SystemRequestType::BROWSE_FOR_IMAGE: {
    ...
    std::vector<std::string> result =
        pfd::open_file(title, "", filters).result();
    if (!result.empty()) {
  ...
  case SystemRequestType::BROWSE_FOR_FILE_SAVE: {
    ...
    if (type == SystemRequestType::BROWSE_FOR_FILE) {
      std::vector<std::string> result =
          pfd::open_file(title, initialFilename, filters).result();
      if (!result.empty()) {
        g_requestManager.PostSystemSuccess(requestId, result[0]);
      } else {
        g_requestManager.PostSystemFailure(requestId);
      ...
      std::string result =
          pfd::save_file(title, initialFilename, filters).result();
      if (!result.empty()) {
  ...
  case SystemRequestType::SHOW_FILE_IN_FOLDER: {
    ...
    HRESULT hr = SHParseDisplayName(
        ConvertUTF8ToWString(ReplaceAll(path, "/", "\\")).c_str(), nullptr,
        &pidl, 0, &flags);
    if (pidl) {
      if (SUCCEEDED(hr))
...
void System_LaunchUrl(LaunchUrlType urlType, std::string_view url) {
  ...
  case LaunchUrlType::MARKET_URL: {
    ...
    INFO_LOG(Log::System,
             "Would have gone to %.*s but LaunchBrowser is not implemented on "
             "this platform",
             STR_VIEW(url));
#elif defined(_WIN32)
    ...
    if (err) {
      INFO_LOG(
          Log::System,
          "Would have gone to %.*s but xdg-utils seems not to be installed",
          STR_VIEW(url));
    }
#endif
    break;
  ...
  case LaunchUrlType::EMAIL_ADDRESS: {
    ...
    INFO_LOG(Log::System,
             "Would have opened your email client for %.*s but LaunchEmail is "
...
#if PPSSPP_PLATFORM(MAC)
    // Let's keep using the old code on Mac for safety. Evaluate later if to be
    // deleted.
...
#else
#if PPSSPP_PLATFORM(SWITCH)
    return false; // No file dialogs on Switch
#else
...
struct InputStateTracker {
  void MouseCaptureControl() {
    bool captureMouseCondition =
        g_Config.bMouseControl &&
        ((GetUIState() == UISTATE_INGAME && g_Config.bMouseConfine) ||
         g_IsMappingMouseInput);
...
static void ProcessSDLEvent(SDL_Window *window, const SDL_Event &event,
  ...
  case SDL_WINDOWEVENT:
    ...
    case SDL_WINDOWEVENT_SIZE_CHANGED: // better than RESIZED, more general
    {
      ...
      if (lastUIState == UISTATE_INGAME && fullscreen &&
  ...
  case SDL_KEYDOWN: {
    ...
    if (mapped == KeyMapRawSDLtoNative.end() ||
        mapped->second == NKCODE_UNKNOWN) {
      break;
    }
...
// This behavior doesn't feel right on a macbook with a touchpad.
  ...
  case SDL_MOUSEBUTTONDOWN:
    ...
    case SDL_BUTTON_LEFT: {
      ...
      input.id = 0;
      NativeTouch(input);
      KeyInput key(DEVICE_ID_MOUSE, NKCODE_EXT_MOUSEBUTTON_1,
                   KeyInputFlags::DOWN);
      NativeKey(key);
    } break;
    case SDL_BUTTON_RIGHT: {
      ...
      input.id = 0;
      NativeTouch(input);
      KeyInput key(DEVICE_ID_MOUSE, NKCODE_EXT_MOUSEBUTTON_2,
                   KeyInputFlags::DOWN);
      NativeKey(key);
    } break;
    case SDL_BUTTON_MIDDLE: {
      KeyInput key(DEVICE_ID_MOUSE, NKCODE_EXT_MOUSEBUTTON_3,
                   KeyInputFlags::DOWN);
      NativeKey(key);
    } break;
    case SDL_BUTTON_X1: {
      KeyInput key(DEVICE_ID_MOUSE, NKCODE_EXT_MOUSEBUTTON_4,
                   KeyInputFlags::DOWN);
      NativeKey(key);
    } break;
    case SDL_BUTTON_X2: {
      KeyInput key(DEVICE_ID_MOUSE, NKCODE_EXT_MOUSEBUTTON_5,
                   KeyInputFlags::DOWN);
      NativeKey(key);
    } break;
    }
  ...
  case SDL_MOUSEWHEEL: {
    ...
    if (event.wheel.preciseY != 0.0f) {
      ...
      key.keyCode = event.wheel.preciseY > 0 ? NKCODE_EXT_MOUSEWHEEL_UP
  ...
  case SDL_AUDIODEVICEADDED:
    ...
    if (event.adevice.iscapture == 0) {
      ...
      // Don't start auto switching for a couple of seconds, because some
      // devices init on start.
      bool doAutoSwitch = g_Config.bAutoAudioDevice;
      if ((time_now_d() - g_audioStartTime) < 3.0) {
        INFO_LOG(Log::Audio, "Ignoring new audio device: %s (current: %s)",
                 name, g_Config.sAudioDevice.c_str());
        doAutoSwitch = false;
      }
      if (doAutoSwitch || g_Config.sAudioDevice == name) {
        StopSDLAudioDevice();

        INFO_LOG(Log::Audio, "!!! Auto-switching to new audio device: '%s'",
                 name);
...
static int printUsage(const char *progname) {
  ...
  fprintf(dst,
          "  --loglevel=INTEGER    set the log level to specified value\n");
  ...
  fprintf(dst, "  --fullscreen          force full screen mode, ignoring saved "
               "configuration\n");
  fprintf(dst, "  --windowed            force windowed mode, ignoring saved "
               "configuration\n");
  ...
  fprintf(dst, "                        options: gles, software, etc. (also "
               "opengl3.1, etc.)\n");

  fprintf(dst, "  --pause-menu-exit     change \"Exit to menu\" in pause menu "
               "to \"Exit\"\n");
  fprintf(dst, "  --escape-exit         escape key exits the application\n");
  fprintf(dst, "  --gamesettings        go directly to settings\n");
  fprintf(
      dst,
      "  --touchscreentest     go directly to the touchscreentest screen\n");
  fprintf(dst, "  --appendconfig=FILE   merge config FILE into the current "
               "configuration\n");
...
int main(int argc, char *argv[]) {
  ...
  // Implemented as a workaround for
  // https://github.com/hrydgard/ppsspp/issues/20687 NOTE: this is currently not
  // persistent (doesn't go to config), even though --graphics=openglX.Y also
  // sets the GPU backend which does persist.
  ...
  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_JOYSTICK | SDL_INIT_GAMECONTROLLER |
    ...
    fprintf(
        stderr,
        "Failed to initialize SDL with joystick support. Retrying without.\n");
  ...
  fprintf(stderr, "Info: We compiled against SDL version %d.%d.%d",
          compiled.major, compiled.minor, compiled.patch);
  if (compiled.minor != linked.minor || compiled.patch != linked.patch) {
    fprintf(stderr,
            ", but we are linking against SDL version %d.%d.%d., be aware that "
  ...
  // Get the video info before doing anything else, so we don't get skewed
  // resolution results.
...
#endif
  NativeInit(remain_argc, (const char **)remain_argv, path, external_dir,
             nullptr);
  ...
  if (g_Config.iGPUBackend == (int)GPUBackend::OPENGL) {
    ...
    if (glctx->Init(window, x, y, w, h, mode, &error_message,
                    force_gl_version) != 0) {
#if !PPSSPP_PLATFORM(SWITCH)
      ...
      if (!vkctx->Init(window, x, y, w, h, mode | SDL_WINDOW_VULKAN,
                       &error_message)) {
...
#else
      // On Switch, Vulkan is not available via SDL, so just fail
  ...
  SDL_SetWindowTitle(window,
                     (app_name_nice + " " + PPSSPP_GIT_VERSION).c_str());

  char iconPath[PATH_MAX];
#if defined(ASSETS_DIR)
  snprintf(iconPath, PATH_MAX, "%sui_images/icon.png", ASSETS_DIR);
  if (access(iconPath, F_OK) != 0)
    snprintf(iconPath, PATH_MAX, "%sassets/ui_images/icon.png",
  ...
  // Avoid the IME popup when holding keys. This doesn't affect all versions of
  // SDL. Note: We re-enable it in text input fields! This is necessary
  // otherwise we don't receive KeyInputFlags::CHAR events.
  SDL_StopTextInput();
  ...
  // Check if the path to a directory containing an unpacked ISO is passed as a
  ...
  if (!mainThreadIsRender) {
    ...
    while (true) {
      ...
      bool renderThreadPaused = Native_IsWindowHidden() &&
                                g_Config.bPauseWhenMinimized &&
  ...
  // If a restart was requested (and supported on this platform), respawn the
  // executable.
  if (g_RestartRequested) {
    ...
    // Hackery from
    // https://unix.stackexchange.com/questions/207935/how-to-restart-or-reset-a-running-process-in-linux,

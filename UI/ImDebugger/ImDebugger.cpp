...
void ShowInMemoryDumperMenuItem(uint32_t addr, uint32_t size, MemDumpMode mode,
                                ImControl &control) {
  if (ImGui::MenuItem(mode == MemDumpMode::Raw ? "Dump bytes to file..."
                                               : "Disassemble to file...")) {
...
// TODO: Style it more
// Left click performs the preferred action, if any. Right click opens a menu
// for more.
void ImClickableValue(const char *id, uint32_t value, ImControl &control,
                      ImCmd cmd) {
  ImGui::PushID(id);
  ...
  // Create a right-click popup menu. Restore the color while it's up. NOTE:
  // can't query the theme, pushcolor modifies it!
  ImGui::PushStyleColor(ImGuiCol_Text, g_normalTextColor);
  if (ImGui::BeginPopupContextItem(temp)) {
    if (ImGui::MenuItem(validAddr ? "Copy address to clipboard"
...
void DrawTimeView(ImConfig &cfg) {
  ...
  // Display timing
  if (ImGui::CollapsingHeader("Display Timing",
    ...
    ImGui::Text("HCount cur: %d accum: %d", __DisplayGetCurrentHcount(),
                __DisplayGetAccumulatedHcount());
...
void DrawSchedulerView(ImConfig &cfg) {
  ...
  if (ImGui::BeginChild("event_list", ImVec2(300.0f, 0.0))) {
    ...
    while (event) {
      ImGui::Text("%s (%lld): %d",
                  CoreTiming::GetEventTypes()[event->type].name,
                  event->time - ticks, (int)event->userdata);
      event = event->next;
    }
    ImGui::EndChild();
  }
  ...
  if (ImGui::BeginChild("general")) {
    ...
    ImGui::Text("Clock (MHz): %0.1f",
                (float)CoreTiming::GetClockFrequencyHz() / 1000000.0f);
...
static void DrawGPRs(ImConfig &config, ImControl &control,
                     const MIPSDebugInterface *mipsDebug,
                     const ImSnapshotState &prev) {
  ...
  if (ImGui::Button("Copy all to clipboard")) {
    ...
    for (int i = 0; i < 32; i++) {
      ...
      w.F("%s: %08x (%d)", mipsDebug->GetRegName(0, i).c_str(), value, value)
          .endl();
    }
  ...
  if (ImGui::BeginTable("gpr", 3,
    ...
    auto gprLine = [&](int index, const char *regname, int value,
      ...
      if (diff) {
        ImGui::PushStyleColor(ImGuiCol_Text, !disabled
                                                 ...
                                                 : ImDebuggerColor_DiffAlpha);
      } else if (disabled) {
        ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(255, 255, 255, 128));
      }
      // TODO: Check if the address is in the code segment to decide default
      // action.
      ImClickableValue(regname, value, control,
                       index == MIPS_REG_RA ? ImCmd::SHOW_IN_CPU_DISASM
                                            : ImCmd::SHOW_IN_MEMORY_VIEWER);
      ImGui::TableNextColumn();
    ...
    for (int i = 0; i < 32; i++) {
      ImGui::TableNextRow();
      gprLine(i, mipsDebug->GetRegName(0, i).c_str(),
              mipsDebug->GetGPR32Value(i), prev.gpr[i]);
    }
    ImGui::TableNextRow();
...
static void DrawFPRs(ImConfig &config, ImControl &control,
                     const MIPSDebugInterface *mipsDebug,
                     const ImSnapshotState &prev) {
  ImGui::SetNextWindowSize(ImVec2(320, 600), ImGuiCond_FirstUseEver);
...
static void DrawVFPU(ImConfig &config, ImControl &control,
                     const MIPSDebugInterface *mipsDebug,
                     const ImSnapshotState &prev) {
  ImGui::SetNextWindowSize(ImVec2(320, 600), ImGuiCond_FirstUseEver);
...
void WaitIDToString(WaitType waitType, SceUID waitID, char *buffer,
                    size_t bufSize) {
  switch (waitType) {
...
void DrawThreadView(ImConfig &cfg, ImControl &control) {
  ...
  if (ImGui::BeginTable("threads", 8,
    ...
    for (int i = 0; i < (int)info.size(); i++) {
      ...
      ImGui::TableNextColumn();
      if (ImGui::Selectable(thread.name, cfg.selectedThread == i,
      ...
      ImGui::TableNextColumn();
      ImClickableValue("curpc", thread.curPC, control,
                       ImCmd::SHOW_IN_CPU_DISASM);
      ImGui::TableNextColumn();
      ImClickableValue("entry", thread.entrypoint, control,
                       ImCmd::SHOW_IN_CPU_DISASM);
      ImGui::TableNextColumn();
...
void DrawParamSFO(ImConfig &cfg, ImControl &control) {
  ...
  auto renderParamSFOTable = [](ParamSFOData &data) {
    ...
    if (ImGui::BeginTable("paramsfo", 3,
                          ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH |
  ...
  if (ImGui::BeginTabBar("ParamSFOTabs")) {
    ...
    if (ImGui::BeginTabItem("GameInfo")) {
      ...
      std::shared_ptr<GameInfo> info =
          g_gameInfoCache->GetInfo(nullptr, path, GameInfoFlags::PARAM_SFO);

      if (info &&
          ImGui::BeginTable("paramsfo", 2,
                            ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH |
                                ImGuiTableFlags_Resizable)) {
  ...
  for (auto &file : fileInfo) {
    if (file.type == FileType::FILETYPE_DIRECTORY) {
      if (file.name != "." && file.name != ".." &&
          ImGui::TreeNode(file.name.c_str())) {
      ...
      if (ImGui::BeginPopupContextItem()) {
        ...
        if (ImGui::MenuItem("Save file...")) {
          ...
          System_BrowseForFileSave(
              token, "Save file", file.name, BrowseFileType::ANY,
              [fullPath, fs, size](const char *responseString, int) {
...
static void DrawKernelObjects(ImConfig &cfg) {
  ...
  if (ImGui::BeginTable("kos", 4,
    ...
    for (int i = 0; i < (int)KernelObjectPool::maxCount; i++) {
      ...
      ImGui::TableNextColumn();
      ImGui::PushID(i);
      if (ImGui::Selectable("", cfg.selectedKernelObject == i,
                            ImGuiSelectableFlags_AllowDoubleClick |
                                ImGuiSelectableFlags_SpanAllColumns)) {
...
static void DrawApctl(ImConfig &cfg) {
  ...
  if (netApctlState != PSP_NET_APCTL_STATE_DISCONNECTED &&
      ImGui::CollapsingHeader("ApCtl Details")) {
  ...
  if (g_Config.bInfrastructureAutoDNS) {
    ...
    if (dnsConfig.loaded) {
      ...
      ImGui::Text("connectAdhocForGrouping: %s",
                  BoolStr(dnsConfig.connectAdHocForGrouping));
...
static void DrawInternals(ImConfig &cfg) {
  ...
  if (ImGui::CollapsingHeader("ImGui state")) {
    ...
    ImGui::Text("WantCaptureMouseUnlessPopupClose: %s",
                BoolStr(io.WantCaptureMouseUnlessPopupClose));
    ImGui::Text("WantTextInput: %s", BoolStr(io.WantTextInput));
  }

  if (ImGui::CollapsingHeader("Save detection")) {
    ImGui::Text("Last in-game save/load: %0.1f seconds ago",
                SecondsSinceLastGameSave());
    ImGui::Text("Last save/load state: %0.1f seconds ago",
                SaveState::SecondsSinceLastSavestate());
  }
...
static void DrawAdhoc(ImConfig &cfg) {
  ...
  case 0:
    discoverStatusStr = "NONE";
    break;
  case 1:
    discoverStatusStr = "IN_PROGRESS";
    break;
  case 2:
    discoverStatusStr = "COMPLETED";
    break;
  default:
    break;
  }
  ...
  ImGui::Text("sceNetAdhocctl state: %s",
              AdhocCtlStateToString(NetAdhocctl_GetState()));
  ...
  if (ImGui::BeginTable("sock", 5,
    ...
    for (int i = 0; i < MAX_SOCKET; i++) {
      ...
      switch (socket->type) {
      case SOCK_PDP:
        ImGui::TextUnformatted("PDP");
        break;
      case SOCK_PTP:
        ImGui::TextUnformatted("PTP");
        break;
      default:
        ImGui::Text("(%d)", socket->type);
        break;
      }
      ImGui::TableNextColumn();
...
static void DrawSockets(ImConfig &cfg) {
  ...
  if (ImGui::BeginTable("sock", 9,
    ...
    for (int i = SocketManager::MIN_VALID_INET_SOCKET;
         i < SocketManager::VALID_INET_SOCKET_COUNT; i++) {
      ...
      ImGui::TableNextColumn();
      ImGui::TextUnformatted(inetSocket->nonblocking ? "Non-blocking"
                                                     : "Blocking");
      ImGui::TableNextColumn();
...
static const char *MemCheckConditionToString(MemCheckCondition cond) {
  ...
  switch ((int)cond) {
  case (int)MEMCHECK_READ:
    return "Read";
  case (int)MEMCHECK_WRITE:
    return "Write";
  case (int)MEMCHECK_READWRITE:
    return "Read/Write";
  case (int)(MEMCHECK_WRITE | MEMCHECK_WRITE_ONCHANGE):
    return "Write Change";
  case (int)(MEMCHECK_READWRITE | MEMCHECK_WRITE_ONCHANGE):
    return "Read/Write Change";
  default:
    return "(bad!)";
  }
}
...
static void DrawBreakpointsView(MIPSDebugInterface *mipsDebug, ImConfig &cfg) {
  ...
  if (ImGui::BeginTable("bp_window", 2,
                        ImGuiTableFlags_Resizable |
    ...
    if (ImGui::Button("Add MemCheck")) {
      cfg.selectedMemCheck =
          g_breakpoints.AddMemCheck(0, 0, MemCheckCondition::MEMCHECK_WRITE,
                                    BreakAction::BREAK_ACTION_PAUSE);
      cfg.selectedBreakpoint = -1;
    }

    if (ImGui::BeginTable("breakpoints", 8,
                          ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH)) {
      ...
      for (int i = 0; i < (int)bps.size(); i++) {
        ...
        if (ImGui::Selectable("", cfg.selectedBreakpoint == i,
                              ImGuiSelectableFlags_SpanAllColumns |
        ...
        ImGui::SameLine();
        ImGui::CheckboxFlags("##enabled", (int *)&bp.result,
                             (int)BREAK_ACTION_PAUSE);
        ImGui::TableNextColumn();
      ...
      for (int i = 0; i < (int)mcs.size(); i++) {
        ...
        if (ImGui::Selectable("##memcheck", cfg.selectedMemCheck == i,
                              ImGuiSelectableFlags_SpanAllColumns |
                                  ImGuiSelectableFlags_AllowOverlap)) {
          cfg.selectedBreakpoint = -1;
    ...
    if (cfg.selectedMemCheck >= 0) {
      ...
      if (ImGui::BeginChild("mc_edit")) {
        ...
        if (ImGui::BeginCombo("Condition",
                              MemCheckConditionToString(mc.cond))) {
          ...
          if (ImGui::Selectable("Read / Write",
                                mc.cond ==
                                    MemCheckCondition::MEMCHECK_READWRITE)) {
        ...
        ImGui::CheckboxFlags("Enabled", (int *)&mc.result,
                             (int)BREAK_ACTION_PAUSE);
        ImGui::InputScalar("Start", ImGuiDataType_U32, &mc.start, NULL, NULL,
                           "%08x", ImGuiInputTextFlags_CharsHexadecimal);
        ImGui::InputScalar("End", ImGuiDataType_U32, &mc.end, NULL, NULL,
                           "%08x", ImGuiInputTextFlags_CharsHexadecimal);
        if (ImGui::Button("Delete")) {
          g_breakpoints.RemoveMemCheck(mcs[cfg.selectedMemCheck].start,
                                       mcs[cfg.selectedMemCheck].end);
        }
        ImGui::EndChild();
      }
    }
...
void DrawMediaDecodersView(ImConfig &cfg, ImControl &control) {
  ...
  if (ImGui::CollapsingHeaderWithCount("sceMpeg", (int)mpegCtxs.size())) {
    if (ImGui::BeginTable("mpegs", 2,
      ...
      for (auto iter : mpegCtxs) {
        ...
        if (ImGui::Selectable(temp, iter.first == cfg.selectedMpegCtx,
                              ImGuiSelectableFlags_SpanAllColumns)) {
    ...
    if (iter != mpegCtxs.end()) {
      ...
      if (ctx &&
          ImGui::CollapsingHeader(temp, ImGuiTreeNodeFlags_DefaultOpen)) {
        // ImGui::ProgressBar((float)sas->CurPos() / (float)info.fileDataEnd,
        // ImVec2(200.0f, 0.0f));
        ImGui::Text("Mpeg version: %d raw: %08x", ctx->mpegVersion,
                    ctx->mpegRawVersion);
        ImGui::Text("Frame counts: Audio %d, video %d", ctx->audioFrameCount,
                    ctx->videoFrameCount);
        ImGui::Text("Video pixel mode: %d", ctx->videoPixelMode);
        ImGui::Text("AVC status=%d width=%d height=%d result=%d",
                    ctx->avc.avcFrameStatus, ctx->avc.avcDetailFrameWidth,
                    ctx->avc.avcDetailFrameHeight, ctx->avc.avcDecodeResult);
  ...
  if (ImGui::CollapsingHeaderWithCount("sceAtrac", atracCount,
                                       ImGuiTreeNodeFlags_DefaultOpen)) {
    ImGui::Checkbox("Force FFMPEG", &g_Config.bForceFfmpegForAudioDec);
    if (ImGui::BeginTable("atracs", 8,
                          ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH)) {
      ...
      for (int i = 0; i < maxAtracContexts; i++) {
        ...
        if (ImGui::Selectable(temp, i == cfg.selectedAtracCtx,
                              ImGuiSelectableFlags_SpanAllColumns)) {
        ...
        ImGui::TableNextColumn();
        ImGui::TextUnformatted(ctx->GetContextVersion() >= 2 ? "NewImpl"
                                                             : "Legacy");
        ImGui::PopID();
      }
    ...
    if (cfg.selectedAtracCtx >= 0 && cfg.selectedAtracCtx < PSP_MAX_ATRAC_IDS) {
      ...
      char header[32];
      snprintf(header, sizeof(header), "Atrac context %d",
               cfg.selectedAtracCtx);
      if (ctx &&
        ...
        if (isNormal) {
          ...
          ImGui::ProgressBar((float)pos / (float)endSample,
                             ImVec2(200.0f, 0.0f));
        ...
        if (ctx->context_.IsValid()) {
          ImGui::Text("ctx addr: ");
          ImGui::SameLine();
          ImClickableValue("ctx", ctx->context_.ptr, control,
                           ImCmd::SHOW_IN_MEMORY_VIEWER);
        }
        if (ctx->context_.IsValid() && ctx->GetContextVersion() >= 2) {
          const auto &info = ctx->context_->info;
          if (isNormal) {
            ImGui::Text("Buffer: (size: %d / %08x) Frame: %d", info.bufferByte,
                        info.bufferByte, info.sampleSize);
            ImGui::SameLine();
            ImClickableValue("buffer", info.buffer, control,
                             ImCmd::SHOW_IN_MEMORY_VIEWER);
            if (info.secondBuffer || info.secondBufferByte) {
              ImGui::Text("Second: (size: %d / %08x)", info.secondBufferByte,
                          info.secondBufferByte);
              ImGui::SameLine();
              ImClickableValue("second", info.secondBuffer, control,
                               ImCmd::SHOW_IN_MEMORY_VIEWER);
            }
            ImGui::Text("Data: %d/%d", info.dataOff, info.fileDataEnd);
            if (info.state != ATRAC_STATUS_STREAMED_WITHOUT_LOOP) {
              ImGui::Text("LoopNum: %d (%d-%d)", info.loopNum, info.loopStart,
                          info.loopEnd);
            }
            ImGui::Text("DecodePos: %d EndSample: %d", info.decodePos,
                        info.fileDataEnd);
            if (AtracStatusIsStreaming(info.state)) {
              ImGui::Text("Stream: offset %d, streamDataBytes: %d",
                          info.streamOff, info.streamDataByte);
            }
            ImGui::Text("numFrame: %d curBuffer: %d streamOff2: %d",
                        info.numSkipFrames, info.curBuffer,
            ...
            if (sas) {
              ImGui::ProgressBar((float)sas->CurPos() / (float)info.fileDataEnd,
                                 ImVec2(200.0f, 0.0f));
              ImGui::ProgressBar((float)sas->streamOffset /
                                     (float)sas->bufSize[sas->curBuffer],
                                 ImVec2(200.0f, 0.0f));
              ImGui::Text("Cur pos: %08x File offset: %08x File end: %08x%s",
                          sas->CurPos(), sas->fileOffset, info.fileDataEnd,
                          sas->fileOffset >= info.fileDataEnd ? " (END)" : "");
              ImGui::Text("Second (next buffer): %08x (sz: %08x)",
                          info.secondBuffer, info.secondBufferByte);
              ImGui::Text("Cur buffer: %d (%08x, sz: %08x)", sas->curBuffer,
                          sas->bufPtr[sas->curBuffer],
                          sas->bufSize[sas->curBuffer]);
              ImGui::Text("2nd buffer: %d (%08x, sz: %08x)", sas->curBuffer ^ 1,
                          sas->bufPtr[sas->curBuffer ^ 1],
                          sas->bufSize[sas->curBuffer ^ 1]);
              ImGui::Text("Loop points: %08x, %08x", info.loopStart,
                          info.loopEnd);
              ImGui::TextUnformatted(sas->isStreaming ? "Streaming mode!"
                                                      : "Non-streaming mode");
            } else {
          ...
          if (ctx->BufferState() == ATRAC_STATUS_ALL_DATA_LOADED) {
            if (ImGui::Button("Save to disk...")) {
              System_BrowseForFileSave(
                  cfg.requesterToken, "Save AT3 file", "song.at3",
                  BrowseFileType::ATRAC3,
                  [=](const std::string &filename, int) {
                    const u8 *data =
                        Memory::GetPointerRange(info.buffer, info.bufferByte);
                    if (!data) {
                      return;
                    }
  ...
  if (ImGui::CollapsingHeaderWithCount("sceMp3", (int)g_mp3Map.size(),
                                       ImGuiTreeNodeFlags_DefaultOpen)) {
    if (ImGui::BeginTable("mp3", 3,
                          ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH)) {
      ...
      for (auto &iter : g_mp3Map) {
        ...
        if (ImGui::Selectable(temp, iter.first == cfg.selectedMp3Ctx,
                              ImGuiSelectableFlags_SpanAllColumns)) {
    ...
    if (iter != g_mp3Map.end() &&
        ImGui::CollapsingHeader("MP3 %d", iter->first)) {
      ...
      if (iter->second) {
        ...
        ImGui::Text("Pos: %d (%d -> %d)", ctx->ReadPos(), (int)ctx->startPos,
                    (int)ctx->endPos);
      }
    }
  }

  if (ImGui::CollapsingHeaderWithCount("sceAac", (int)g_aacMap.size(),
                                       ImGuiTreeNodeFlags_DefaultOpen)) {
    if (ImGui::BeginTable("aac", 3,
                          ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH)) {
      ImGui::TableSetupColumn("Handle", ImGuiTableColumnFlags_WidthFixed);
      ImGui::TableSetupColumn("Channels", ImGuiTableColumnFlags_WidthFixed);
      ImGui::TableSetupColumn("ReadPos", ImGuiTableColumnFlags_WidthFixed);
      ...
      for (auto &iter : g_aacMap) {
        ...
        char temp[16];
        snprintf(temp, sizeof(temp), "%d", iter.first);
        if (ImGui::Selectable(temp, iter.first == cfg.selectedAacCtx,
                              ImGuiSelectableFlags_SpanAllColumns)) {
    ...
    if (iter != g_mp3Map.end() &&
        ImGui::CollapsingHeader("AAC %d", iter->first)) {
  ...
  if (ImGui::CollapsingHeaderWithCount("sceAudiocodec",
                                       (int)g_audioDecoderContexts.size(),
                                       ImGuiTreeNodeFlags_DefaultOpen)) {
    if (ImGui::BeginTable("codecs", 2,
                          ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH)) {
      ...
      for (auto &iter : g_audioDecoderContexts) {
        ...
        case PSP_CODEC_AAC:
          ImGui::TextUnformatted("AAC");
          break;
        case PSP_CODEC_MP3:
          ImGui::TextUnformatted("MP3");
          break;
        case PSP_CODEC_AT3PLUS:
          ImGui::TextUnformatted("Atrac3+");
          break;
        case PSP_CODEC_AT3:
          ...
          break;
        default:
          ImGui::Text("%08x", iter.second->GetAudioType());
          break;
        }
      }
      ImGui::EndTable();
...
void DrawAudioOut(ImConfig &cfg, ImControl &control) {
  ...
  if (g_Config.iAudioPlaybackMode == (int)AudioSyncMode::GRANULAR) {
    ...
    ImGui::Text("Frame time estimate: %0.1f ms",
                stats.frameTimeEstimate * 1000.0f);
    ImGui::Text("Queued samples target: %d", stats.queuedSamplesTarget);
    ImGui::Text("Queued min/max: %d / %d", stats.queuedGranulesMin,
                stats.queuedGranulesMax);
    ...
    ImGui::Text("Computed queue latency: %d ms",
                (int)(stats.smoothedQueuedGranules *
                      (GranularMixer::GRANULE_SIZE * 1000.0 / 44100.0)));
...
void DrawAudioChannels(ImConfig &cfg, ImControl &control) {
  ...
  if (ImGui::BeginTable("audios", 7,
    ...
    for (int i = 0; i < PSP_AUDIO_CHANNEL_MAX + 1; i++) {
      ...
      char id[2]{};
      id[0] = i + 1;
      ImClickableValue(id, g_audioChans[i].sampleAddress, control,
                       ImCmd::SHOW_IN_MEMORY_VIEWER);
      ImGui::TableNextColumn();
      ImGui::Text("%08x", g_audioChans[i].sampleCount);
      ImGui::TableNextColumn();
      ImGui::Text("%d | %d", g_audioChans[i].leftVolume,
                  g_audioChans[i].rightVolume);
      ImGui::TableNextColumn();
...
void DrawLogConfig(ImConfig &cfg) {
  ...
  static const char *logLevels[] = {"N/A",
...
static const char *VoiceTypeToString(VoiceType type) {
  ...
  case VOICETYPE_TRIWAVE:
    return "TRIWAVE"; // are these used? there are functions for them
                      // (sceSetTriangularWave)
  case VOICETYPE_PULSEWAVE:
    return "PULSEWAVE";
  case VOICETYPE_PCM:
    return "PCM";
...
void DrawSasAudio(ImConfig &cfg) {
  ...
  if (ImGui::BeginTable("saschannels", 9,
                        ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH)) {
    ImGui::TableSetupColumn("Index", ImGuiTableColumnFlags_WidthFixed);
    ImGui::TableSetupColumn("Type", ImGuiTableColumnFlags_WidthFixed);
    ...
    for (int i = 0; i < PSP_SAS_VOICES_MAX; i++) {
      ...
      case VOICETYPE_OFF:
        ...
        break;
      case VOICETYPE_VAG:
        ImGui::Text("%08x", voice.vag.GetReadPtr());
        break;
      case VOICETYPE_PCM:
        ImGui::Text("%08x", voice.pcmAddr + voice.pcmIndex * 2);
        break;
      default:
        ImGui::TextUnformatted("N/A");
        break;
      ...
      case VOICETYPE_OFF:
        ...
        break;
      case VOICETYPE_VAG:
        ImGui::Text("%08x", voice.vagAddr);
        break;
      case VOICETYPE_PCM:
        ImGui::Text("%08x", voice.pcmAddr);
        break;
      case VOICETYPE_ATRAC3:
        ImGui::Text("atrac: %d", voice.atrac3.AtracID());
        break;
      default:
        ImGui::TextUnformatted("N/A");
        break;
      ...
      case VOICETYPE_OFF:
        ...
        break;
      case VOICETYPE_VAG:
        ImGui::Text("%08x", voice.vagSize);
        break;
      case VOICETYPE_PCM:
        ImGui::Text("%08x", voice.pcmSize);
        break;
      case VOICETYPE_ATRAC3:
        ImGui::Text("atrac: n/a");
        break;
      default:
        ImGui::TextUnformatted("N/A");
        break;
...
static void DrawCallStacks(const MIPSDebugInterface *debug, ImConfig &config,
                           ImControl &control) {
  ...
  std::vector<DebugThreadInfo> info = GetThreadsInfo();
  // TODO: Add dropdown for thread choice, so you can check the callstacks of
  // other threads.
  u32 entry = 0;
  u32 stackTop = 0;
  ...
  if (entry != 0 &&
    ...
    std::vector<MIPSStackWalk::StackFrame> frames =
        MIPSStackWalk::Walk(debug->GetPC(), debug->GetRegValue(0, 31),
                            debug->GetRegValue(0, 29), entry, stackTop);
    ...
    for (auto &frame : frames) {
      ...
      ImClickableValue("frameentry", frame.entry, control,
                       ImCmd::SHOW_IN_CPU_DISASM);
      ...
      ImClickableValue("framepc", frame.sp, control,
                       ImCmd::SHOW_IN_MEMORY_VIEWER);
...
static void DrawUtilityModules(ImConfig &cfg, ImControl &control) {
  if (!ImGui::Begin("Utility Modules", &cfg.utilityModulesOpen) ||
      !g_symbolMap) {
    ImGui::End();
    return;
  }
  ...
  if (ImGui::BeginTable("modules", 3,
    ...
    for (const auto &iter : modules) {
      ...
      ImGui::TableNextRow();
      ImGui::TableNextColumn();
      if (ImGui::Selectable(info->name, cfg.selectedUtilityModule == i,
                            ImGuiSelectableFlags_SpanAllColumns)) {
        cfg.selectedUtilityModule = i;
      }
      ImGui::TableNextColumn();
      if (loadedAddr) {
        ImClickableValue("addr", loadedAddr, control,
                         ImCmd::SHOW_IN_MEMORY_VIEWER);
      } else {
        ImGui::TextUnformatted("-");
      }
...
static void DrawModules(const MIPSDebugInterface *debug, ImConfig &cfg,
  ...
  ImGui::TextUnformatted(
      "This shows modules that have been loaded by the game (not plain HLE)");

  if (ImGui::BeginChild("module_list", ImVec2(170.0f, 0.0),
    ...
    if (ImGui::BeginTable("modules", 2,
      ...
      // TODO: Add context menu and clickability
      kernelObjects.Iterate<PSPModule>(
          [&cfg](int id, PSPModule *module) -> bool {
            ImGui::PushID(id);
            ImGui::TableNextRow();
            ImGui::TableNextColumn();
            if (ImGui::Selectable(module->GetName(), cfg.selectedModuleId == id,
                                  ImGuiSelectableFlags_SpanAllColumns)) {
  ...
  if (ImGui::BeginChild("info")) {
    if (kernelObjects.Is<PSPModule>(cfg.selectedModuleId)) {
      ...
      if (mod) {
        ...
        ImGui::Text("%s %d.%d (%s)\n", mod->GetName(), mod->nm.version[1],
                    mod->nm.version[0], mod->isFake ? "FAKE/HLE" : "normal");
        ...
        if (!mod->impModuleNames.empty() &&
            ImGui::CollapsingHeader("Imported modules")) {
          for (auto &name : mod->impModuleNames) {
            ImGui::TextUnformatted(name);
          }
        }
        if (!mod->expModuleNames.empty() &&
            ImGui::CollapsingHeader("Exported modules")) {
        ...
        if (!mod->importedFuncs.empty() || !mod->importedVars.empty()) {
          if (ImGui::CollapsingHeader("Imports")) {
            ...
            for (auto &import : mod->importedFuncs) {
              ...
              const HLEFunction *func =
                  GetHLEFunc(import.moduleName, import.nid);
              ...
              ImGui::SameLine();
              ImClickableValue("addr", import.stubAddr, control,
                               ImCmd::SHOW_IN_CPU_DISASM);
            }
          }
        }
        if (!mod->exportedFuncs.empty() || !mod->exportedVars.empty()) {
          if (ImGui::CollapsingHeader("Exports")) {
            ...
            for (auto &exportFunc : mod->exportedFuncs) {
              // Look the name up in our HLE database.
              const HLEFunction *func =
                  GetHLEFunc(exportFunc.moduleName, exportFunc.nid);
              ...
              ImGui::SameLine();
              ImClickableValue("addr", exportFunc.symAddr, control,
                               ImCmd::SHOW_IN_CPU_DISASM);
            }
          }
        }
...
// Started as a module browser but really only draws from the symbols database,
// so let's evolve it to that.
static void DrawSymbols(const MIPSDebugInterface *debug, ImConfig &cfg,
                        ImControl &control) {
  ...
  if (ImGui::BeginTable("modules", 4,
    ...
    for (int i = 0; i < (int)modules.size(); i++) {
      ...
      ImGui::TableNextRow();
      ImGui::TableNextColumn();
      if (ImGui::Selectable(module.name.c_str(), cfg.selectedSymbolModule == i,
                            ImGuiSelectableFlags_SpanAllColumns)) {
        cfg.selectedSymbolModule = i;
      }
      ImGui::TableNextColumn();
      ImClickableValue("addr", module.address, control,
                       ImCmd::SHOW_IN_MEMORY_VIEWER);
      ImGui::TableNextColumn();
...
void ImWatchWindow::Draw(ImConfig &cfg, ImControl &control,
                         MIPSDebugInterface *mipsDebug) {
  ...
  for (auto &watch : watches_) {
    ...
    watch.evaluateFailed =
        !parseExpression(mipsDebug, watch.expression, watch.currentValue);
  }
  ...
  if (ImGui::BeginTable("watches", 4,
    ...
    for (int i = 0; i < (int)watches_.size(); i++) {
      ...
      if (editingWatchIndex_ == i && editingColumn_ == 1) {
        ...
        bool confirmed =
            ImGui::InputText("##edit", editBuffer_,
                             ImGuiInputTextFlags_EnterReturnsTrue |
                                 ImGuiInputTextFlags_AutoSelectAll);
        // Filter for only enter presses
        confirmed = confirmed && (ImGui::IsKeyPressed(ImGuiKey_Enter) ||
        ...
        bool cellHovered =
            ImGui::IsMouseHoveringRect(cellRect.Min, cellRect.Max);
      ...
      if (ImGui::BeginPopup("watchType")) {
        ...
        if (ImGui::MenuItem("Hex", nullptr, watch.format == WatchFormat::HEX)) {
          watch.format = WatchFormat::HEX;
        }
        if (ImGui::MenuItem("Int", nullptr, watch.format == WatchFormat::INT)) {
          watch.format = WatchFormat::INT;
        }
        if (ImGui::MenuItem("Float", nullptr,
                            watch.format == WatchFormat::FLOAT)) {
          watch.format = WatchFormat::FLOAT;
        }
        if (ImGui::MenuItem("Str", nullptr, watch.format == WatchFormat::STR)) {
          watch.format = WatchFormat::STR;
        }
        ImGui::EndPopup();
      }
...
void ImAtracToolWindow::Load() {
  if (File::ReadBinaryFileToString(Path(atracPath_), &data_)) {
    ...
    AnalyzeAtracTrack((const u8 *)data_.data(), (u32)data_.size(), track_.get(),
                      &error_);
  } else {
...
void ImAtracToolWindow::Draw(ImConfig &cfg) {
  ...
  if (ImGui::Button("Choose...")) {
    System_BrowseForFile(
        cfg.requesterToken, "Choose AT3 file", BrowseFileType::ATRAC3,
        [this](const std::string &filename, int) {
  ...
  if (track_.get() != 0) {
    ImGui::Text("Codec: %s",
                track_->codecType != PSP_CODEC_AT3 ? "at3+" : "at3");
    ImGui::Text("Bitrate: %d kbps Channels: %d", track_->Bitrate(),
                track_->channels);
    ImGui::Text("Frame size in bytes: %d (%04x) Output frame in samples: %d",
                track_->BytesPerFrame(), track_->BytesPerFrame(),
  ...
  if (data_.size()) {
    if (ImGui::Button("Dump 64 raw frames")) {
      std::string firstFrames =
          data_.substr(track_->dataByteOffset, track_->bytesPerFrame * 64);
      System_BrowseForFileSave(
          cfg.requesterToken, "Save .at3raw", "at3.raw", BrowseFileType::ANY,
          [firstFrames](const std::string &filename, int) {
...
void DrawHLEModules(ImConfig &config) {
  ...
  std::sort(
      ...
      [](const HLEModule *a, const HLEModule *b) { return a->name < b->name; });
  std::string label;
    ...
    if (ImGui::BeginPopupContextItem(label.c_str())) {
      if (ImGui::MenuItem("Copy as JpscpTrace")) {
        ...
        for (int j = 0; j < mod->numFunctions; j++) {
          ...
          w.F("%s 0x%08x %d %s", func.name, func.ID, strlen(func.argmask),
              amask.c_str())
              .endl();
        }
        System_CopyStringToClipboard(w.as_view());
      ...
      if (ImGui::MenuItem("Copy as imports.S")) {
        ...
        for (int j = 0; j < mod->numFunctions; j++) {
          ...
          w.F("IMPORT_FUNC  \"%.*s\",0x%08X,%s\n", (int)mod->name.size(),
              mod->name.data(), func.ID, func.name);
        }
        w.endl();
        System_CopyStringToClipboard(w.as_view());
        delete[] buffer;
...
ImDebugger::ImDebugger() {
  ...
  // Just enable ring buffer logging here, it's cheap (but needed for the log
  // window)
  g_logManager.EnableOutput(LogOutput::RingBuffer);
}

ImDebugger::~ImDebugger() { cfg_.SaveConfig(ConfigPath()); }

void ImDebugger::Frame(MIPSDebugInterface *mipsDebug,
                       GPUDebugInterface *gpuDebug, Draw::DrawContext *draw) {
  ...
  if (Achievements::HardcoreModeActive()) {
    ...
    ImGui::Text("To use the debugger, go into Settings / Tools / "
                "RetroAchievements and disable them.");
    ImGui::End();
    return;
  }
  ...
  if (lastGpuStepCount_ != GPUStepping::GetSteppingCounter()) {
    // A GPU step has happened since last time. This means that we should
    // re-center the cursor. Snapshot();
  ...
  if (ImGui::BeginMainMenuBar()) {
    if (ImGui::BeginMenu("Debug")) {
      ...
      ImGui::Separator();
      ImGui::MenuItem("Ignore bad memory accesses", nullptr,
                      &g_Config.bIgnoreBadMemAccess);
      ImGui::MenuItem("Break on frame timeout", nullptr,
                      &g_Config.bBreakOnFrameTimeout);
      ImGui::MenuItem("Don't break on start", nullptr,
                      &g_Config.bAutoRun); // should really invert this bool!
      ImGui::MenuItem("Fast memory", nullptr, &g_Config.bFastMemory);
      ImGui::Separator();
      if (ImGui::MenuItem("Take screenshot")) {
        g_TakeScreenshot = true;
      }
      ImGui::MenuItem("Save screenshot as .png", nullptr,
                      &g_Config.bScreenshotsAsPNG);
      ...
      if (System_GetPropertyBool(SYSPROP_HAS_TEXT_CLIPBOARD)) {
        if (ImGui::MenuItem("Copy memory base to clipboard")) {
          System_CopyStringToClipboard(
              StringFromFormat("%016llx", (uint64_t)(uintptr_t)Memory::base));
        }
      }
      ImGui::Separator();
    ...
    if (ImGui::BeginMenu("Symbols")) {
      ...
      if (ImGui::MenuItem("Load .ppmap...")) {
        System_BrowseForFile(
            reqToken_, "Load PPSSPP symbol map", BrowseFileType::SYMBOL_MAP,
            [&](const char *responseString, int) {
      ...
      if (ImGui::MenuItem("Save .ppmap...")) {
        System_BrowseForFileSave(
            reqToken_, "Save PPSSPP symbol map", "symbols.ppmap",
            BrowseFileType::SYMBOL_MAP, [](const char *responseString, int) {
              Path path(responseString);
      ...
      if (ImGui::MenuItem("Load No$ .sym...")) {
        System_BrowseForFile(
            reqToken_, "Load No$ symbol map", BrowseFileType::SYMBOL_MAP,
            [&](const char *responseString, int) {
              Path path(responseString);
      ...
      if (ImGui::MenuItem("Save No$ .sym...")) {
        System_BrowseForFileSave(
            reqToken_, "Save No$ symbol map", "symbols.sym",
            BrowseFileType::SYMBOL_MAP, [](const char *responseString, int) {
              Path path(responseString);
      ...
      ImGui::Separator();
      ImGui::MenuItem("Compress .ppmap files", nullptr,
                      &g_Config.bCompressSymbols);
    ...
    if (ImGui::BeginMenu("OS HLE")) {
      ...
      ImGui::MenuItem("File System Browser", nullptr,
                      &cfg_.filesystemBrowserOpen);
    ...
    if (ImGui::BeginMenu("Misc")) {
      ...
      ImGui::MenuItem("Dear ImGui Style editor", nullptr,
                      &cfg_.styleEditorOpen);
      ImGui::EndMenu();
    }
    if (ImGui::MenuItem("Close")) {
      g_Config.bShowImDebugger = false;
  ...
  case ImCmd::SHOW_IN_MEMORY_DUMPER: {
    ...
    memDumpWindow_.SetRange(control.command.param, control.command.param2,
                            (MemDumpMode)control.command.param3);
...
void ImConfig::LoadConfig(const Path &iniFile) {
  ...
  IniFile ini;
  ini.Load(iniFile); // Ignore return value, might not exist yet. In that case
                     // we'll end up loading defaults.
...
public:
  ...
  template <class T> void Sync(std::string_view key, T *value, T defaultValue) {
    if (save_) {

...
// This is the main state container of the whole Dear ImGUI-based in-game
// cross-platform debugger.
//
// Code conventions for ImGUI in PPSSPP
// * If windows/objects need state, prefix the class name with Im and just store
// straight in parent struct
...
enum class ImCmd {
  ...
  SHOW_IN_PIXEL_VIEWER,  // param is address, param2 is stride, |0x80000000 if
                         // depth, param3 is w/h
...
public:
  ...
  void PostCmd(ImCommand cmd) { externalCommand_ = cmd; }
  void DeviceLost();
